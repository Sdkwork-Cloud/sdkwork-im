...
const filters = [
  { id: 'all', name: '全部收藏', icon: <Star size={18} /> },
  { id: 'links', name: '链接', icon: <Link2 size={18} /> },
  { id: 'images', name: '图片与视频', icon: <ImageIcon size={18} /> },
  { id: 'files', name: '文件', icon: <FileText size={18} /> },
  { id: 'chat', name: '聊天记录', icon: <MessageSquare size={18} /> },
];
      ...
      <div className="flex-1 flex flex-col bg-[#1e1e1e] min-w-0 p-6 lg:p-8 overflow-y-auto custom-scrollbar">
        <div className="w-full h-full flex flex-col">
          ...
          <div className={cn(
            ...
            {loading ? (
              <div className="text-gray-500 text-sm col-span-full">加载中...</div>
            ) : filteredFavorites.length === 0 ? (
              <div className="text-gray-500 text-sm col-span-full flex justify-center py-20">暂无收藏内容</div>
            ) : (
              filteredFavorites.map(item => (
                <div 
                  ...
                  onClick={() => toast(`正在打开: ${item.title}`, 'success')}
                >
                  <button 
                    onClick={async (e) => {
                       e.stopPropagation();
                  ...
                  {viewMode === 'grid' ? (
                    <>
                      <div className="flex items-center gap-3 mb-3 shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[12px] text-gray-400 truncate">来自 {item.source}</div>
                    ...
                    <>
                      ...
                      <div className="shrink-0 flex flex-col items-end justify-center min-w-[120px]">
                        <div className="text-[12px] text-gray-400">来自 {item.source}</div>
