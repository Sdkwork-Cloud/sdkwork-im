...
import { Cloud, Search, Plus, Folder, File, FileText, Image, MoreHorizontal, Download, Share2, Grid, List, ChevronRight, X, Upload, Trash2, Edit2, Info } from 'lucide-react';
import { driveService, FolderItem, DriveFileItem } from './services/DriveService';
...
export const DriveView: React.FC = () => {
  ...
  const [selectedItem, setSelectedItem] = useState<{ type: 'folder' | 'file', id: string, name: string, size?: string, time?: string, fileType?: string } | null>(null);
  const [showNewFolderModal, setShowNewFolderModal] = useState(false);
  const [showRenameModal, setShowRenameModal] = useState(false);
  const [newFolderName, setNewFolderName] = useState('');
  const [isUploading, setIsUploading] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [breadcrumbs, setBreadcrumbs] = useState([{ id: 'root', name: '我的文件' }]);

  const refreshData = () => {
    driveService.getFolders().then(setFolders);
    driveService.getRecentFiles().then(setRecentFiles);
  };
  ...
  const getFileIcon = (type: string) => {
    switch (type) {
      ...
      case 'word': return <FileText size={40} className="text-blue-400" />;
      case 'image': return <Image size={40} className="text-green-400" />;
      case 'excel': return <File size={40} className="text-green-500" />;
  ...
  const getFileIconSmall = (type: string) => {
    switch (type) {
      ...
      case 'excel': return <File size={20} className="text-green-500" />;
  ...
  const handleDownload = (e: React.MouseEvent, fileName: string) => {
    e.stopPropagation();
    const blob = new Blob(["This is a dummy file content generated by the AI Studio environment for: " + fileName], { type: "text/plain" });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await driveService.uploadFile({
        ...
        time: '刚刚',
        type: (file.name.split('.').pop() || 'unknown') as any
      });
  ...
  const handleRenameSubmit = async (newName: string) => {
    ...
    if (selectedItem.type === 'folder') {
       ...
       setRecentFiles(recentFiles.map(f => f.id === selectedItem.id ? { ...f, name: newName } : f));
    }
    ...
    <div className="flex-1 flex flex-col bg-[#1e1e1e] min-w-0 min-h-0 relative">
      ...
      <div className="h-16 px-6 border-b border-white/5 bg-[#181818] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 text-xl font-medium text-gray-200">
           <Cloud size={24} className="text-cyan-400" /> 企业云盘
        </div>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#1e1e1e]">
            ...
            <div className="flex items-center gap-1 text-sm text-gray-400">
              {breadcrumbs.map((crumb, idx) => (
                <React.Fragment key={crumb.id}>
                  ...
                  {idx < breadcrumbs.length - 1 && <ChevronRight size={14} className="text-gray-600" />}
                </React.Fragment>
              ))}
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
            ...
            {folders.length > 0 && (
              <div className="mb-10">
                <div className="flex items-center justify-between mb-4 px-1">
                  <h2 className="text-gray-300 text-sm font-semibold tracking-wide">文件夹</h2>
                </div>
                {viewMode === 'grid' ? (
                  <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-4">
                    {folders.map((folder) => (
                      ...
                      >
                          ...
                          <div>
                            <div className="text-sm text-gray-200 font-medium truncate group-hover:text-cyan-50 transition-colors">{folder.name}</div>
                            <div className="text-xs text-gray-500 mt-1">{folder.fileCount} 个项目</div>
                          </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="bg-[#2b2b2d] border border-white/5 rounded-xl overflow-hidden">
                    <div className="grid grid-cols-12 gap-4 p-4 border-b border-white/5 text-gray-400 text-xs font-semibold bg-[#1e1e1e]/80 uppercase tracking-wider">
                        <div className="col-span-8 px-2">名称</div>
                        <div className="col-span-4 px-2">项目数</div>
                    </div>
                    {folders.map((folder) => (
                      <div 
                        key={folder.id} 
                        onClick={() => setSelectedItem({ type: 'folder', id: folder.id, name: folder.name })}
                        onDoubleClick={() => setBreadcrumbs([...breadcrumbs, { id: folder.id, name: folder.name }])}
                        className={`grid grid-cols-12 gap-4 p-3 border-b border-white/5 items-center cursor-pointer transition-colors ${selectedItem?.id === folder.id ? 'bg-cyan-900/20' : 'hover:bg-white/5'}`}
                      >
                         <div className="col-span-8 flex items-center gap-3 px-2">
                           <Folder size={20} className="text-yellow-500" />
                           <span className={`text-sm font-medium ${selectedItem?.id === folder.id ? 'text-cyan-400' : 'text-gray-200'}`}>{folder.name}</span>
                         </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4 px-1">
                <h2 className="text-gray-300 text-sm font-semibold tracking-wide">最近文件</h2>
              </div>
              ...
              {viewMode === 'grid' ? (
                <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-4">
                  {recentFiles.map((file) => (
                    <div 
                      key={file.id} 
                      onClick={() => setSelectedItem({ type: 'file', id: file.id, name: file.name, size: file.size, time: file.time, fileType: file.type })}
                    ...
                    >
                        ...
                        <div className="p-4 bg-[#2b2b2d] group-hover:bg-[#343438] transition-colors border-t border-white/5">
                           <div className="text-sm text-gray-200 font-medium truncate mb-1" title={file.name}>{file.name}</div>
                ...
                <div className="bg-[#2b2b2d] border border-white/5 rounded-xl overflow-hidden">
                   <div className="grid grid-cols-12 gap-4 p-4 border-b border-white/5 text-gray-400 text-xs font-semibold bg-[#1e1e1e]/80 uppercase tracking-wider">
                      <div className="col-span-6 px-2">名称</div>
                      <div className="col-span-2 px-2">大小</div>
                      <div className="col-span-3 px-2">修改时间</div>
                      <div className="col-span-1 text-right px-2">操作</div>
                   </div>
                   ...
                   {recentFiles.map((file) => (
                     ...
                     >
                        <div className="col-span-6 flex items-center gap-3 px-2">
                           ...
                           <span className={`text-sm font-medium truncate ${selectedItem?.id === file.id ? 'text-cyan-400' : 'text-gray-200'}`}>{file.name}</span>
                        </div>
