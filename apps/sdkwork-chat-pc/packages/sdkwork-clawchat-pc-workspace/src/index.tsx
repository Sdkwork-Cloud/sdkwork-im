...
import { Calendar, FileText, CheckSquare, Mail, PieChart, Clock, Cloud, ShieldCheck, Search, Plus, MoreHorizontal, Settings, ExternalLink, Activity, Server, Video, Image as ImageIcon, Mic, Music, PenTool } from 'lucide-react';
      ...
      <div className="bg-gradient-to-br from-indigo-900/40 via-[#181818] to-[#181818] border-b border-white/5 shrink-0 px-8 py-8 relative overflow-hidden">
         ...
         <div className="flex items-center justify-between relative z-10">
            ...
            <div className="flex gap-4">
               <div className="bg-[#2b2b2d]/80 backdrop-blur-sm border border-white/10 rounded-xl p-4 flex items-center gap-4 w-48 shadow-lg">
                  <div className="w-10 h-10 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center">
                     <CheckSquare size={20} />
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-gray-100 leading-none">3</div>
                    <div className="text-xs text-gray-400 mt-1">{t('todo')}</div>
                  </div>
               </div>
               <div className="bg-[#2b2b2d]/80 backdrop-blur-sm border border-white/10 rounded-xl p-4 flex items-center gap-4 w-48 shadow-lg">
                  ...
                  <div>
                    <div className="text-2xl font-bold text-gray-100 leading-none">12</div>
                    <div className="text-xs text-gray-400 mt-1">{t('unreadMessage')}</div>
                  </div>
               </div>
            </div>
         </div>
              ...
              <div
                ...
                onClick={() => {
                  if (onAppSelect) {
                    ...
                    toast(t('tryingLoadService', { name: t(app.nameKey), id: app.id }), 'success');
                  }
                }}
              >
                ...
                {/* Simulated notification badge for certain apps */}
                {['mail', 'approval'].includes(app.id) && (
                  <div className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-[#1e1e1e]"></div>
                )}
              </div>
            ))}
            <div 
                ...
                <input 
                  ...
                  onKeyDown={e => {
                     if (e.key === 'Enter' && newAppName.trim()) {
                        ...
                        toast(t('connectingAppCenter', { name: newAppName.trim() }), 'success');
                     }
                  }}
                />
              </div>
