...
} from '../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeDecodeService';
import type {
...
} from '../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeScanService';

const qrDecodeServiceModule = await import('../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeDecodeService') as typeof import('../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeDecodeService') & {
  default?: {
    isSupportedQrImageFile?: typeof IsSupportedQrImageFile;
  };
};
const qrScanServiceModule = await import('../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeScanService') as typeof import('../packages/sdkwork-clawchat-pc-chat/src/services/QrCodeScanService') & {
  default?: {
...
const communityFromDeepLink = parseQrCodeContent('clawchat://community/community-1');
assert.equal(communityFromDeepLink.kind, 'community');
assert.equal(communityFromDeepLink.communityId, 'community-1');

const communityFromQueryOnlyDeepLink = parseQrCodeContent('clawchat://community?communityId=query-community-1');
...
assert.match(
  desktopLibSource,
  /mod qr_code;\s*mod tray;\s*mod window_control;/u,
  'Desktop lib.rs must stay a thin module assembly and declare focused host capability modules.',
);
assert.match(
  desktopLibSource,
  /tauri::generate_handler!\[[\s\S]*window_control::sdkwork_chat_pc_window_control[\s\S]*qr_code::sdkwork_chat_pc_decode_qr_code_image[\s\S]*qr_code::sdkwork_chat_pc_decode_qr_code_rgba[\s\S]*\]/u,
  'Desktop Rust shell must register window control and native QR decode commands.',
);
assert.doesNotMatch(
  desktopLibSource,
...
assert.match(
  ...
  /allow-sdkwork-chat-pc-qr-code/u,
  'Desktop default capability must grant only the SDKWork Chat PC QR native command permission.',
);
assert.match(
...
console.log('sdkwork chat qr scan standard contract passed.');
