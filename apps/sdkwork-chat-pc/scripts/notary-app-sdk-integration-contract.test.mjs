...
const devRunnerSource = readRepoText('scripts', 'dev', 'run-sdkwork-chat-pc-dev.mjs');
const unifiedServerSource = readRepoText('scripts', 'dev', 'start-craw-chat-unified-web.mjs');
const gatewayConfigSource = readRepoText('crates', 'craw-chat-gateway-config', 'src', 'lib.rs');
const gatewaySource = readRepoText('services', 'web-gateway', 'src', 'lib.rs');
...
const notaryPackageSources = fs
  .readdirSync(path.join(appRoot, 'packages', 'sdkwork-clawchat-pc-notary', 'src'), { recursive: true })
  .filter((entry) => typeof entry === 'string' && /\.(?:ts|tsx)$/u.test(entry))
  .map((entry) => readText('packages', 'sdkwork-clawchat-pc-notary', 'src', entry))
  .join('\n');
...
assert.match(
  gatewayConfigSource,
  /sdkwork-notary-app-api[\s\S]*CRAW_CHAT_NOTARY_APP_API_UPSTREAM[\s\S]*SDKWORK_NOTARY_APP_API_UPSTREAM[\s\S]*SDKWORK_NOTARY_APP_API_BASE_URL/u,
...
assert.match(
  ...
  'Core notary client must share the SDKWork Chat global token manager.',
);
...
console.log('sdkwork chat notary app SDK integration contract passed.');
