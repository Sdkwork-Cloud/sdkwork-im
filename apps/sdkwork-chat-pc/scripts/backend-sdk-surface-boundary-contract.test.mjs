...
const pcCoreIndexSource = readFileSync(
  path.join(packagesRoot, 'sdkwork-clawchat-pc-core', 'src', 'index.ts'),
  'utf8',
);
const adminCoreSdkIndexSource = readFileSync(
  path.join(packagesRoot, 'sdkwork-clawchat-admin-core', 'src', 'sdk', 'index.ts'),
  'utf8',
);
...
function isAdminSourceFile(filePath) {
  ...
  return normalized.includes('/packages/sdkwork-clawchat-admin-')
    && normalized.includes('/src/');
}

assert.ok(statSync(packagesRoot).isDirectory(), 'Expected sdkwork-chat-pc packages directory to exist.');
...
console.log('sdkwork chat pc backend SDK surface boundary contract passed.');
