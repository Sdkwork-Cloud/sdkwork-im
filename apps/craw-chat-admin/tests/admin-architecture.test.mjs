...
test('workspace donor roots include verified worktree app dependencies', () => {
  ...
  const expectedWorktreeAppRoots = listVerifiedWorktreeAdminRoots(workspaceRoot);

  assert.equal(
    expectedWorktreeAppRoots.length > 0,
    true,
    'test setup should include at least one verified worktree app donor root',
  );
  assert.equal(
    expectedWorktreeAppRoots.every((candidateRoot) => donorRoots.includes(candidateRoot)),
    true,
    'official app should discover every verified worktree app donor root',
  );
});
