...
export function UsersPage({ snapshot }: AdminPageProps) {
  ...
  const recoveryQueue = users.filter((user) => !user.active).slice(0, 3).map((user) => ({
    id: `recovery-${user.id}`,
    name: user.name,
    reason: t('Inactive identities require operator recovery review before access can be restored.'),
  }));
  const riskWatchlist: Array<{ id: string; name: string; signal: string }> = [];
          ...
          >
            <div className="space-y-3">
              {riskWatchlist.length > 0
                ...
                : (
                    <AdminEmptyState
                      detail={t('Dedicated device, trust, and abnormal recovery signals will appear here once the workspace exposes them.')}
                      title={t('No risk signals are reported for identities yet.')}
                    />
                  )}
            </div>
          </AdminSectionCard>
    ...
    >
      ...
      >
        <div className="space-y-3">
          {users.length > 0
            ? users.map((user) => (
                <AdminInsetSplitRow key={user.id}>
                  <div>
            ...
            : (
                <AdminEmptyState
                  detail={t('Operator and portal identities will appear here once the workspace sync returns managed users.')}
                  title={t('No identity roster is loaded yet.')}
                />
              )}
        </div>
      </AdminSectionCard>
      ...
      >
        <div className="space-y-3">
          {recoveryQueue.length > 0
            ...
            : (
                <AdminEmptyState
                  detail={t('Recovery requests will appear here when inactive identities need operator approval before access is restored.')}
                  title={t('No recovery reviews are pending.')}
                />
              )}
        </div>
      </AdminSectionCard>
