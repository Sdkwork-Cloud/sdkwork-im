  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[min(92vw,46rem)]">
        <DialogHeader>
          <DialogTitle>{draft.id ? t('Edit workspace') : t('New workspace')}</DialogTitle>
          <DialogDescription>
            {t(
              'Workspace creation happens in a dedicated dialog so the registry remains the primary operating surface.',
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <form className="space-y-6" onSubmit={onSubmit}>
          <FormSection
            description={t(
              'Workspaces inherit tenant scope and become the primary boundary for usage, audit, and key issuance.',
            )}
          ...
          >
            <FormGrid columns={2}>
              ...
              <DialogField htmlFor="project-id" label={t('Workspace ID')}>
                <Input
                  id="project-id"
                  onChange={(event: ChangeEvent<HTMLInputElement>) =>
              ...
              <DialogField htmlFor="project-name" label={t('Workspace name')}>
                <Input
                  id="project-name"
                  onChange={(event: ChangeEvent<HTMLInputElement>) =>
          ...
          <FormActions>
            ...
            <Button type="submit" variant="primary">
              {draft.id ? t('Save workspace') : t('Create workspace')}
            </Button>
          </FormActions>
        </form>
      </DialogContent>
