    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {t('Active workspaces, portal users, and live access key coverage for the selected tenant.')}
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 text-sm sm:grid-cols-2">
          <div className="space-y-1">
            ...
            <div className="text-[var(--sdk-color-text-secondary)]">
              {t('Tenant ID: {id}', { id: selectedTenant.id })}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-[var(--sdk-color-text-muted)]">{t('Workspace footprint')}</div>
            <div className="font-medium text-[var(--sdk-color-text-primary)]">
              {selectedTenant.projectSummary}
            </div>
            <div className="text-[var(--sdk-color-text-secondary)]">
              {t('{count} workspaces attached', {
                count: formatNumber(selectedTenant.projectCount),
              })}
            </div>
          </div>
          <div className="space-y-1">
            <div className="text-[var(--sdk-color-text-muted)]">{t('Access coverage')}</div>
            <div className="font-medium text-[var(--sdk-color-text-primary)]">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('Linked workspaces')}</CardTitle>
          <CardDescription>
            {t('The busiest workspaces stay visible here so operators can judge ownership, traffic, and access key coverage before opening another dialog.')}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {selectedTenant.projectRecords.length ? (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-[var(--sdk-color-text-secondary)]">
                      {t('Workspace ID: {id}', { id: project.id })}
                    </div>
                  </div>
            ...
            <div className="rounded-2xl border border-dashed border-[var(--sdk-color-border-default)] p-4 text-sm text-[var(--sdk-color-text-secondary)]">
              {t('No workspaces are linked to this tenant yet.')}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <InlineAlert
        description={
          selectedTenant.canIssueApiKey
            ? t('At least one workspace exists, so live access key issuance can proceed immediately.')
            : t('Issue live access keys only after at least one workspace exists for the selected tenant.')
        }
        showIcon
