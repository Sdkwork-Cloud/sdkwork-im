    ...
    <>
      <AdminPageFrame
        ...
        rail={
          <div className="space-y-6">
            ...
            >
              <div className="space-y-3 text-sm text-[var(--admin-text-secondary)]">
                <div>{t('Live workspace status: {status}', { status })}</div>
                <div>{t('Detail drawers keep the selected tenant in view while edits stay isolated in dialogs.')}</div>
                <div>{t('Key issuance remains guarded until a workspace environment exists and coverage can be reviewed.')}</div>
              </div>
            </AdminSectionCard>
          </div>
        }
      ...
      >
        <div className="grid gap-4 md:grid-cols-3">
          ...
          />
          <AdminMetricCard
            detail={t('Workspace environments connected to the operator shell.')}
            label={t('Workspace')}
