...
import { adminRoutes, useAdminI18n } from 'sdkwork-control-plane-core';
...
export function SettingsPage() {
  ...
  const sections = useMemo<SettingsCenterSection[]>(
    () => [
      {
        title: t('Control-plane workspace'),
        items: items
    ...
    <SettingsCenter
      actions={
        <div className="flex items-center gap-2">
          <SettingsBadge variant="secondary">{t('control-plane settings center')}</SettingsBadge>
      ...
      description={t(
        'This workspace keeps operator preferences, transcript review density, realtime visibility, and moderation continuity aligned across the shared shell.',
      )}
      emptyState={
        <InlineAlert
      ...
      navHeader={
          <div className="space-y-3">
            <SettingsBadge variant="outline">{t('Control-plane workspace')}</SettingsBadge>
