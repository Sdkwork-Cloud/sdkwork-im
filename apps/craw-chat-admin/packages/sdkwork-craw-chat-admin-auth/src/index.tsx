...
function resolveLoginFeedbackClassName(status: string, loading: boolean) {
  ...
  if (
    ...
    normalized.includes('refreshing live im admin data') ||
...
export function AdminLoginPage({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === 'register') {
      setFeedback(
        t(
          'Operator account requests stay inside the IM operator workspace. Ask an existing admin to review and provision {name} access from Identity.',
