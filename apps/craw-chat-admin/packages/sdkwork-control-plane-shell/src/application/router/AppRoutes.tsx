...
import { AdminLoginPage } from 'sdkwork-control-plane-auth';
import {
  ADMIN_ROUTE_PATHS,
  isAdminAuthPath,
  useAdminI18n,
...
const OverviewPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-overview')).OverviewPage,
}));
const TenantsPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-tenants')).TenantsPage,
}));
const UsersPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-users')).UsersPage,
}));
const GroupsPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-groups')).GroupsPage,
}));
const AnnouncementsPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-announcements')).AnnouncementsPage,
}));
const ConversationsPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-conversations')).ConversationsPage,
}));
const MessagesPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-messages')).MessagesPage,
}));
const ModerationPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-moderation')).ModerationPage,
}));
const AutomationPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-automation')).AutomationPage,
}));
const RealtimePage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-realtime')).RealtimePage,
}));
const SystemPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-system')).SystemPage,
}));
const StoragePage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-storage')).StoragePage,
}));
const SettingsPage = lazy(async () => ({
  default: (await import('sdkwork-control-plane-settings')).SettingsPage,
}));
    ...
    <div className="admin-shell-route-stage admin-shell-route-stage-loading">
      <div className="admin-shell-route-scroll">
        <div className="flex min-h-full flex-col items-center justify-center gap-4 text-center">
          <LoadingBlock label={t('Synchronizing control-plane workspace...')} />
