...
const DEFAULT_ADMIN_SANDBOX_PASSWORD = 'ChangeMe123!';
...
function defaultSandboxPassword(seed = sandboxSeed) {
  return normalizeCredentialString(seed?.sandboxPassword ?? seed?.sandbox_password)
    || DEFAULT_ADMIN_SANDBOX_PASSWORD;
}

function generateFallbackSandboxPassword() {
...
export function resolveAdminSandboxCredentials({
  ...
  const password = explicitPassword
    || envPassword
    || defaultSandboxPassword(seed)
    || generateFallbackSandboxPassword();
