...
export class ImMessagesModule {
    ...
    send(message) {
        if (message.target.channel === 'system') {
            return this.context.transportClient.conversation.publishSystemChannelMessage(message.target.conversationId, message.body);
        }
        return this.context.transportClient.conversation.postConversationMessage(message.target.conversationId, message.body);
    }
