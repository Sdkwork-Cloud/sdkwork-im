    ...
    constructor(receiver) {
        ...
        this.receiver.onSocketError((event) => {
            const error = event instanceof Error
                ? event
                : new CrawChatSdkError('websocket_transport_error', 'Realtime websocket transport error.');
            this.emitError({
                code: 'socket_error',
                source: 'socket',
                error,
...
export class CrawChatLiveModule {
    ...
    async connect(options = {}) {
        const receiver = new CrawChatWebSocketReceiver(new CrawChatRealtimeModule(this.context), this.context, {
            url: options.url,
            mode: 'legacy_json',
