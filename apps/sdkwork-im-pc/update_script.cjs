...
const notaryDir = 'packages/sdkwork-im-pc-notary/src';
execSync(`find ${notaryDir} -type f -name '*.tsx'`).toString().trim().split('\n').filter(Boolean).forEach(file => {
  updateFile(file, [
    [/import \{\s*notaryService\s*\} from '@sdkwork\/im-pc-core';/g, "import { notaryService } from './services/NotaryService';"]
  ]);
});
...
// 5. Update drive
const driveDir = 'packages/sdkwork-im-pc-drive/src';
