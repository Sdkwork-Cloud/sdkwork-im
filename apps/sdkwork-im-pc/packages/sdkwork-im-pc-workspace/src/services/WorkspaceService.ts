...
const recentDocumentCatalog: DocumentItem[] = [
...
class WorkspaceCatalogService implements WorkspaceService {
  async getApps(): Promise<AppItem[]> {
    return new Promise(resolve => {
      setTimeout(() => resolve([...workspaceAppCatalog]), 300);
    });
  }

  async getRecentDocuments(): Promise<DocumentItem[]> {
    return new Promise(resolve => {
      setTimeout(() => resolve([...recentDocumentCatalog]), 400);
    });
  }
  ...
  async searchApps(query: string): Promise<AppItem[]> {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        resolve(workspaceAppCatalog.filter(app => app.name.includes(query) || app.name.toLowerCase().includes(lowered)));
      }, 300);
    });
  }
  ...
  async addRecentDocument(doc: DocumentItem): Promise<void> {
    return new Promise(resolve => {
      setTimeout(() => {
        recentDocumentCatalog.unshift(doc);
        resolve();
      }, 200);
    });
  }
  ...
  async deleteRecentDocument(id: string): Promise<void> {
    return new Promise(resolve => {
      setTimeout(() => {
        const index = recentDocumentCatalog.findIndex(d => d.id === id);
        if (index > -1) recentDocumentCatalog.splice(index, 1);
        resolve();
      }, 200);
    });
  }
  ...
  async addApp(app: AppItem): Promise<void> {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const existingIndex = workspaceAppCatalog.findIndex((item) => item.id === app.id);
        if (existingIndex > -1) {
          workspaceAppCatalog[existingIndex] = app;
        } else {
          workspaceAppCatalog.push(app);
        }
        resolve();
      }, 200);
    });
  ...
  async removeApp(id: string): Promise<void> {
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        const index = workspaceAppCatalog.findIndex(a => a.id === id);
        if (index > -1) workspaceAppCatalog.splice(index, 1);
        resolve();
      }, 200);
    });
  }
...
export const workspaceService = new WorkspaceCatalogService();
