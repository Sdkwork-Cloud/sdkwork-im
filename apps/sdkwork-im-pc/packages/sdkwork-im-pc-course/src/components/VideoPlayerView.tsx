...
export const VideoPlayerView: React.FC<VideoPlayerViewProps> = ({ course, onBack }) => {
  ...
  const [commentCount, setCommentCount] = useState(course.comments?.length ?? 0);
  ...
  const handleSendComment = async () => {
    ...
    try {
      await courseInteractionService.createComment(course.id, inputText.trim());
      setInputText("");
    } catch {
      return;
    }
      ...
      <div className="w-[400px] bg-[#121214] border-l border-white/5 flex flex-col shrink-0 relative z-20 shadow-2xl">
        ...
        <div className="p-6 border-b border-white/5 bg-gradient-to-b from-[#18181b]/80 to-[#121214]">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3 cursor-pointer group">
              <Avatar
                ...
                size="lg"
                shape="circle"
                className="border-2 border-white/10 group-hover:border-indigo-400/50 transition-colors shadow-md"
              />
              <div className="flex flex-col justify-center">
                ...
                <p className="text-[12px] text-gray-500 font-medium">{course.students?.toLocaleString() ?? 0} 人在学</p>
              </div>
            </div>
            <button 
          ...
          <div className="text-[11px] font-medium text-gray-500 flex items-center gap-3">
            <span className="flex items-center gap-1"><Users size={12}/> {course.students?.toLocaleString() ?? 0} 人在学</span>
              ...
              {comments.length === 0 ? (
                ...
                comments.map(comment => (
                  <div key={comment.id} className="flex gap-3 group/comment">
                    <Avatar src={comment.avatar || undefined} alt={comment.user} fallback={comment.user?.charAt(0) ?? "?"} size="sm" shape="circle" className="border border-white/5" />
        ...
        {activeTab === "comments" && (
          <div className="p-4 bg-[#18181b] border-t border-white/5">
            <div className="flex items-center gap-3">
              <Avatar src={currentUserAvatar || undefined} alt={currentUserName} fallback={currentUserName.charAt(0) || "?"} size="sm" shape="circle" className="border border-white/10" />
