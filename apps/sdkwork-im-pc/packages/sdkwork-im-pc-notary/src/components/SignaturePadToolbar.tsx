        ...
        <div className="flex items-center gap-3">
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            >
              ...
              <span>{ratios.find(r => r.value === aspectRatio)?.label}</span>
            ...
            {openDropdown === 'ratio' && (
              <div className="absolute bottom-full left-0 mb-2 w-28 bg-[#2a2a2d] border border-white/10 rounded-lg shadow-xl overflow-hidden py-1 z-20">
                {ratios.map(ratio => (
                  <button
                    key={ratio.value}
                    onClick={() => { setAspectRatio(ratio.value); setOpenDropdown(null); }}
            ...
            {openDropdown === 'color' && (
              <div className="absolute bottom-full left-0 mb-2 w-32 bg-[#2a2a2d] border border-white/10 rounded-lg shadow-xl overflow-hidden py-1 z-20">
                {colors.map(color => (
                  <button
                    key={color.value}
                    onClick={() => { setPenColor(color.value); setOpenDropdown(null); }}
          ...
          >
            <RotateCcw size={14} /> <span>清除重写</span>
          </button>
        </div>
      </div>
