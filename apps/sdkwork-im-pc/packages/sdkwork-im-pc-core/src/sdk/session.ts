...
function readPersistedSessionRawValue(): string | null {
  ...
  const fromSession = sessionStorage?.getItem(SDKWORK_IM_SESSION_KEY);
  if (fromSession) {
    persistedSessionRawValueCache = fromSession;
    return fromSession;
  }

  const legacyValue = getLocalStorage()?.getItem(SDKWORK_IM_LEGACY_SESSION_STORAGE_KEY) ?? null;
...
function migrateLegacySessionStorage(): void {
  ...
  const legacyValue = localStorage.getItem(SDKWORK_IM_LEGACY_SESSION_STORAGE_KEY);
  if (!legacyValue || sessionStorage.getItem(SDKWORK_IM_SESSION_KEY)) {
    return;
  }

  sessionStorage.setItem(SDKWORK_IM_SESSION_KEY, legacyValue);
  localStorage.removeItem(SDKWORK_IM_LEGACY_SESSION_STORAGE_KEY);
}
