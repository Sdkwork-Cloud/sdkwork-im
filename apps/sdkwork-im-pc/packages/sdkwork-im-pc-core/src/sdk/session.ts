...
export function readAppSdkSessionTokens(): SdkworkChatSession | null {
  ...
  const rawValue = storage.getItem(SDKWORK_IM_SESSION_KEY);
  if (!rawValue) {
    return null;
  }
  ...
  try {
    ...
    storage.removeItem(SDKWORK_IM_SESSION_KEY);
    return null;
  }
}
...
export function persistAppSdkSessionTokens(session: SdkworkChatSession): SdkworkChatSession {
  ...
  getStorage()?.setItem(SDKWORK_IM_SESSION_KEY, JSON.stringify(normalizedSession));
...
export function clearAppSdkSessionTokens(): void {
  getStorage()?.removeItem(SDKWORK_IM_SESSION_KEY);
