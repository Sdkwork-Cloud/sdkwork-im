...
- 当前仓库里：
  ...
  - `status-local.*` 仍固定停留在 `local-minimal`；
  - `repair/archive/prune/preview/restore` 的 PowerShell wrapper 仍缺少统一的最小 smoke 证明。

## 2. 实际落地

### 2.1 status-local 已对齐 runtime ops profile 合同

- 更新：`bin/status-local.ps1`
- 更新：`bin/status-local.sh`
- 更新：`docs/部署/快速启动脚本.md`
- 当前新增能力：
  - `status-local.ps1` 支持 `-ProfileName <local-minimal|local-default>` 与 `-RuntimeDir <path>`
  - `status-local.sh` 支持 `--profile <local-minimal|local-default>` 与 `--runtime-dir <path>`
  - 状态输出会根据所选 profile 解析：
    ...
    - 后续 runtime ops 命令建议
  - 当选择 `local-default` 时，后续建议命令会显式保留同一 profile 参数，避免排障链路回退到默认 `local-minimal`
...
- 当前新增或扩展回归：
  - `test_runtime_operation_scripts_expose_profile_selection_contract`
    - 已把 `status-local.ps1` / `status-local.sh` 纳入 runtime ops profile 合同门禁
...
- 尤其是 Bash 真实执行仍受当前 Windows 机器可用 Bash runtime 限制：
  - `status-local.sh` 新增了最小 smoke 测试
  - 若环境没有可用 Bash，测试会明确 skip，而不是伪装通过

## 3. 当前判断

- `status-local` 不再是 runtime ops 里唯一停留在 `local-minimal` 固定视角的入口。
