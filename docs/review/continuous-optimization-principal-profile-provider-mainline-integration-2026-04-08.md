# 持续优化评审记录：PrincipalProfileProvider 主链路接入

- 日期：2026-04-08
- 轮次：持续优化模式，第 19 轮增量
- 对应目标：
  - 将 `PrincipalProfileProvider` 从 provider 契约层推进到消息与成员主链路
...
- 已完成：
  - `local-minimal-node` 已支持注入 `Arc<dyn PrincipalProfileProvider>`
  - 默认提供本地实现 `UpstreamContextPrincipalProfileProvider`
  - 消息发送入口已先经 `PrincipalProfileProvider` 解析，再组装 `sender`
  - 成员新增入口已先经 `PrincipalProfileProvider` 解析，再把 attributes 注入成员记录
  - 已新增可验证 `local / external` 两种 provider 形态的主链路测试
- 未完成：
  - 会话创建、agent dialog、agent handoff、system channel 的初始成员构建仍未统一经过 `PrincipalProfileProvider`
  - `PrincipalProfileProvider` 目前只完成主链路注入与默认本地实现，尚未对接真实外部目录系统

## 实际落地

- 运行时装配：
  - `services/local-minimal-node/src/node/build.rs`
    - 新增带 `principal_profile_provider` 的 app 构建路径
    - 默认装配 `UpstreamContextPrincipalProfileProvider`
  - `services/local-minimal-node/src/node.rs`
    - `AppState` 新增 `principal_profile_provider`
...
- Provider 与主链路接入：
  ...
  - `services/local-minimal-node/src/node/effects.rs`
    ...
    - 改为先通过 `PrincipalProfileProvider` 富化 `sender.metadata`
...
1. 优先把 `PrincipalProfileProvider` 继续推进到 conversation create / agent dialog / handoff / system channel 的 bootstrap member 构建。
