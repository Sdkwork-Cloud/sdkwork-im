...
- 当前 Step：`04`
- 当前状态：`已启动，未闭环`
- 本轮结论：`需要做部分 docs/架构 回写，97 尚未整体通过`
...
这说明 Link Plane 已经从“只有 step 文档中的概念”进入“有真实 crate owner 的状态”。

### 3.2 `runtime-route` 最小 owner
...
- `session-gateway` 真实连接热路径尚未迁到 `runtime-link`
- `session-gateway` 真实 route owner 尚未迁到 `runtime-route`
- `local-minimal-node` 尚未变成 Link / Route runtime 的明显装配层
...
- 本轮新增的 `docs/架构/09 / 131 / 136` as-built 回写

## 8. 决议

- `Step 04` 已进入真实实施阶段
- 本轮允许继续留在 `Step 04` 深挖热路径抽离
- 本轮不允许宣布 `95 / 97` 通过
...
- 前文中“`session-gateway` 真实 route owner 尚未迁到 `runtime-route`”的判断已失效
- 最新 as-built 为：
  - `craw-chat-runtime-route` 已成为 `session-gateway` route ownership / epoch / drain / migration 的公开 owner
  - `session-gateway/src/cluster.rs` 已降为 runtime 集成层，负责：
    - route 切换前后的 runtime 状态转移
    - cluster 级错误包装
    - disconnect fence 与 route owner 的编排衔接

### 10.2 本轮新增架构回写范围

- `docs/架构/09-实施计划.md`
  - 补充 `Wave B / Step 04` 从 skeleton 进入 route owner 集成阶段的 as-built
- `docs/架构/131-连接管理与分层弹性扩容架构设计-2026-04-06.md`
  - 补充 Route Plane owner 已切入真实 gateway 集成层
- `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
...
- 验证：
  - fail-first route owner 测试先 red 后 green
...
- `97` 继续维持“部分完成”
- 原因：
  - Route Plane 的 owner 收口已形成真实回写依据
  - 但 Link Plane 热路径和 `local-minimal-node` 装配收口仍未完成，不能宣告 Step 04 整体兑现

## 11. 2026-04-07 Link Plane 增量回写决议

## 12. 2026-04-07 local-minimal-node 装配收口回写决议

### 12.1 对应架构能力

- `docs/架构/09-实施计划.md`
  - `Wave B / Step 04` 的 Link / Route runtime 收口与 as-built 追踪
- `docs/架构/131-连接管理与分层弹性扩容架构设计-2026-04-06.md`
  - 单设备单权威 route owner、drain / migrate 期间 owner 一致性
- `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
  - “旧节点 draining -> 新 owner 生效 -> 旧节点释放 ownership” 的时序不允许逆写

### 12.2 本轮确认已兑现的能力

- `local-minimal-node` 已不再把 realtime cluster / runtime / presence 拆散在本地 build surface 内部
- `session-gateway` 已提供统一 `RealtimePlaneAssembly`
- `local-minimal-node` 已通过该 bundle 显式装配 Link / Route runtime
- 旧 drain 节点上的非 takeover 设备绑定请求会先通过 locality 校验
- 当请求已经不属于本地 route owner 时，系统会在状态变更之前拒绝该请求，避免 route owner / runtime owner 重新回流到旧节点
...
- `CP04-2` 仍未完成
  ...
  - queue / backpressure
  - resume / reconnect
  这些 Link Plane 热路径 owner 仍主要留在 `session-gateway`

### 12.4 偏差判断

- 未发现与 `131 / 136` 冲突的实现偏差
- 本轮属于“按架构收口边界”的纠偏，不是额外扩展
- 因为 `CP04-2` 未过，所以 Step 04 整体仍不能按 `95 / 97` 宣告通过
...
- `OutboundQueuePolicy` 的默认实时出站队列阈值已进入 `craw-chat-runtime-link`
- `session-gateway` 已从“定义默认阈值”降级为“消费 owner 提供的默认策略”

### 13.2 架构判断

- 这属于 `CP04-2` 的真实前推，而不是样式性重命名
- 它把 Link Plane 的 queue/backpressure 默认语义从服务实现层推进到了 runtime owner 层
- 但这还不是 `CP04-2` 的完成态，因为：
  - 默认阈值 owner 已迁移
...
- `runtime-link` 已从“仅有 crate skeleton”推进到“websocket 热路径开始真实消费”
- 这次兑现的具体落点：
  - `LinkSession` 已在 `serve_realtime_websocket` 中创建
  - `RealtimeWindowCheckpoint -> ResumeWindow` 已形成真实映射
...
- `docs/架构/09-实施计划.md`
  - `Wave B / Step 04` 的 Link Plane hot path owner 增量收口
- `docs/架构/131-连接管理与分层弹性扩容架构设计-2026-04-06.md`
  - 连接身份语义继续从服务实现层迁往 `runtime-link`
- `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
  - 握手阶段的 `auth_bind -> auth_ok` 身份匹配开始由 `LinkSession` owner 承担

### 14.2 本轮已兑现能力

- `LinkSession` 不再只承载状态、默认队列和恢复窗口
- `LinkSession` 现在开始持有 `principal / actor_kind / device / session` 的身份匹配语义
- websocket 握手热路径已经直接消费 `link_session.matches_auth_bind(...)`
...
- 本轮未发现与 `131 / 136 / 147` 冲突的实现偏离
- 这次变更属于“继续把 Link Plane 身份语义从服务层回收到 runtime owner”，不是额外扩张 crate 职责

### 14.5 回写决议

- 允许把本轮 as-built 继续回写到：
  - `docs/架构/09-实施计划.md`
  - `docs/架构/131-连接管理与分层弹性扩容架构设计-2026-04-06.md`
  - `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
- 但不允许据此宣布：
...
- `docs/架构/09-实施计划.md`
  - `Wave B / Step 04` 的 Link Plane hot path 增量收口
- `docs/架构/131-连接管理与分层弹性扩容架构设计-2026-04-06.md`
  - `hello / accept` 协商 owner 继续从服务层回收至 `runtime-link`
- `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
...
- HTTP/WebSocket accept / upgrade 接入层还在 `session-gateway`
...
- 本轮未发现与 `131 / 136 / 147` 冲突的实现偏离
- 这次变化属于继续把 Link Plane 协商语义从服务层回收到 runtime owner，而不是新增架构外职责
...
- `docs/架构/147-CCP到Crate与接口模块落地映射设计-2026-04-06.md`
  - `session_resume` 语义向 `craw-chat-runtime-link` 映射

### 16.2 本轮已兑现能力

- `runtime-link` 新增 `ResumeDecision`
- `runtime-link` 新增 `decide_resume(...)`
- `session-gateway` 的 `SessionPresenceRuntime::resume(...)` 已开始直接消费 `runtime-link` 的 resume 结果
- 这说明 `resume_required / resume_from_sync_seq / latest_sync_seq` 的纯语义已不再由服务层私有计算

### 16.3 本轮仍未兑现能力

- `session_resume / session_resumed` 控制帧时序仍未完整迁入 `runtime-link`
...
- 本轮未发现与 `131 / 136 / 147` 冲突的实现偏离
- 本轮变化属于继续把 Link Plane 的恢复语义从服务层收口到 runtime owner，而不是新增架构外职责
...
- `docs/架构/136-关键业务链路与跨Plane时序设计-2026-04-06.md`
  - 断连 / 迁移前的 `goaway` 时序
- `docs/架构/147-CCP到Crate与接口模块落地映射设计-2026-04-06.md`
  - `goaway` 语义向 `craw-chat-runtime-link` 映射

### 17.2 本轮已兑现能力

- `runtime-link` 新增 `LinkGoAwayDirective`
- `runtime-link` 新增 `session_disconnect_goaway()`
- `session-gateway` 的 CCP websocket 断连路径已开始直接消费该 owner：
  - 先发 `goaway`
  - 再发 close
- `SESSION_DISCONNECT_CLOSE_CODE / REASON` 已从服务侧常量改为对齐 runtime owner 常量

### 17.3 本轮仍未兑现能力

- 完整 `session_resume / session_resumed` 控制帧时序仍未迁入 `runtime-link`
- HTTP/WebSocket accept / upgrade 仍在 `session-gateway`
- 真实 queue/backpressure 执行 owner 仍在 `session-gateway`
- 因此 `CP04-2` 仍未通过
...
- 本轮未发现与 `131 / 136 / 147` 冲突的实现偏离
- 本轮变化属于把 Link Plane 的断连控制语义从服务层收口到 runtime owner，而不是引入新的协议分叉
...
## 18. 2026-04-07 runtime-link session_resume 控制帧 owner 回写决议
...
- `docs/架构/147-CCP到Crate与接口模块落地映射设计-2026-04-06.md`
  - `session_resume / session_resumed` 从 CCP 控制帧映射到 `craw-chat-runtime-link`

### 18.2 本轮已兑现能力

- `runtime-link` 已不再只持有恢复窗口的纯计算
- `runtime-link` 现在已经开始持有：
  - `session_resume` 的 `session_id` 校验
  - `catchup_after_seq` 计算
  - `session_resumed` 控制帧产出
...
- 本轮未发现与 `131 / 136 / 147` 冲突的实现偏差
- 本轮属于把既有 Step 04 架构要求继续落到真实 crate owner，而不是引入新协议分支
...
- `runtime-link` 已拥有 WebSocket upgrade 的最小 owner contract
  - `LINK_WEBSOCKET_SUBPROTOCOL`
  - `LinkWebsocketMode`
  - `supported_websocket_subprotocols()`
  - `select_websocket_mode(...)`
- `session-gateway` 仅保留 Axum `WebSocketUpgrade` 适配，并把 `runtime-link` owner contract 映射为 `RealtimeWebsocketMode`
- `services/session-gateway/src/websocket.rs` 对外暴露的 `CCP_WEBSOCKET_SUBPROTOCOL` 已跟随 `runtime-link` owner 常量

### 19.3 本轮未兑现能力

- `on_upgrade(...)` 执行动作仍未从 `session-gateway` 回收到 `runtime-link`
...
- 未偏离
- 本轮改动符合 `131 / 136 / 147` 中 “Link Plane owner 下沉到 runtime crate，transport adapter 仅做适配”的方向
- 与既有 `session_resume / session_resumed / goaway` owner 回收路径连续，没有引入新的协议旁路

### 19.5 回写决议

- 需要回写 `09 / 131 / 136 / 147`
- 回写口径
  - `accept / upgrade` 已开始按最小 contract 迁入 `runtime-link`
  - `CP04-2` 仍不通过
  - `CP04-3 / CP04-4` 已通过
  - `91 / 95 / 97` 对 `Step 04` 仍不整体通过
...
- 测试
  ...
  - `cargo test -p craw-chat-runtime-link --offline`
  - `cargo test -p session-gateway --offline`
- 结论
  - 本轮已完成 `97` 要求的增量回写
  - 但 `Step 04` 仍未达到整体关口
...
- `runtime-link` 已从“live push degrade-to-pull owner”继续推进到“pull 降压后恢复 buffered push owner”
  - `LinkBufferedPushPlan`
  - `LinkPushCursor`
  - `LinkSession::start_push_cursor(...)`
- `session-gateway` 的 websocket watch push / `events.pull` 热路径已改为消费上述 runtime owner，而不是在服务层私有持有 backlog 恢复判断

### 23.3 本轮仍未兑现能力

- 完整 async outbound queue / buffer 实体仍未回收到 `runtime-link`
- backlog drop / overload close 语义仍未闭环
- Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍未从 `session-gateway` 回收
- 因此 `CP04-2` 仍未完成，`Step 04` 仍不可关闭

### 23.4 是否偏离架构

- 未偏离
- 本轮继续符合 `131 / 136 / 147` 中“backpressure controller 下沉到 runtime crate，gateway 仅消费 runtime 裁决”的方向
- 本轮没有引入新的协议分支，只把既有 realtime 热路径中的 backlog 恢复语义从 gateway 私有判断迁移为 runtime owner

### 23.5 回写决议

- 允许把本轮 as-built 回写到 `09 / 131 / 136 / 147`
- 回写口径
  - live push 已从“degrade-to-pull owner”继续推进到“buffered push recovery owner”
  - `CP04-2` 仍不通过
  - `CP04-3 / CP04-4` 保持通过
  - `91 / 95 / 97` 对 `Step 04` 仍不整体放行
...
- websocket 热路径已开始消费上述 runtime owner，在 extreme overload backlog 下主动关闭异常连接，而不是无限停留在 `PullOnly`
...
- 未偏离
- 本轮继续符合 `131 / 136 / 147` 中“慢消费者极端情况下主动关闭异常连接、gateway 仅消费 runtime 裁决”的方向
- 本轮没有引入第二套关闭语义 authority，只把既有 slow-consumer extreme overload 处理迁移为 runtime owner
...
- `runtime-link` 已从“队列配置 owner”推进到“live batch/backpressure owner”
...
- `session-gateway` 的 catchup / push / pull 已开始消费上述 runtime owner，而不是保留硬编码批量逻辑
...
- 未偏离
- 本轮属于把 `131 / 136 / 147` 中 “Link Plane queue/backpressure owner 下沉到 runtime crate，gateway 仅做 transport adapter” 的要求继续落到真实代码
- 本轮没有引入新的协议分支，也没有引入第二套队列策略 authority
...
- `runtime-link` 已从 websocket upgrade contract owner 继续推进到 handoff execute owner
...
- `session-gateway` 改为只负责：
  ...
  - `RealtimeWebsocketUpgradeContext` 上下文装配
  - 调用 `upgrade.execute(socket, serve_realtime_websocket_upgrade)` 完成 transport -> runtime 移交

### 21.3 本轮仍未兑现能力

- Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍未从 `session-gateway` 回收到 `runtime-link`
- 完整 async outbound queue / buffer / drop / degrade / overload controller 仍未闭环
- 因此 `CP04-2` 仍未完成，`Step 04` 仍不可关闭
...
- 未偏离
- 本轮继续符合 `131 / 136 / 147` 中“Link Plane owner 下沉到 runtime crate，gateway 只保留 transport adapter”的既定方向
- 与此前 `session_resume / queue batch owner` 的回收路径连续，没有引入第二套 websocket 升级 authority

### 21.5 回写决议

- 允许把本轮 as-built 回写到 `09 / 131 / 136`
- `147` 的 `As-Built 9` 已覆盖本轮 crate 映射，不重复追加
- 回写口径
...
- `session-gateway` 的 websocket watch push 路径已改为消费上述 runtime owner，而不是自行决定 backlog 过大时是否继续 push
...
- 未偏离
- 本轮继续符合 `131 / 136 / 147` 中“backpressure controller 下沉到 runtime crate，gateway 仅消费 runtime 裁决”的方向
- 本轮没有引入新的外部协议，只把既有 realtime push 热路径中的 overload 降级判断迁移为 runtime owner
...
- 这使当前 as-built 更接近 `131 / 136 / 147` 中“runtime-link 持有 owner，gateway 仅保留 transport adapter” 的边界表达

### 25.3 本轮仍未兑现能力

- Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍未从 `session-gateway` transport seam 回收到 `runtime-link`
...
- 未偏离
- 判断为“实现更具体”
- 本轮不是新增第二套连接 authority，而是把 gateway 侧仅剩的 Axum transport seam 从 `lib.rs` 散落 helper 收敛为单一 adapter 模块，便于后续证明该边界是否可最终保留
...
- 这使 `gateway 只保留 transport seam` 的 as-built 边界更明确
...
- 未偏离
- 判断为“实现更具体”
- 本轮只是在既有 `session-gateway` transport seam 已集中后的基础上，继续把 websocket route handler 从 `lib.rs` 收到该单一 seam 模块，没有引入第二套入口 authority
...
- 回写口径
  ...
  - `services/session-gateway/src/lib.rs` 只保留路由装配，不再直接持有 websocket upgrade 热路径入口
  - `CP04-2` 仍不通过
  - `CP04-3 / CP04-4` 保持通过
  - `91 / 95 / 97` 对 `Step 04` 仍不整体放行
...
- `runtime-link` 已把 pending backlog 数学公开为 owner 公式：
  - `pending_outbound_events(...)`
- `session-gateway` 的 catchup / pull 路径已消费该 runtime 公式，而不再保留本地 helper
- 这让 `queue/backpressure` 的输入 authority 比此前更一致
...
- 未偏离
- 判断为“实现更具体”
- 本轮只是把 backlog 输入公式从 service duplication 收回 runtime crate，没有引入新的协议语义或第二套 queue authority

### 27.5 回写决议

- 允许把本轮 as-built 回写到 `09 / 131 / 136 / 147`
- 回写口径
  - pending backlog 数学已从 service duplication 推进到 runtime owner
  - `CP04-2` 仍不通过
  - `CP04-3 / CP04-4` 保持通过
  - `91 / 95 / 97` 对 `Step 04` 仍不整体放行
...
- `docs/鏋舵瀯/09-瀹炴柦璁″垝.md`
- `docs/鏋舵瀯/131-杩炴帴绠＄悊涓庡垎灞傚脊鎬ф墿瀹规灦鏋勮璁?2026-04-06.md`
- `docs/鏋舵瀯/136-鍏抽敭涓氬姟閾捐矾涓庤法Plane鏃跺簭璁捐-2026-04-06.md`
- `docs/鏋舵瀯/147-CCP鍒癈rate涓庢帴鍙ｆā鍧楄惤鍦版槧灏勮璁?2026-04-06.md`
