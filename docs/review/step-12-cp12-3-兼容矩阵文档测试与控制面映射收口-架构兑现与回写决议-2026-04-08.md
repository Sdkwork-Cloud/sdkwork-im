...
## 本轮已兑现能力
- `09`
  - `Wave D / Step 12` 已具备 compatibility matrix 的三类证据：
    - 文档
    - 测试
    - control-plane 映射
- `146`
  ...
  - app-facing 与 admin-facing SDK 现在都能指向自己的 `matrixClientTypes`
- `148`
  - control-plane governance 响应新增 `sdkCompatibilityBaseline`
  - governance 现在同时公开：
    - raw matrix 对应的 client baseline
    - `effectiveSnapshot.allowedBindings`
    - `effectiveSnapshot.allowedCodecs`
    - `effectiveSnapshot.enabledCapabilities`
  - 这让 registry row 与 runtime 生效结果之间的映射首次对 SDK facade 可见
- `149`
  - 当前已把升级兼容中的关键边界进一步冻结为：
    - raw compatibility matrix 可以声明 `ccp/mqtt/1`、`cbor`
    - governance `effectiveSnapshot` 可以在 rollback / kill switch 下收回 `ccp/mqtt/1`、`cbor`、`payload.cbor`
  - 这为多 cell / 多 region 升级中的“声明能力”和“当前允许能力”提供了真实 consumer-side 区分
...
- `148`
  ...
  - 更高 tier 的治理演练
- `149`
  - 多 cell / 多 region 动态 rollout orchestration
  - 发布后门禁观测
  - 跨 region 灾备切换自动化

## 是否偏离架构
- 本轮修正的是一个证据链断裂，而不是单点 bug：
  - 偏离现象：
    - compatibility matrix 的测试与 control-plane 已存在，但 SDK / CLI 文档没有把它们连成统一 baseline
    - governance 响应里也没有可被 SDK facade 直接消费的 `sdkCompatibilityBaseline`
  - 修正结果：
    - 文档、registry test、control-plane governance response 已形成同一条映射链
- 因此当前实现回到了 `146 / 148 / 149` 共同定义的“兼容矩阵必须既可治理、又可消费、还能安全降级”的主路径。
