...
- 上一轮已完成 `session-gateway` 与 `local-minimal-node` 的 route preflight owner 收口，但 [`services/session-gateway/src/session.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\session-gateway\src\session.rs) 仍直接编排 disconnect 生命周期 glue：
...
- [`services/session-gateway/src/device_registration.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\session-gateway\src\device_registration.rs)
...
- [`services/session-gateway/src/lib.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\session-gateway\src\lib.rs)
  - 新增 `AppState::disconnect_active_device_route(...)` delegate
  - 清理 owner 下沉后不再需要的 `node_id`、`realtime_cluster` 主状态字段
- [`services/session-gateway/src/session.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\session-gateway\src\session.rs)
  - `disconnect_session(...)` 不再直接操作 cluster/runtime raw disconnect glue
  - 只根据 owner outcome 决定返回 `presence_snapshot(...)` 还是 `disconnect(...)`
- [`services/session-gateway/tests/lib_structure_test.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\session-gateway\tests\lib_structure_test.rs)
...
- `CP05-4` 仍未整体闭环，因为 [`services/local-minimal-node/src/node/session.rs`](D:\javasource\spring-ai-plus\spring-ai-plus-business\apps\craw-chat\services\local-minimal-node\src\node\session.rs) 还保留对等的 raw disconnect lifecycle glue。
- `Step 05` 仍未闭环。
