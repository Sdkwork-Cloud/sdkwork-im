...
- 已用 TDD 方式先写出 `runtime_plane_split_test`
...
- `CP04-1`：通过
  - `runtime-link` 与 `runtime-route` 骨架已存在且有真实测试
- `CP04-2`：未通过
  - 连接热路径尚未从 `session-gateway` 真实抽离
- `CP04-3`：部分通过
  - route epoch / drain / migration 模型已有最小 runtime owner
  - 但现有 cluster bridge 还未正式切到该 owner
- `CP04-4`：未通过
  - `local-minimal-node` 还未明显朝纯装配层收敛
...
- 本轮 slice 的 red/green 已完成
- 当前验证只能证明 `Step 04` 已开始形成真实 runtime owner，不能证明 `Step 04` 已闭环

## 5. 本轮发现问题

### 5.1 运行时 owner 仍在旧服务内

- `session-gateway/src/cluster.rs` 仍然是 route lifecycle 的主要实现 owner
- `session-gateway/src/websocket.rs` 仍然是连接状态与握手运行时的主要实现 owner

### 5.2 新 crate 还处于 skeleton 阶段

- `runtime-link` 当前只有状态模型和队列/恢复窗口配置
- `runtime-route` 当前只有 route directory 与 node lifecycle 的最小实现
- 距离 Step 04 的真实“热路径抽离”还差一整段集成工作

## 6. 风险评估

- 当前残余风险：
  - 如果下一轮直接把大量逻辑搬迁到新 crate，而不沿现有 cluster / websocket 测试回归推进，容易破坏实时路径
  - 如果 `runtime-route` 与现有 cluster bridge 分别定义 route 语义，会造成双 owner 冲突
- 是否允许进入下一 step：`否`
- 当前阻塞项：
  - `CP04-2`
  - `CP04-4`
...
对应 `docs/step/95-架构能力闭环验收标准.md` 中 Step 04 条目：

- Link Plane：未闭环
- Route Plane：未闭环
- `resume / reconnect / drain / fencing`：只有部分 owner 开始收口
...
- 本轮已做部分 as-built 回写
- 但 Step 04 尚未整体闭环，因此 `97` 当前只能判为部分完成
...
- 继续留在 `Step 04`
- 优先让 `session-gateway` 的现有 route lifecycle 开始依赖 `runtime-route`
- 再让连接状态与 resume window 的 owner 开始从 `websocket.rs` 向 `runtime-link` 移动

## 10. 质量评分表

| 维度 | 分数 | 说明 |
| --- | --- | --- |
| 架构对齐 | 9 | slice 与 `131 / 136` 对齐，边界清楚 |
| 边界清晰 | 10 | 明确只做 runtime skeleton，不混 Step 05/07 |
| 路径真实 | 10 | 代码、测试和文档都是真实仓库路径 |
| 实施可执行 | 9 | red/green 路径清晰，但仍是起步 slice |
| 测试完整 | 9 | 已有 fail-first 与回归补充 |
| 验证可重复 | 10 | 命令可重跑 |
| 检查点可判定 | 8 | 能清楚指出哪些 CP 还未过 |
| 风险与回滚 | 8 | 通过最小 slice 控制风险，但集成工作未开始 |
| 依赖明确 | 10 | 下一轮入口清楚 |
...
- 原因不是只改了文档或类型别名，而是：
  - 先写出 fail-first 测试 `test_cluster_bridge_public_route_models_use_runtime_route_owner_types`
  - `runtime-route` 已补齐 `session_id / connection_kind / bound_at / route_epoch`
  - `session-gateway/src/cluster.rs` 已改为通过 `RouteDirectory` 持有 route owner 与 node lifecycle
  - `session-gateway` 的公开 route 模型已收口为 `runtime-route` 类型

### 11.2 对旧问题判断的修正

- 旧判断“`session-gateway/src/cluster.rs` 仍然是 route lifecycle 的主要实现 owner”不再准确
- 当前更准确的判断应为：
  - `cluster.rs` 仍然是 route handoff、runtime 状态转移、错误映射的集成层
  - 但 route ownership / epoch / drain / migration 的公开模型与目录 owner 已收口到 `craw-chat-runtime-route`
- 旧风险“`runtime-route` 与现有 cluster bridge 双 owner 冲突”已从现实风险降为已消除风险
  - 当前 `cluster bridge` 已消费同一套 owner 模型
...
- 本轮代码、测试、review、架构回写均已追加
- `Step 04` 已从“runtime skeleton 被测试消费”推进到“route owner 已接入真实 cluster bridge”
- 但由于 Link Plane 与本地节点装配层尚未闭环，`Step 04` 依旧不能判定完成

## 12. 2026-04-07 Link Plane 增量审计更新

## 13. 2026-04-07 local-minimal-node 装配收口与迁移护栏审计更新
...
- `services/session-gateway/src/cluster.rs`
  - 新增 `RealtimeClusterBridge::ensure_device_route_local(...)`
- `services/local-minimal-node/src/node/access.rs`
  - 非 takeover 的设备绑定路径已在状态变更前执行 owner locality 校验
  - 当旧节点已不是权威 route owner 时，请求会被直接拒绝，不再把 runtime ownership 抢回本地

### 13.3 fail-first 与回归验证证据
...
- `CP04-4`：更新为通过
  - `local-minimal-node` 已经通过统一 `RealtimePlaneAssembly` 显式装配 Link / Route runtime
  - drain 后的旧节点不会在失败前先改写本地 route/runtime ownership
- `CP04-3`：保持通过
- `CP04-2`：仍未通过
  - accept / upgrade / queue / backpressure / resume / reconnect 热路径仍未完整迁入 `runtime-link`
- `91`：仍未整体通过
  - 剩余阻塞已收敛到 `CP04-2`
- `95`：仍未整体通过
  - Step 04 仍不能宣告整体验收通过
- `97`：继续为部分完成
  - 允许补写 as-built，但不允许宣告 Step 04 已全部兑现
- `93`：不触发
  - `Wave B` 尚未完成

### 13.5 风险与后续动作

- 当前未发现与 `131 / 136` 冲突的实现偏差
- 剩余主风险已收敛到 `session-gateway/src/websocket.rs` 的 Link Plane hot path owner
...
- `queue / backpressure` 默认阈值已不再由 `session-gateway/src/websocket.rs` 私有常量定义
- `crates/craw-chat-runtime-link/src/lib.rs` 已新增：
  - 默认实时出站队列阈值常量
  - `OutboundQueuePolicy::realtime_default()`
- `services/session-gateway/src/websocket.rs` 的 `build_link_session(...)` 已改为直接消费 `runtime-link` owner 默认策略
...
- 该增量说明 `CP04-2` 已继续前推，但仍未通过
- 本轮只能确认：
  - Link Plane 默认队列 owner 已收口到 `runtime-link`
  - `session-gateway` 不再持有这组默认阈值的权威定义
- 本轮仍不能确认：
  - 真实 queue/backpressure 执行策略已完全迁入 `runtime-link`
  - accept / upgrade / resume / reconnect 已整体迁移
- 因此：
  - `91` 仍未整体通过
  - `95` 仍未整体通过
  - `97` 仍为部分完成
...
- `95` 仍不通过：
  - 原因不是 Link Plane 零进展，而是尚未完成完整热路径抽离
- `97` 仍为部分完成：
...
- 本轮只审计一个新的 hot path slice：
  - `auth_bind` 身份匹配 owner 是否继续从 `session-gateway` 收口到 `runtime-link`
...
- `services/session-gateway/src/websocket.rs`
  - `build_link_session(...)` 改为把 actor kind 一并收口到 `runtime-link`
  - `complete_ccp_handshake(...)` 不再保留服务侧私有 `auth_bind_matches_context(...)`
  - `AuthOkFrame` 改为从 `LinkSession` 回填身份字段
- `services/session-gateway/tests/runtime_plane_split_test.rs`
  - 跟进 `LinkSession::new(...)` 的 owner 新签名

### 15.3 fail-first 与回归验证证据
...
- `CP04-2`：仍未通过，但继续前推
  ...
  - `runtime-link` 已不再只是状态容器，也开始持有连接身份语义
- 本轮仍不能宣告：
  - accept / upgrade 已迁入 `runtime-link`
  - 真实 queue/backpressure 执行已迁入 `runtime-link`
  - resume / reconnect 已迁入 `runtime-link`
- 因此维持总判断：
  - `CP04-3` 通过
  - `CP04-4` 通过
  - `CP04-2` 未通过
...
- 当前仍位于 `Wave B / Step 04`
- 上一轮已把默认队列 owner 与 `auth_bind` 身份 owner 推入 `runtime-link`
- 本轮继续只围绕 `CP04-2`，检查 `hello` 协商 owner 是否继续从 `session-gateway` 收口
...
- `services/session-gateway/src/websocket.rs`
  ...
  - 删除服务侧 `supported_capabilities / negotiated_capabilities`

### 16.3 fail-first 与回归验证证据
...
- `CP04-2`：仍未通过，但继续前推
  - `hello` 协商 owner 已从 websocket 服务侧开始迁入 `runtime-link`
  - `runtime-link` 已不再只承载状态/队列/身份匹配，也开始承载连接协商语义
- 本轮仍不能宣告：
...
- 根据 `docs/架构/131` 与 `docs/架构/136`，`resume / reconnect` 语义应继续从服务层回收到 `runtime-link`
...
- `services/session-gateway/src/presence.rs`
  - `DevicePresenceRuntime::resume(...)` 改为直接消费 `runtime-link` 的 resume 计算结果
  - 服务层不再本地保留该段纯判断逻辑
...
- `CP04-2`：仍未通过，但继续前推
  ...
  - 这说明 Link Plane hot path 已继续从“握手 owner 回收”推进到“恢复窗口判断 owner 回收”
- 本轮仍不能宣告：
  - `session_resume / session_resumed` 控制帧时序已完整迁入 `runtime-link`
  - HTTP/WebSocket accept / upgrade 已迁入 `runtime-link`
  - 真实 `queue/backpressure` 执行已迁入 `runtime-link`
- 因此维持总判断：
  - `CP04-3` 通过
  - `CP04-4` 通过
...
- 当前仍位于 `Wave B / Step 04`
- `resume` 纯语义 owner 已开始迁入 `runtime-link` 后，`goaway` 仍是 Step 04 文档明确列出的 Link Plane 运行时语义
...
- `CP04-2`：仍未通过，但继续前推
  ...
  - 这说明 Step 04 的 Link Plane hot path 已继续从“恢复窗口 owner 回收”推进到“断连信号 owner 回收”
- 本轮仍不能宣告：
  - `session_resume / session_resumed` 控制帧时序已完整迁入 `runtime-link`
  - HTTP/WebSocket accept / upgrade 已迁入 `runtime-link`
  - 真实 `queue/backpressure` 执行已迁入 `runtime-link`
...
## 19. 2026-04-07 runtime-link session_resume 控制帧 owner 审计更新
...
- `services/session-gateway/tests/websocket_smoke_test.rs`
  - CCP smoke 已更新为校验：
    - `session_resumed` 控制帧先于 `connected` 业务帧

### 19.2 fail-first 与回归验证证据
...
- `CP04-2`：仍未通过，但继续前推
  - 这次新增证明：
    ...
    - `runtime-link` 已开始作为该时序的 authority owner
  - 这次仍未证明：
    - accept / upgrade 已迁入 `runtime-link`
    - 真正的 `queue/backpressure` 执行 owner 已迁入 `runtime-link`
- `CP04-3`：保持通过
- `CP04-4`：保持通过
- `91`：仍未整体通过
- `95`：仍未整体通过
...
- 通过项
  - `runtime-link` 已开始直接拥有 WebSocket upgrade contract
    - `LINK_WEBSOCKET_SUBPROTOCOL`
    - `LinkWebsocketMode`
    - `supported_websocket_subprotocols()`
    - `select_websocket_mode(...)`
  - `session-gateway` 的 `realtime_websocket(...)` 已不再硬编码 `CCP_WS_SUBPROTOCOL` 与 `selected_protocol().is_some()`
  - WebSocket CCP smoke 仍通过，说明 owner contract 下沉没有破坏现有对外协议行为
- 未通过项
  - Axum `on_upgrade(...)` 执行动作仍在 `session-gateway`
  - live `queue/backpressure` 执行 owner 仍在 `session-gateway`
  - 因此 `CP04-2` 仍未整体通过

### 20.3 fail-first 与 fresh 验证
...
- Red
  ...
  - 失败原因是缺少 `realtime_websocket_subprotocols / select_realtime_websocket_mode`，与本轮新增 owner API 精确对应
- Green
...
- 当前只回收了 `accept / upgrade` 的最小 contract，没有回收 transport adapter 的 `on_upgrade(...)`
- 当前也没有触及真实队列缓冲、丢弃、降级、背压控制，因此 `queue/backpressure` 风险仍在
- `CP04-2` 已显著收敛，但距离 `91 / 95` 通过仍差最后一段热路径 owner
...
- `CP04-3`：通过
- `CP04-4`：通过
- `91`：本轮 review 证据已补齐，但 `Step 04` 整体不通过
...
- 通过项
  - `runtime-link` 已开始持有 live outbound batch/backpressure 的最小 owner contract
    - `LinkOutboundBatchPlan`
    - `plan_stream_batch(...)`
    - `plan_pull_batch(...)`
  - `session-gateway` websocket 热路径已不再硬编码 catchup / push 的 `100`
  - `events.pull` 已开始通过 `runtime-link` hard limit 执行限流裁决
  - e2e smoke 已证明真实 websocket 执行路径开始消费上述 owner
- 未通过项
  - Axum `on_upgrade(...)` 执行 owner 仍在 `session-gateway`
  - 目前回收的是 live batch/limit 裁决，不是完整 async queue / buffer / drop / degrade controller
  - 因此 `CP04-2` 仍未整体通过

### 21.3 fail-first 与 fresh 验证
...
- Red
  - `$env:CARGO_TARGET_DIR='C:\\Users\\admin\\.codex\\memories\\target-step04-queue-owner-red-runtime'; cargo test -p craw-chat-runtime-link test_runtime_link_plans_live_outbound_queue_batches_from_owner_limits --offline`
    - 失败原因：缺少 `plan_stream_batch / plan_pull_batch`
...
- `CP04-3`：通过
- `CP04-4`：通过
- `91`：本轮 review 证据完整，但 `Step 04` 整体不通过
...
- 通过项
  ...
  - 结构测试已证明 websocket upgrade seam 已从 `lib.rs` 移出，并建立了防回退约束
- 未通过项
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍保留在 `session-gateway`
  - backlog drop 语义仍未形成 `runtime-link` owner
  - 完整 async queue / buffer 实体仍未形成 `runtime-link` owner
  - 因此 `CP04-2` 仍未整体通过

### 26.3 fail-first 与 fresh 验证
...
- 通过项
  - `runtime-link` 已开始拥有 pending backlog 数学 owner：
    - `pending_outbound_events(...)`
  - `LinkPushCursor` 内部与 `session-gateway` catchup / pull 路径现在都消费同一份 runtime 公式
  - 结构测试已证明 `services/session-gateway/src/websocket.rs` 不再保留本地 backlog helper
- 未通过项
  - 这只是输入数学 owner 回收，不是 backlog drop / queue 实体闭环
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍保留在 `session-gateway`
  - backlog drop 语义仍未形成 `runtime-link` owner
  - 完整 async queue / buffer 实体仍未形成 `runtime-link` owner
  - 因此 `CP04-2` 仍未整体通过

### 28.3 fail-first 与 fresh 验证
...
- pending backlog 公式虽已统一，但这还没有引入真正的 async queue / buffer 实体
- 当前仍缺：
  - backlog drop 权威语义
  - 对“单一 Axum transport seam 是否最终可接受”的最终闭环判断
...
- 通过项
  - `/im/v3/api/realtime/ws` 的 route handler 已从 `lib.rs` 移到 `websocket_upgrade.rs`
  - `services/session-gateway/src/lib.rs` 已不再直接导入 `WebSocketUpgrade`
  - 结构测试已证明 `lib.rs` 不再直接保留 websocket route handler 和 upgrade 调用入口
  - `session-gateway` 全包测试通过，说明 HTTP / websocket 现有路径未因 handler 收敛而回归
- 未通过项
...
- Red
  - `$env:CARGO_TARGET_DIR='C:\\Users\\admin\\.codex\\memories\\target-step04-route-seam-red'; cargo test -p session-gateway test_session_gateway_websocket_route_handler_moves_out_of_lib_impl --offline`
    - 失败原因：`lib.rs` 仍保留 `WebSocketUpgrade` 导入和 websocket route handler
- Green
...
- websocket route handler 与 upgrade helper 虽然都已集中到 `websocket_upgrade.rs`，但这仍只是“服务侧单一 seam”收敛，不等于 owner 完整回收
- 当前仍缺：
  - backlog drop 权威语义
  - 完整 async queue / buffer 实体
  - 对“Axum adapter 是否就是最终可接受服务边界”的明确闭环判断
- 因此本轮只能认定 `session-gateway` 的 websocket 入口边界更清晰，不能认定 `CP04-2` 已闭环
...
- 通过项
  - `runtime-link` 已开始拥有 websocket upgrade handoff execute owner
    - `prepare_websocket_upgrade(...)`
    - `LinkWebsocketUpgradeHandoff<TContext>`
    - `LinkWebsocketUpgradeHandoff::execute(...)`
  - `session-gateway` 的 `realtime_websocket(...)` 已不再私有持有 socket -> runtime serve 的 handoff 执行逻辑
  - handoff execute 仍保留 `LinkWebsocketMode` 与业务上下文，说明 owner contract 已从“协议协商”推进到“升级执行移交”
- 未通过项
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 调用入口仍在 `session-gateway`
  - 完整 async queue / buffer / drop / degrade / overload controller 仍未回收到 `runtime-link`
  - 因此 `CP04-2` 仍未整体通过

### 22.3 fail-first 与 fresh 验证
...
- Red
  - `$env:CARGO_TARGET_DIR='C:\\Users\\admin\\.codex\\memories\\target-step04-upgrade-handoff-red-runtime'; cargo test -p craw-chat-runtime-link test_runtime_link_prepares_websocket_upgrade_handoff_owner_contract --offline`
    - 失败原因为 `prepare_websocket_upgrade` 尚不存在
  - `$env:CARGO_TARGET_DIR='C:\\Users\\admin\\.codex\\memories\\target-step04-upgrade-handoff-red-session'; cargo test -p session-gateway test_realtime_websocket_upgrade_prepares_runtime_link_handoff_owner --offline`
    - 失败原因为 `prepare_realtime_websocket_upgrade` 尚不存在
- Green
...
- 当前回收的是 websocket upgrade handoff execute owner，不是 Axum transport adapter 本体
- 当前也没有触及完整 async queue / buffer / drop / degrade / overload controller，因此高压场景下的真实背压与降级语义仍未闭环
- `CP04-2` 已继续收敛，但距离 `91 / 95` 放行仍差最后一段热路径 owner
...
- 通过项
  - `runtime-link` 已开始拥有 live push overload 时的降级裁决 owner
    - `LinkPushMode`
    - `LinkPushPlan`
    - `plan_push_batch(...)`
  - `session-gateway` 的 realtime watch push 路径已不再私有持有“积压过大时是否仍继续自动 push”的判断
  - smoke test 已证明 backlog 超过 hard limit 时，连接进入 pull-only 降级，而不是继续自动 push
- 未通过项
  - 当前只回收了 degrade-to-pull owner，没有回收完整 async queue / buffer 实体
  - backlog drop / overload close 语义仍未形成 runtime owner
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍在 `session-gateway`
  - 因此 `CP04-2` 仍未整体通过

### 23.3 fail-first 与 fresh 验证
...
- Red
  ...
  - `$env:CARGO_TARGET_DIR='C:\\Users\\admin\\.codex\\memories\\target-step04-push-degrade-red-session'; cargo test -p session-gateway test_realtime_websocket_degrades_live_push_to_pull_only_when_runtime_link_detects_overload --offline`
    - 失败原因为 gateway 在 overload backlog 下仍继续自动 push
- Green
...
- 当前仍没有真实 async queue / buffer 实体，因此高压场景下的完整排队、丢弃、过载断链语义还未闭环
- `CP04-2` 已从“live batch owner”推进到“live push degrade owner”，但距离 `91 / 95` 通过仍差最后一段 queue controller owner
...
- 通过项
  - `runtime-link` 已开始拥有“pull 降压后恢复 buffered push”的 owner contract
    - `LinkBufferedPushPlan`
    - `LinkPushCursor`
    - `LinkSession::start_push_cursor(...)`
  - `session-gateway` 的 realtime watch push 路径已不再要求“等待下一条新发布事件”才能从 `PullOnly` 恢复到继续自动 push
  - smoke test 已证明 backlog 超过 hard limit 后先退化为 `pull-only`，随后客户端 pull 把 backlog 降到阈值以下时，已有 buffered 事件会立即恢复自动 push
- 未通过项
  - 当前只回收了 buffered push recovery owner，没有回收完整 async queue / buffer 实体
  - backlog drop / overload close 语义仍未形成 runtime owner
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍在 `session-gateway`
  - 因此 `CP04-2` 仍未整体通过

### 24.3 fail-first 与 fresh 验证
...
- buffered push recovery 已进入 runtime owner，但这仍不是完整 async queue / buffer 实体
- 当前仍没有回收：
  - backlog drop 语义
  - overload close 语义
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` transport 入口
...
- 通过项
  - `runtime-link` 已开始拥有 extreme overload close owner contract
    - `LinkPushMode::Disconnect`
    - `LinkPushPlan::disconnect`
    - `realtime_overload_goaway()`
  - websocket 热路径在 runtime owner 裁决为 `Disconnect` 时，已不再停留在 `PullOnly`，而是主动关闭异常连接
  - smoke test 已证明 backlog 达到 extreme overload 阈值后，连接会收到 owner 指定的 close code / reason
- 未通过项
  - 当前只回收了 overload close owner，没有回收 backlog drop 语义
  - 完整 async queue / buffer 实体仍未形成 runtime owner
  - Axum `WebSocketUpgrade` adapter 与 `.on_upgrade(...)` 入口调用仍在 `session-gateway`
  - 因此 `CP04-2` 仍未整体通过

### 25.3 fail-first 与 fresh 验证
...
- overload close 已进入 runtime owner，但 backlog drop 仍无明确权威语义
- 当前也还没有真实 async queue / buffer 实体，因此“排队、降级、断开”三者只是 owner 裁决下沉，不是完整队列实现闭环
- `CP04-2` 已从“buffered push recovery owner”继续推进到“extreme overload close owner”，但距离 `91 / 95` 通过仍差最后一段 queue/controller 边界
...
- pass items
  ...
  - targeted queue-state tests and full package tests for `craw-chat-runtime-link` and `session-gateway` are green
- not-yet-pass items
...
- pass items
  ...
  - fresh full-package verification for `session-gateway`, `craw-chat-runtime-link`, `craw-chat-runtime-route`, and `local-minimal-node` is green
- pass judgement
