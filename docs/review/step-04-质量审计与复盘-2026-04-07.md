...
- 当前验证只能证明 `Step 04` 已开始形成真实runtime owner，不能证`Step 04` 已闭环
...
- 继续留在 `Step 04`
- 优先`session-gateway` 的现route lifecycle 开始依赖`runtime-route`
...
- `CP04-2` 仍未通过，但已从“完全未接线”更新为“真实热路径已开始消runtime-link owner
- `95` 仍不通过
...
- 本轮只审计一个新hot path slice
  - `auth_bind` 身份匹配 owner 是否继续由 `session-gateway` 收口径`runtime-link`
...
- `CP04-2`：仍未通过，但继续前推
  - `resume` 纯语owner 已开始从 `session-gateway` 迁入 `runtime-link`
  - 这说明 Link Plane hot path 已继续从“握owner 回收”推进到“恢复窗口判owner 回收口
- 本轮仍不能宣告：
...
- 当前仍位`Wave B / Step 04`
- `resume` 纯语owner 已开始迁`runtime-link` 后，`goaway` 仍是 Step 04 文档明确列出Link Plane 运行时语
- 当前 websocket CCP 断连路径只发送close，不发送`goaway` 控制帧，与架构定义不完全一
...
- `CP04-2`：仍未通过，但继续前推
  ...
  - 这说Step 04 Link Plane hot path 已继续从“恢复窗owner 回收”推进到“断连信owner 回收口
- 本轮仍不能宣告：
  - `session_resume / session_resumed` 控制帧时序已完整迁入 `runtime-link`
  - HTTP/WebSocket accept / upgrade 已迁`runtime-link`
  - 真实 `queue/backpressure` 执行已迁`runtime-link`
...
- `services/session-gateway/src/websocket.rs`
  ...
  - `catchup_after_seq` 已开始消runtime owner 决策结果
- `services/session-gateway/tests/websocket_smoke_test.rs`
  - CCP smoke 已更新为校验证
