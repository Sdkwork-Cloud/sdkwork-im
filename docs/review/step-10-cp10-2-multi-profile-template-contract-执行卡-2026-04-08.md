...
- 新增 `docs/部署/多环境Profile与配置模md`
  - 明确区分
    ...
    - 已命名、已compose/template 入口但仍复用 `self-hosted.split-services.development` 服务合同`self-hosted.split-services.development`
...
- `cargo test -p sdkwork-im-gateway --offline test_quick_start_doc_freezes_full_local_command_surface -- --exact`
  - 本轮补充文档 contract 后先失败，证`快速启动脚md` 还未公开 `self-hosted.split-services.development` `profile selector`
