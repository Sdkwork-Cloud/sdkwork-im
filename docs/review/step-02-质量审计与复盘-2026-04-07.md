...
| 边界清晰 | `10` | Step 02 只收口 workspace / facade / 大文件治理，不冒进做 CCP 设计 |
| 路径真实 | `10` | 所有关键路径都落在真实文件与真实 crate 上 |
| 实施可执行 | `10` | 本轮直接完成 `chat-cli` 尾项，不停留在计划 |
| 测试完整 | `9` | 关键 crate 均有 fresh `cargo test -p`，但仍依赖分包验证而非 `cargo test --workspace` |
| 验证可重复 | `10` | 行数扫描、结构测试、包级测试、fmt 检查均可重复 |
| 检查点可判定 | `10` | `> 1000` 文件清零、结构测试转绿、架构回写完成，标准客观 |
| 风险与回滚 | `9` | 风险明确，但 `projection-service` 接近红线仍需后续严控 |
| 依赖明确 | `10` | Step 03 的进入条件已清楚 |
| 复盘沉淀 | `10` | `docs/review` 与 `docs/架构` 已同步回写 |
...
- 没有把 `chat-cli` 留成“只差一点”的尾项，而是彻底把 parse/config/runtime 分开。
- 没有把 Step 02 的闭环标准降格为“主要大文件差不多都拆了”，而是继续追到仓库级红线清零。
