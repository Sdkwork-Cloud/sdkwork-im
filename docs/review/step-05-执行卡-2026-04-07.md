...
- 当前定位
  - `Wave B / Step 05 / CP05-4`
- 本轮为什么继续做
  - `local-minimal-node` 虽然已经把 device registration owner 收到 `node/device_registration.rs`，但 [`services/local-minimal-node/src/node/session.rs`](<workspace-root>\craw-chat\services\local-minimal-node\src\node\session.rs) 仍在多个 session / realtime / websocket 入口保留本地 `bind_device(...)` preflight 胶水。
