...
- 前置状态：
  ...
  - `CP10-3` 第一增量已经把 `SmokeBaseUrl` / `--smoke-base-url` 提升到统一 `deploy-local` 入口
  - 但 Docker/public smoke 仍存在真实缺口：
    - `build_public_app()` 强制要求 HS256 bearer
    - `deployments/docker-compose/local-minimal.yml` 未注入 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`
    - `tools/smoke/local_stack_smoke.*` 仍内嵌 `alg=none` demo bearer
...
- 如果 public 路径已经切到 HS256 验签，而 Docker smoke 还沿用 unsigned bearer，则当前 smoke 不是可重复执行，而是伪成功合同。
- 因此本轮最优动作是补齐 public smoke 的签名认证闭环，而不是继续在入口层打补丁。

## 本轮实际完成

### 1. Docker/public smoke 已具备真实认证真源
- `deployments/docker-compose/local-minimal.yml`
  - 新增 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET: local-minimal-public-dev-secret`
  - Docker `local-minimal` profile 现在自带可重复执行的 public bearer dev secret

### 2. smoke 脚本已从 unsigned demo bearer 切换到 HS256 合同
...
### 3. 部署文档已追平 public auth 合同
- `docs/部署/本地最小安装与运行.md`
  - 删除 unsigned demo bearer 与“当前不会做签名校验”的错误表述
  - 新增 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`、Docker dev secret 与 smoke secret 解析顺序说明
  - 新增手工生成 `DEMO_BEARER` / `OPS_AUDIT_BEARER` 的 HS256 示例
- `docs/部署/快速启动脚本.md`
  - 新增 `deploy-local` smoke bearer/secret 口径说明
- `docs/部署/README.md`
  - 标注本地最小安装文档已包含 HS256 public bearer 合同
- `README.md`
  - 根文档 Docker 入口新增 signed bearer / dev secret 说明
...
- `cargo test -p local-minimal-node --offline --test deployment_profile_test`
  - 初始失败 3 项：
    - compose 未注入 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`
    - smoke 脚本仍嵌入 `alg=none`
    - 本地最小安装文档仍宣称不做签名校验
...
- 已兑现：
  ...
  - smoke 脚本具备真实 signed bearer 生成与 override 能力
  - PowerShell / Bash smoke 已在测试中真实命中 `build_public_app()`
  - 本地最小部署文档与快启文档已追平当前认证合同
- 仍未兑现：
  - `CP10-4`
  - `Step 10` 整步闭环
  - `Wave D` 总验收
...
3. `Step 10` 通过后，再按 `93` 规则准备 `Wave D` 总验收
