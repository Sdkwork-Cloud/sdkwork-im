...
## 正向结果
- 没有引入新的 benchmark 框架或外部依赖，仍然复用当前仓库的 `self-hosted.split-services.development` runtime。
- 指标是由测试 fresh 执行得到，而不是手工填数。
- `connection / message / stream` 三类结果已共享同一 profile 与 tier 口径。

## 仍需关注的风险
- 当前结果仅代表 `CI Smoke Tier / self-hosted.split-services.development`，不能外推为预发布容量或架构上限。
