...
- `CP10-2` 把 `self-hosted.split-services.development` / `self-hosted.split-services.development` 从名称约定推进成 compose + template + deploy 入口合同。
...
## 剩余风险
- `self-hosted.split-services.development` 当前仍不是独立运行拓扑；这是架构冻结结果，不是当前缺陷。
