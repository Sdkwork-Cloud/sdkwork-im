...
- `local-minimal-node` 已兑现 assembled stream 能力：
  - conversation-bound open/write/read access
  - realtime frame/completion/abort fanout
  - restart rebuild
- `rtc-signaling-service` 已兑现 standalone RTC 能力：
...
- `local-minimal-node` 已兑现 assembled RTC 能力：
...
- `local-minimal-node` 已保持 media provider mirror surface：
  - `provider-health`
  - `download-url`
...
- 说明：
  ...
  - 当前实现把 ownership 进一步明确为：
    ...
    - assembled conversation-bound binding 由 `local-minimal-node` 统一承担
