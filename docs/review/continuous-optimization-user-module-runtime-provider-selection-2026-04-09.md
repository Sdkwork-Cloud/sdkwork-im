...
- 发现真实偏差：文档长期宣称 `user-module-local / user-module-external` 双形态已闭环，但默认运行时入口始终硬编码本地实现，`external` 只能在测试里手工注入。
- 本轮已修复：`local-minimal-node` 默认装配已支持按配置切换到真实 `user-module-external`。
...
- 现象：
  - `build_default_app*` 全链路都经由 `build_default_user_module_provider()`
  - 该函数原先始终返回 `LocalUserModuleProvider`
  - 结果是默认运行时即使配置 external 目录，也不会真正走 `user-module-external`
- 风险：
  - 架构文档与实现不一致
  - 外部用户目录集成无法落地到真实启动入口
...
- 增加默认提供商选择配置：
  - `CRAW_CHAT_USER_MODULE_PROVIDER=local|external`
  - 默认 `local`
- 增加 external 目录配置：
  - `CRAW_CHAT_USER_MODULE_EXTERNAL_CATALOG_PATH=<json>`
  - `CRAW_CHAT_USER_MODULE_EXTERNAL_SYSTEM=<name>`，默认 `external-directory`
- 新增真实 `ExternalUserModuleProvider`
  - 从外部目录 JSON 读取用户
  - 统一输出 `displayName / externalSystem / externalPrincipalId / userModulePluginId`
...
- 通过：
  - `cargo test -p local-minimal-node --test user_module_provider_runtime_selection_test -- --nocapture`
  - `cargo test -p local-minimal-node user_module_provider -- --nocapture`
...
2. 为 `user-module-external` 增加 operator 示例配置文档与健康检查断言。
