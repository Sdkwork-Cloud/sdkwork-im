# 持续优化评审记录：UserModuleProvider 消息 mutation actor 收口
...
- 当前 step / 波次：
  ...
  - `CP05-2` 收口轮
- 本轮为什么做：
  - 上一轮已把 `UserModuleProvider` 接入 bootstrap member
  - 当前剩余最高价值缺口集中在 `edit_message / recall_message`
  - 若 mutation actor 仍沿用裸 `AuthContext` sender snapshot，则 `sender / actor / user-module` 的 authority 边界仍不完整

## 本轮结论

- 已完成：
  - `edit_message` 已先经 `UserModuleProvider` 富化 `editor`
  - `recall_message` 已先经 `UserModuleProvider` 富化 `recalled_by`
...
- 当前判断：
  - `UserModuleProvider` 已覆盖：
    ...
    - edit_message
    - recall_message
  - 以当前仓库真实状态看，Step 05 中 `CP05-2` 的 `sender / auth-context / user-module` 主链路收口已可判定通过
- 未完成：
...
- 入口修复：
  - `services/local-minimal-node/src/node/message.rs`
    - `EditMessageCommand::from_auth_context(...)` 之后，改为用 `UserModuleProvider` 富化 `command.editor`
    - `RecallMessageCommand::from_auth_context(...)` 之后，改为用 `UserModuleProvider` 富化 `command.recalled_by`
  - 保持：
...
- 测试补齐：
  - `services/local-minimal-node/tests/user_module_provider_mainline_test.rs`
...
- 先红后绿：
  - `cargo test -p local-minimal-node --offline --test user_module_provider_mainline_test message_edit_and_recall_actor_metadata -- --nocapture`
