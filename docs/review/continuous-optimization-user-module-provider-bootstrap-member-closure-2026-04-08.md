# 持续优化评审记录：UserModuleProvider bootstrap member 收口
...
- 当前 step / 波次：
  ...
  - `CP05-2` 第二阶段补强
- 本轮为什么做：
  - 上一轮已把 `UserModuleProvider` 接入消息 sender 与显式 `add-member`
...
- 未完成：
  - `edit_message / recall_message` 的 actor metadata 仍沿用 runtime auth snapshot，尚未统一进入 `UserModuleProvider`
...
- 本地节点接入：
  - `services/local-minimal-node/src/node/conversation.rs`
    ...
    - handoff target 若为 `user`，统一经 `UserModuleProvider` 富化后再进入 runtime
- 测试与结构契约：
...
- `UserModuleProvider` 对 conversation/member 主链路的覆盖已经从：
...
1. 优先把 `edit_message / recall_message` 的 actor enrichment 统一接入 `UserModuleProvider`，补完 Step 05 在消息 mutation 侧的 authority metadata。
