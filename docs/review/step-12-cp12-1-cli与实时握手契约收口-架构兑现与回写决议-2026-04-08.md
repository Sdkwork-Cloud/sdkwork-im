# Step 12 / CP12-1 CLI 与实时握手契约收口 架构兑现与回写决议 - 2026-04-08
...
## 本轮已兑现能力
- `09`
  - `Wave D / Step 12` 已完成首个真实 CLI 检查点，当前 step 不再停留在 HTTP 文档与 contract test。
- `146`
  - `tools/chat-cli` 已成为当前 registry / capability negotiation 的真实消费方：
    - `payload.json` 为当前默认协商基线
    - `session.resume` 仅在协商成功时启用
    - `watch / chat-session / wrapper` 均受同一套主链路约束
- `148`
  - control-plane `effective snapshot` 与 websocket 热路径握手现在重新一致：
    - 未协商 `session.resume` 时，runtime 不再继续等待该控制帧
    - 已协商时，CLI 仍能继续执行 `session_resume -> session_resumed`
  - 这让 `hello_ack.capabilities` 恢复成 runtime 的权威输入，而不是“只读但不生效”的旁路信息。
- `149`
  - 当前 CLI consumer path 已具备最小可信的升级兼容 / 安全降级证据：
    - 默认 snapshot 未协商 `session.resume` 时，不再因 capability drift 卡死首帧
    - future `session.resume` 路径仍保留专门回归测试

## 本轮未兑现能力
- `146`
  - SDK facade README、多语言消费链路仍未进入本轮范围
- `148`
  - tenant / client segment 级治理、release bundle 仍未开始
- `149`
  - 多 cell / 多 region rollout orchestration、发布后观测门禁与灾备切换自动化仍未开始

## 是否偏离架构
- 发现过一次真实偏离，并已在本轮修正：
  - 偏离现象：
    - `hello_ack.capabilities` 默认未协商 `session.resume`
    - websocket 热路径却仍强制等待 `session_resume`
  - 修正结果：
    - runtime 现在按协商条件化要求 `session_resume`
    - CLI 也按协商决定是否发送该控制帧
- 因此当前实现已回到 `146 / 148 / 149` 共同定义的“能力由协商与快照决定”主路径。
