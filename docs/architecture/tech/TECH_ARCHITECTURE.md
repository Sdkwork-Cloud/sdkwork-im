...
- **Defense in Depth**: Trusted-proxy IP validation, per-service circuit breakers, bounded rate limiter memory, and Docker/Kubernetes `_FILE` secret injection.
...
2. **Rate Limiting** (per-IP token bucket, default 600 RPM / 50 burst): Uses `std::sync::Mutex` for minimal lock contention. Bounded eviction at `SDKWORK_IM_GATEWAY_RATE_LIMIT_MAX_ENTRIES` (default 5000) prevents unbounded memory growth from rotating client IPs.
