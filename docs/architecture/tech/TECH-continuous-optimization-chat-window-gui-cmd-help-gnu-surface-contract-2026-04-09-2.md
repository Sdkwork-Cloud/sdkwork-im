> Migrated from `docs/step/continuous-optimization-chat-window-gui-cmd-help-gnu-surface-contract-2026-04-09.md` on 2026-06-24.
...
- The remaining real gap was operator discoverability, not GUI runtime behavior.
- `chat-window-gui.cmd --help` still surfaced only PowerShell-style parameter names, which made the visible Windows GUI wrapper look inconsistent with the actual `.cmd` contract.

## Closure Target

1. Add a Windows regression that calls `bin/chat-window-gui.cmd --help`.
...
- Added `test_chat_window_gui_cmd_help_surfaces_gnu_style_named_flags`
- Reproduced the real Windows discoverability gap first
- Extended `chat-window-gui.ps1` help text with an explicit Windows `.cmd` GNU-style usage line
