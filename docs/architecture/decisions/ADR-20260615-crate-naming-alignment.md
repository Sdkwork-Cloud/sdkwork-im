...
Status: superseded by ADR-20260615-craw-chat-to-sdkwork-im-rebrand
...
> ADR-20260615-craw-chat-to-sdkwork-im-rebrand superseded that choice by
...
> URN namespace), and renaming the `CRAW_CHAT_*` / `SDKWORK_CHAT_*` environment
...
`craw-chat` is the SDKWork Chat instant-messaging backend. Its `specs/component.spec.json`
...
1. `craw-chat-*` — the legacy product codename (contract, codec, runtime-link, gateway
...
| --- | --- |
| `craw-chat-contract-core` | `-core` |
| `craw-chat-ccp-core` | `-core` |
| `im-domain-core` | `-core` |
...
| --- | --- | --- |
| IM contracts | `sdkwork-im-<capability>-contract` | `craw-chat-contract-message` → `sdkwork-im-message-contract`; drop forbidden `-core` from `craw-chat-contract-core` → `sdkwork-im-contract` |
...
| CCP transport | `sdkwork-im-ccp-*` | `craw-chat-ccp-core` → `sdkwork-im-ccp`; `craw-chat-ccp-binding-ws` → `sdkwork-im-ccp-binding-ws` |
...
| Service host | `sdkwork-<app>-service-host` | `local-minimal-node` → `sdkwork-chat-service-host` |
| App config | `sdkwork-chat-*` | `craw-chat-gateway-config` → `sdkwork-chat-gateway-config` |
...
2. **Batch B — IM contracts and domain (leaves).** The `craw-chat-contract-*` and
...
- **Register a governance exception and keep `craw-chat-*`/`im-*`.** Rejected: the prefixes
