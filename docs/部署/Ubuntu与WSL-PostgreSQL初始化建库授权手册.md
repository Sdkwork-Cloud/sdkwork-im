# Ubuntu �?WSL PostgreSQL 初始化建库授权手�?

本文用于�?Ubuntu �?WSL Ubuntu 中为 SDKWork Chat 准备本地 PostgreSQL。当前应�?app code �?`chat`，开发数据库使用 `sdkwork_ai_dev`，生产示例使�?`sdkwork`�?

desktop 开发编排默认使�?PostgreSQL：`pnpm dev:desktop 默认使用 PostgreSQL`。安装后�?desktop runtime 本地数据默认保留 SQLite，文件位�?`~/.sdkwork/chat/data/chat.sqlite`。本文的 PostgreSQL profile 用于 `pnpm dev`、`pnpm dev:browser`、`pnpm dev:desktop`、server 集成测试和数据库迁移验证�?
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public_ai_dev
...
Windows 跑应用，PostgreSQL 跑在 WSL Ubuntu 时，Windows 应用不应该使�?WSL 内部�?Unix socket。Windows PowerShell 访问 WSL PostgreSQL 时必须走 TCP�?
...
初始�?role、database、schema、grant 和默认权限：
...
默认 PostgreSQL 通常只监听本机。如果需要局域网机器访问，需要同时修�?`postgresql.conf`、`pg_hba.conf` 和系统防火墙�?
...
�?`pg_hba.conf` 中添加受控网段，示例认证方式使用 `scram-sha-256`�?
...
如果 `pnpm db:postgres:migrate` �?`permission denied for schema sdkwork_ai_dev`，重新执行第 2 节中�?grant �?`ALTER DEFAULT PRIVILEGES`�?

## 9. 生产配置参�?

生产 server 不使�?`.env.postgres`。使�?`/etc/sdkwork/chat/chat.toml`、`/etc/sdkwork/chat/server.env`、`/etc/sdkwork/chat/postgresql.yaml`、`/etc/sdkwork/chat/database.secret`�?
...
- Server �?container 默认 PostgreSQL�?
