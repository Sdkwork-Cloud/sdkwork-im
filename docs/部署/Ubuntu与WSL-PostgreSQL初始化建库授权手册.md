...
如果 `pnpm db:postgres:migrate` 报 `permission denied for schema sdkwork_ai_dev`，重新执行第 2 节中的 grant 和 `ALTER DEFAULT PRIVILEGES`。
...
- Server 和 container 默认 PostgreSQL。
