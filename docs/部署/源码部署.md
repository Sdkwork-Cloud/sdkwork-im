...
`SDKWORK_IM_APPLICATION_PUBLIC_HTTP_URL` 和 `SDKWORK_IM_APPLICATION_PUBLIC_WEBSOCKET_URL` 是生产 base URL 的核心来源。源码构建命令会读取 `server.env`，前端构建时会把这些地址写进 Vite bundle。修改域名或 websocket 地址后，必须重新运行 `pnpm run server:source:build`。
...
需要。前端 bundle 会读取构建期 base URL，修改 `SDKWORK_IM_APPLICATION_PUBLIC_HTTP_URL` 或 `SDKWORK_IM_APPLICATION_PUBLIC_WEBSOCKET_URL` 后必须重新构建。

### 为什么不用 `pnpm server:dev`

`pnpm server:dev` 是 topology v2 开发栈（`self-hosted.split-services.development`），会启动 `sdkwork-im-server` 与开发网关。生产源码部署使用 `pnpm run server:source:build` 和 `pnpm run server:source:start`。
