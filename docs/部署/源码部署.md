...
`SDKWORK_IM_APPLICATION_PUBLIC_HTTP_URL` 和 `SDKWORK_IM_APPLICATION_PUBLIC_WEBSOCKET_URL` 是生产 base URL 的核心来源。源码构建命令会读取 `server.env`，前端构建时会把这些地址写进 Vite bundle。修改域名或 websocket 地址后，必须重新运行 `pnpm run build:server:source`。
...
不需要。只改 Rust 后端时，重新运行 Cargo 构建即可；用 `pnpm run build:server:source` 也可以，但会多跑一次前端构建。
...
`pnpm dev:server` 是本地开发栈（`standalone.split-services.development`），会启动 `sdkwork-im-server` 与开发网关。生产源码部署使用 `pnpm run build:server:source` 和 `pnpm run start:server:source`。