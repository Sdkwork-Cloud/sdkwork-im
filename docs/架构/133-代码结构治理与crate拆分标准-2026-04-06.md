...
- 当前 workspace 已具备多 crate / 多 service 基础骨架，但并不等于本文所有目标拆分、命名收敛和目录治理都已全部完成。
...
- 写未来拆分标准时，必须显式标记 `目标态`、`标准目标面` 或 `整改方向`。

## 2026-04-07 As-Built 回写

### 已兑现

- `services/session-gateway/src/lib.rs` 已从 `1192` 行降到 `601` 行，回到本文定义的 `1000` 行硬门禁以内。
- presence 运行时与持久化恢复逻辑已从 `lib.rs` 抽到 `src/presence.rs`，`lib.rs` 继续向“装配与稳定导出入口”收敛。
...
- `services/conversation-runtime/src/runtime/http.rs` 已承接 HTTP 路由装配、请求 DTO、错误映射与 handlers，`runtime.rs` 不再直接暴露 HTTP surface。
- `services/local-minimal-node/src/lib.rs` 已降到 `2` 行，并通过 facade 形式维持原有公开接口。
...
- `services/local-minimal-node/src/lib.rs` 已降到 `2` 行，但 `services/local-minimal-node/src/node.rs` 仍为 `5214` 行。
- 因此本文列出的高风险 `lib.rs` 治理目标尚未全部完成，Step 02 仍不得宣称闭环。

### 治理结论

- `services/*/src/lib.rs` 禁止超长的规则已经开始进入真实代码门禁，而不再只是架构约定。
- 后续 Step 02 必须继续处理 `services/conversation-runtime/src/runtime.rs` 与 `services/local-minimal-node/src/node.rs`，否则不能视为本文标准已兑现。
...
- `conversation-runtime` 已验证第二层模块切分可行：
  ...
  - `src/runtime.rs` 留在 orchestration 与 state mutation 主流程
...
- 最新尺寸：
  ...
  - `services/local-minimal-node/src/node.rs`: `5214`
- 结论：
  - `conversation-runtime` 的 Step 02 进度继续前移，但尚未达标
  - `local-minimal-node` 仍是 Step 02 主阻塞项
  - 不能据此放行 `91 / 95 / 93`

### 2026-04-07 As-Built 增量回写 3

- `local-minimal-node` 已验证 `effects` 边界可独立抽离：
...
- 最新尺寸：
  - `services/local-minimal-node/src/node.rs`: `4676`
  - `services/local-minimal-node/src/node/effects.rs`: `551`
  - `services/conversation-runtime/src/runtime.rs`: `2666`
- 结论：
  - Step 02 的模块级治理继续兑现，但距离闭环仍远
  - 下一轮仍需继续压缩 `node.rs` 与 `runtime.rs`
### 2026-04-07 As-Built 增量回写 4

- `local-minimal-node` 已验证 Step 02 可继续从单一超大实现文件推进到多边界协同实现：
...
- 最新尺寸：
  ...
  - `services/local-minimal-node/src/node/effects.rs`: `551`
- 这证明 `133` 中定义的 “先做模块边界治理，再做 crate 边界治理” 仍然适用。
- 同时也说明 Step 02 仍未闭环：
  - `services/local-minimal-node/src/node.rs` 仍高于红线
  - `services/conversation-runtime/src/runtime.rs` 仍高于红线
  - 因此 `91 / 95 / 93` 继续阻塞

### 2026-04-07 As-Built 增量回写 5

- `local-minimal-node` 已继续验证 Step 02 可以沿 capability cluster 切分超大 service impl：
  - `src/node.rs` 负责 orchestration / primary handler / route assembly
  - `src/node/access.rs` 负责 access/auth/device-scope gate
  - `src/node/effects.rs` 负责 side effects / realtime publish / RTC signal emit / audit record
  - `src/node/platform.rs` 负责 notification / automation / audit / ops handler 与 node operational refresh
- 最新尺寸：
  - `services/local-minimal-node/src/node.rs`: `4166`
  - `services/local-minimal-node/src/node/platform.rs`: `252`
  - `services/local-minimal-node/src/node/access.rs`: `288`
  - `services/local-minimal-node/src/node/effects.rs`: `551`
- 这进一步证明 `133` 中定义的 “先做模块边界治理，再做 crate 边界治理” 仍然适用。
- 同时也继续说明 Step 02 仍未闭环：
  - `services/local-minimal-node/src/node.rs` 仍高于红线
  - `services/conversation-runtime/src/runtime.rs` 仍高于红线
  - 因此 `91 / 95 / 93` 继续阻塞
### 2026-04-07 As-Built 6

- `local-minimal-node` 已继续证明 Step 02 可以沿 capability cluster 拆分超大 service impl，而不是只能停留在单一 `node.rs`：
  - `src/node/session.rs`
  - `src/node/stream.rs`
  - `src/node/rtc.rs`
- 最新尺寸：
  ...
  - `services/conversation-runtime/src/runtime.rs`: `2666`
- 这条 as-built 继续支持本文标准中“Step 02 先模块化，Step 03 再 crate 化”的执行顺序。
- 但 Step 02 仍未闭环：
  - `services/local-minimal-node/src/node.rs` 仍高于红线
  - `services/conversation-runtime/src/runtime.rs` 仍高于红线
  - `91 / 95 / 93` 仍不得判定通过
...
- `local-minimal-node` 继续验证 Step 02 的“按 capability cluster 切 service impl”标准，而不是提前进入 crate 拆分：
  - `media` handler 已迁出 `node.rs`
  - `projection` handler 已迁出 `node.rs`
  - `membership` handler 已迁出 `node.rs`
  - `message` handler 已迁出 `node.rs`
- `services/local-minimal-node/tests/lib_structure_test.rs` 已继续增加对应 structure gate，说明“边界是否回流”已经被转化为可执行验证，而不是仅靠人工约定。
- 最新 as-built 尺寸：
...
- 这条 as-built 继续支持本文标准中的执行顺序：
  ...
  - Step 03 再基于稳定边界进入 crate 化
- 同时也继续说明 Step 02 尚未闭环：
  - `local-minimal-node/src/node.rs` 仍保留 `create/handoff` 主簇
  - `conversation-runtime/src/runtime.rs` 仍是并列主阻塞项
  - `91 / 95 / 93` 仍不能通过

### 2026-04-07 As-Built 8

- `local-minimal-node` 继续验证 Step 02 的“按 capability / orchestration cluster 切 service impl”标准，而不是提前进入 crate 拆分：
  - `build/runtime assembly` 已迁出 `node.rs`
  - `route assembly` 已迁入 `node/build.rs`
  - `runtime bootstrap / journal replay` 已迁入 `node/build.rs`
- `services/local-minimal-node/tests/lib_structure_test.rs` 已继续增加 build surface gate，说明“边界是否回流”被转化为可执行验证，而不是依赖人工约定。
- 最新 as-built 尺寸：
...
- 这条 as-built 继续支撑本文标准中的执行顺序：
  ...
  - Step 03 再基于稳定边界进入 crate 化
- 同时也继续说明 Step 02 尚未闭环：
  - `local-minimal-node/src/node.rs` 仍保留 runtime-dir / restore helper 主簇
  - `conversation-runtime/src/runtime.rs` 仍是并列主阻塞项
  - `91 / 95 / 93` 仍不能通过
...
- `test_local_minimal_node_runtime_dir_surface_moves_out_of_node_impl` 已 red -> green。
- `conversation-runtime`
- `src/runtime.rs` 不再保留 recovered replay handlers，相关逻辑迁入 `src/runtime/recovery.rs`。
- `test_conversation_runtime_recovery_surface_moves_out_of_runtime_impl` 已 red -> green。
- 最新尺寸证据：
...
- `conversation-runtime`
  ...
  - `test_conversation_runtime_helper_surface_moves_out_of_runtime_impl` 已 red -> green。
- `local-minimal-node`
...
- `conversation-runtime`
  ...
  - `test_conversation_runtime_creation_surface_moves_out_of_runtime_impl` 已 red -> green。
- `local-minimal-node`
...
- `local-minimal-node`
  ...
  - `test_local_minimal_node_runtime_dir_preview_surface_moves_out_of_runtime_dir_impl` 已 red -> green。
- `conversation-runtime`
...
- 最新尺度证据：
  - `services/local-minimal-node/src/node/runtime_dir.rs`: `732`
  - `services/local-minimal-node/src/node/runtime_dir/preview.rs`: `248`
  - `services/local-minimal-node/src/node/runtime_dir/preview/diff.rs`: `829`
  - `services/local-minimal-node/src/node/runtime_dir/preview/format.rs`: `158`
- 结论更新：
  - `runtime.rs`、`node.rs`、`runtime_dir.rs` 已不再违反本文的单文件 `1000` 行硬红线。
  - 但 repo 内仍有生产文件超线：
...
- `session-gateway`
  ...
  - `test_session_gateway_cluster_disconnect_surface_moves_out_of_cluster_impl` 已 red -> green。
  - `src/realtime.rs` 不再保留 checkpoint/subscription memory store 与 record helper，相关逻辑迁入 `src/realtime/storage.rs`。
  - `test_session_gateway_realtime_storage_surface_moves_out_of_realtime_impl` 已 red -> green。
- 最新尺度证据：
...
- 结论更新：
  - `session-gateway` 已回到单文件 `1000` 行硬红线内，不再构成 Step 02 的当前阻塞。
  - 但 repo 内仍有生产文件超线：
    - `adapters/local-disk/src/lib.rs`: `1467`
    - `tools/chat-cli/src/lib.rs`: `1565`
    - `services/projection-service/src/lib.rs`: `1347`
