...
- `domain-*` 不允许依�?`axum`
- `domain-*` 不允许依赖具体数据库实现
- `interface-*` 不允许直接依赖具体存储驱�?
- `services/*` 不允许承载复杂业务实�?
- `runtime-*` 只做运行时编排，不定义领域语�?
...
- `contract-*` 负责对外协议、稳定边界、DTO 与业�?schema
...
- `interface-*` 禁止自行定义 envelope 类型或重复实�?`hello/auth/resume` 状态机
- `services/*` 禁止直接依赖 `ccp-codec-json/cbor` 以外的协议细节实现文�?
...
- 当前 workspace 已具备多 crate / �?service 基础骨架，但并不等于本文所有目标拆分、命名收敛和目录治理都已全部完成�?
- 本文文末 `As-Built 回写` 才是当前结构治理的真实证据�?

### B. 本文哪些是目标�?

- �?2-11 节中的治理目标、红线、目录模板、crate 分层、命名规则和 CI 标准，属于标准目标面�?
- 这些章节回答的是“代码结构应如何治理”，不等于当前仓库已达到最终拆分终态�?

### C. 文档口径规则

- 写当前工程结构时，以 workspace 真实成员、`03` 与本�?`As-Built 回写` 为准�?
- 写未来拆分标准时，必须显式标�?`目标态`、`标准目标面` �?`整改方向`�?

## 2026-04-07 As-Built 回写

### 已兑�?

- `services/session-gateway/src/lib.rs` 已从 `1192` 行降�?`601` 行，回到本文定义�?`1000` 行硬门禁以内�?
- presence 运行时与持久化恢复逻辑已从 `lib.rs` 抽到 `src/presence.rs`，`lib.rs` 继续向“装配与稳定导出入口”收敛�?
- 已新�?`services/session-gateway/tests/lib_structure_test.rs`，对 `lib.rs` 长度做回归保护�?
- `services/conversation-runtime/src/lib.rs` 已降�?`2` 行，并通过 facade 形式维持原有公开接口�?
- `services/conversation-runtime/src/runtime/http.rs` 已承�?HTTP 路由装配、请�?DTO、错误映射与 handlers，`runtime.rs` 不再直接暴露 HTTP surface�?
- `services/sdkwork-im-gateway/src/lib.rs` 已降�?`2` 行，并通过 facade 形式维持原有公开接口�?

### 仍待兑现

- `services/conversation-runtime/src/lib.rs` 已降�?`2` 行，并新�?`services/conversation-runtime/src/runtime/policy.rs`，但 `services/conversation-runtime/src/runtime.rs` 仍为 `2666` 行�?
- `services/sdkwork-im-gateway/src/lib.rs` 已降�?`2` 行，�?`services/sdkwork-im-gateway/src/node.rs` 仍为 `5214` 行�?
- 因此本文列出的高风险 `lib.rs` 治理目标尚未全部完成，Step 02 仍不得宣称闭环�?

### 治理结论

- `services/*/src/lib.rs` 禁止超长的规则已经开始进入真实代码门禁，而不再只是架构约定�?
- 后续 Step 02 必须继续处理 `services/conversation-runtime/src/runtime.rs` �?`services/sdkwork-im-gateway/src/node.rs`，否则不能视为本文标准已兑现�?
...
- `conversation-runtime` 已验证第二层模块切分可行�?
  ...
  - `src/runtime.rs` 留在 orchestration �?state mutation 主流�?
- 这说�?Step 02 的“先把超大实现文件切成可验证边界模块，再进入 Step 03 crate 化”路径仍成立�?
- 最新尺寸：
  - `services/conversation-runtime/src/runtime.rs`: `2666`
  - `services/conversation-runtime/src/runtime/policy.rs`: `461`
  - `services/sdkwork-im-gateway/src/node.rs`: `5214`
- 结论�?
...
- `sdkwork-im-server` 已验�?`effects` 边界可独立抽离：
...
- 最新尺寸：
  - `services/sdkwork-im-gateway/src/node.rs`: `4676`
  - `services/sdkwork-im-gateway/src/node/effects.rs`: `551`
  - `services/conversation-runtime/src/runtime.rs`: `2666`
- 结论�?
...
- `sdkwork-im-server` 已验�?Step 02 可继续从单一超大实现文件推进到多边界协同实现�?
...
- 最新尺寸：
  - `services/sdkwork-im-gateway/src/node.rs`: `4407`
  - `services/sdkwork-im-gateway/src/node/access.rs`: `288`
  - `services/sdkwork-im-gateway/src/node/effects.rs`: `551`
- 这证�?`133` 中定义的 “先做模块边界治理，再做 crate 边界治理�?仍然适用�?
- 同时也说�?Step 02 仍未闭环�?
  - `services/sdkwork-im-gateway/src/node.rs` 仍高于红�?
  - `services/conversation-runtime/src/runtime.rs` 仍高于红�?
  - 因此 `91 / 95 / 93` 继续阻塞

### 2026-04-07 As-Built 增量回写 5

- `sdkwork-im-server` 已继续验�?Step 02 可以�?capability cluster 切分超大 service impl�?
...
- 最新尺寸：
  - `services/sdkwork-im-gateway/src/node.rs`: `4166`
  - `services/sdkwork-im-gateway/src/node/platform.rs`: `252`
  - `services/sdkwork-im-gateway/src/node/access.rs`: `288`
  - `services/sdkwork-im-gateway/src/node/effects.rs`: `551`
- 这进一步证�?`133` 中定义的 “先做模块边界治理，再做 crate 边界治理�?仍然适用�?
- 同时也继续说�?Step 02 仍未闭环�?
  - `services/sdkwork-im-gateway/src/node.rs` 仍高于红�?
  - `services/conversation-runtime/src/runtime.rs` 仍高于红�?
  - 因此 `91 / 95 / 93` 继续阻塞
...
- `sdkwork-im-server` 已继续证�?Step 02 可以�?capability cluster 拆分超大 service impl，而不是只能停留在单一 `node.rs`�?
  - `src/node/session.rs`
  - `src/node/stream.rs`
  - `src/node/rtc.rs`
- 最新尺寸：
  - `services/sdkwork-im-gateway/src/node.rs`: `3672`
  - `services/sdkwork-im-gateway/src/node/session.rs`: `232`
  - `services/sdkwork-im-gateway/src/node/stream.rs`: `103`
  - `services/sdkwork-im-gateway/src/node/rtc.rs`: `117`
  - `services/conversation-runtime/src/runtime.rs`: `2666`
...
- `sdkwork-im-server` 继续验证 Step 02 的“按 capability cluster �?service impl”标准，而不是提前进�?crate 拆分�?
  - `media` handler 已迁�?`node.rs`
  - `projection` handler 已迁�?`node.rs`
  - `membership` handler 已迁�?`node.rs`
  - `message` handler 已迁�?`node.rs`
- `services/sdkwork-im-gateway/tests/lib_structure_test.rs` 已继续增加对�?structure gate，说明“边界是否回流”已经被转化为可执行验证，而不是仅靠人工约定�?
- 最�?as-built 尺寸�?
  - `services/sdkwork-im-gateway/src/node.rs`: `3147`
  - `services/sdkwork-im-gateway/src/node/media.rs`: `69`
  - `services/sdkwork-im-gateway/src/node/projection.rs`: `115`
  - `services/sdkwork-im-gateway/src/node/membership.rs`: `215`
  - `services/sdkwork-im-gateway/src/node/message.rs`: `145`
  - `services/conversation-runtime/src/runtime.rs`: `2666`
- 这条 as-built 继续支持本文标准中的执行顺序�?
  - Step 02 先把单体超大实现文件切到模块级边�?
  - Step 03 再基于稳定边界进�?crate �?
- 同时也继续说�?Step 02 尚未闭环�?
  - `sdkwork-im-server/src/node.rs` 仍保�?`create/handoff` 主簇
  - `conversation-runtime/src/runtime.rs` 仍是并列主阻塞项
  - `91 / 95 / 93` 仍不能通过

### 2026-04-07 As-Built 8

- `sdkwork-im-server` 继续验证 Step 02 的“按 capability / orchestration cluster �?service impl”标准，而不是提前进�?crate 拆分�?
  - `build/runtime assembly` 已迁�?`node.rs`
  - `route assembly` 已迁�?`node/build.rs`
  - `runtime bootstrap / journal replay` 已迁�?`node/build.rs`
- `services/sdkwork-im-gateway/tests/lib_structure_test.rs` 已继续增�?build surface gate，说明“边界是否回流”被转化为可执行验证，而不是依赖人工约定�?
- 最�?as-built 尺寸�?
  - `services/sdkwork-im-gateway/src/node.rs`: `2459`
  - `services/sdkwork-im-gateway/src/node/build.rs`: `511`
  - `services/sdkwork-im-gateway/src/node/conversation.rs`: `84`
  - `services/sdkwork-im-gateway/src/node/handoff.rs`: `106`
  - `services/conversation-runtime/src/runtime.rs`: `2666`
- 这条 as-built 继续支撑本文标准中的执行顺序�?
  - Step 02 先把单体超大实现文件切到模块级边�?
  - Step 03 再基于稳定边界进�?crate �?
- 同时也继续说�?Step 02 尚未闭环�?
  - `sdkwork-im-server/src/node.rs` 仍保�?runtime-dir / restore helper 主簇
  - `conversation-runtime/src/runtime.rs` 仍是并列主阻塞项
  - `91 / 95 / 93` 仍不能通过
...
- 最新尺寸证据：
- `services/sdkwork-im-gateway/src/node.rs`: `524`
- `services/sdkwork-im-gateway/src/node/runtime_dir.rs`: `1947`
...
- `sdkwork-im-server`
  - 在依�?`conversation-runtime` 最新结构后重新跑完整包测试，验证跨包稳定性保持为绿�?
- 最新尺度证据：
...
- `sdkwork-im-server`
  - 在依�?`conversation-runtime` 最新结构后重新跑完整包测试，跨包稳定性继续保持为绿�?
- 最新尺度证据：
...
- `conversation-runtime`
  - 当前工作树中的进一步拆分已�?`handoff / membership` surface 迁出 `src/runtime.rs`�?
  - 最新尺度：
...
- 最新尺度证据：
  - `services/sdkwork-im-gateway/src/node/runtime_dir.rs`: `732`
  - `services/sdkwork-im-gateway/src/node/runtime_dir/preview.rs`: `248`
  - `services/sdkwork-im-gateway/src/node/runtime_dir/preview/diff.rs`: `829`
  - `services/sdkwork-im-gateway/src/node/runtime_dir/preview/format.rs`: `158`
- 结论更新�?
  - `runtime.rs`、`node.rs`、`runtime_dir.rs` 已不再违反本文的单文�?`1000` 行硬红线�?
  - �?repo 内仍有生产文件超线：
...
- 关键文件最新行数：
  ...
  - `services/sdkwork-im-gateway/src/lib.rs`: `3`
