...
本标准不替代 durable `sync-feed`。它只定义在线设备的实时事件窗口投递规则。
...
- `sync-feed` 是 durable truth，负责断线重放和补偿
...
- realtime 只做在线下行，durable 补偿仍由 `sync-feed` 承担
