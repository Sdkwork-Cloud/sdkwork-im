...
在完成 RTC 会话绑定写路径授权收口后，继续 review `local-minimal-node` 的 stream 聚合入口，发现 `scopeKind=conversation` 的 stream 仍存在同类越权缺陷：

- 非会话成员此前可以直接调用 `POST /api/v1/streams`，创建绑定任意已知 `conversationId` 的会话流。
- 非会话成员此前可以继续调用：
  - `POST /api/v1/streams/{id}/frames`
  - `POST /api/v1/streams/{id}/checkpoint`
  - `POST /api/v1/streams/{id}/complete`
  - `POST /api/v1/streams/{id}/abort`
  - `GET /api/v1/streams/{id}/frames`
...
- `POST /api/v1/streams` 请求体中的 `scopeKind = conversation`
...
`POST /api/v1/streams` 还必须同时满足既有 stream 幂等/冲突标准：
...
本轮收口后，`local-minimal-node` 的 conversation-scoped stream 具备以下行为：
