...
- 浏览器、App、桌面端、IoT 设备都能找到合适绑定
- 握手、恢复和断开不再散落在业务 API 里
...
- 相同的权威身份边界
- 相同的 envelope 模型
- 相同的 resume / heartbeat / goaway 行为约束
...
这使得浏览器、移动端和 IoT 设备可以采用最贴合自身约束的 transport，而不破坏统一协议面。
...
| `ccp/mqtt/1` | 设备与边缘链路 | 轻量、可离线缓冲、适合低功耗设备 | 设备、边缘节点 |
| `ccp/quic/1` | 预留高性能绑定 | 多路复用、低时延、弱网潜力更强 | 未来客户端 |
...
- `Connected`：transport 已建立，但尚未完成 CCP 协议协商
...
- `heartbeat` 不承担业务语义
- 心跳超时后，连接层可以关闭 transport
- presence 是否外显为在线状态，由业务策略决定，不由心跳帧直接决定
...
- `auth_bind` 失败后必须终止会话，不允许匿名继续进入业务阶段
- 在 `Active` 之前，禁止发送普通 `cmd / stream / rtc` 帧
...
- 响应可以是单一 `ack / evt / error`，也可以是带 chunk 的流式响应
...
- 控制帧优先级高于普通消息帧
- 慢消费者治理、出站队列和 backpressure 必须在 Link Plane 统一实现
...
- 主要解决“单向服务端持续输出”场景
- 尤其适合浏览器中的 AI 流式结果输出、通知下行和受限网络环境

#### 约束

- `SSE` 只负责下行
- 上行命令通过 `ccp/http/1` 提交
- SSE 会话可通过服务端签发的订阅票据关联到权威 session
- 下行数据仍然使用 CCP envelope，不允许改成另一套 ad-hoc JSON
...
- 面向 IoT 设备、边缘网关、低功耗或不稳定网络场景

#### 约束

- MQTT 只是一种 transport binding，payload 仍然遵循 CCP envelope
- 认证可通过 broker 凭据、设备证书或设备签名扩展完成
- 设备不允许直接声明 `tenantId / actorId / sender / routeEpoch`
- QoS 与 topic 规划服从设备侧可靠性策略，但不能破坏 CCP 的幂等和恢复语义
...
- 协议语义必须与既有 binding 共用同一套握手和 envelope
- 不允许因为上 QUIC 而重新定义一套不兼容协议

## 8. 能力协商

能力协商在 `hello` / `hello_ack` 阶段完成，典型能力包括：
...
- 减少断线重连带来的用户感知抖动
- 为 Route Plane 的 fencing 和多节点迁移提供稳定模型
...
- 当前仓库已具备协议母体、握手/治理基线和部分 runtime 消费链，但并不等于本文列出的所有 binding、恢复语义、设备路径与未来 `QUIC` 规划都已完整落地。
- 当前真实已兑现部分，应以本文文末 `As-Built`、`143 / 145 / 148` 的 `As-Built`、以及 `session-gateway / runtime-link / control-plane-api` 的代码事实为准。

### B. 本文哪些是目标态

- 第 2-11 节中的 binding 原则、统一状态机、L2 控制帧、标准握手顺序和各 binding 设计，属于标准目标面。
- 这些章节用于冻结长期连接语义和 transport contract，不等于所有 transport 都已商用交付。

### C. 文档口径规则

- 写当前握手现状时，只能引用已落地 binding、真实 hello/auth/resume 行为和对应测试证据。
- 写未来 binding 设计时，必须显式标记 `目标态`、`预留` 或 `标准目标面`。
...
- `WebSocket` 主实时链路已在真实入口落地到 `services/session-gateway/src/websocket.rs`：
  - 子协议固定为 `craw-chat.ccp.ws.v1`
  - 线协议通过 `RealtimeWebsocketMode::{LegacyJson, CcpJson}` 同时支持旧 JSON 帧和 CCP JSON 帧
- 当前仓库的实际握手顺序已经从文档冻结转为主入口实现：
  ...
  - 客户端发送 `auth_bind`
  - 服务端发送 `auth_ok`
  - 进入 `Active` 阶段后收发 CCP-wrapped 业务帧
- `local-minimal-node` 的 `/im/v3/api/realtime/ws` 入口已复用相同的子协议协商和 wire mode 选择，不再维护独立 WebSocket 协议细节
- `tools/chat-cli/src/realtime.rs` 已作为当前桌面 CLI 的 L2/L3 参考实现：
  - 主动请求 `craw-chat.ccp.ws.v1`
  - 完成 `hello -> hello_ack -> auth_bind -> auth_ok`
  - 将 CCP 业务帧解包为既有 JSON 业务输出，作为 Step 03 的兼容桥
- 尚未在本 step 落地的部分：
  - `session_resume / session_resumed` 的真实恢复链
  - `goaway / drain / routeEpoch` 的热迁移运行时
  - 以上继续由 `Step 04` 的 Link Plane / Route Plane 兑现
