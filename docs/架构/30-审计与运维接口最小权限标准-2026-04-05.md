...
完成后，`craw-chat` 已经把所有独立服务的公网入口收敛为 `Bearer-only`。

但继续 review 后发现：

- `audit-service`
- `ops-service`
- `local-minimal-node` 中复用的 `/api/v1/audit/*` 与 `/api/v1/ops/*`
...
### 2.3 `local-minimal-node` 继承同样问题

`local-minimal-node` 没有直接复用 `ops-service` / `audit-service` 的 Router，
而是在自身聚合路由中实现了同名 handler。

因此即使独立服务补了权限校验，
`local-minimal-node` 仍可能继续暴露同样的授权缺口。

## 3. 冻结标准

本轮之后，冻结以下授权规则：

1. `GET /api/v1/ops/*` 必须要求 `ops.read`
2. `GET /api/v1/audit/records` 与 `GET /api/v1/audit/export`
   必须要求 `audit.read`
3. `POST /api/v1/audit/records` 必须要求 `audit.write`
...
- `cargo test -p local-minimal-node --test public_auth_e2e_test --offline`
...
1. `craw-chat` 对高敏审计与运维读写接口，已经从“只有认证”升级到“认证 + 最小权限授权”
2. Bearer-only 公网入口与 handler 内授权规则已经对齐
3. `local-minimal-node` 不再是独立服务修复后的旁路回退点

## 7. 下一步

本轮仍然是“permission-based 最小闭环”，后续建议继续推进：

1. 把 `tenant.admin`、`ops.read`、`audit.read`、`audit.write`
   进一步映射到更正式的角色模型，例如 operator / admin / auditor
2. 为 `control-plane-api` 补同类 capability 权限，例如 `control.write`
