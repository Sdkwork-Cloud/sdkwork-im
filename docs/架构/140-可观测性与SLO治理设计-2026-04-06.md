...
- 当前仓库已形成部分 diagnostics、脚本化验证、链路指标与测试资产，但并不等于本文定义的全量四件套与 SLO 治理平台都已完整落地。
- 文末 `As-Built` 才是当前真实证据。

### B. 本文哪些是目标态

- 第 2-10 节中的观测四件套、各 plane 指标、SLO、告警与治理要求，属于标准目标面。
- 这些章节定义的是长期观测治理标准，不等于当前所有服务都已接入完整指标/追踪/日志/事件体系。

### C. 文档口径规则

- 写当前观测现状时，以本文 `As-Built`、当前 diagnostics/test/script 事实为准。
- 写未来观测标准时，必须显式标记 `目标态`、`标准目标面` 或 `治理目标`。

## 2026-04-08 As-Built 1

- `Wave C / Step 09 / CP09-3` 已补出 `Projection Plane` 的第一条真实 diagnostics evidence。
- 当前真实已落地的能力包括：
  ...
  - snapshot persist / restore 现在会记录真实：
    ...
    - structured log
    - last failure
  - `ops-service` 的 `/backend/v3/api/ops/health` 与 `/backend/v3/api/ops/diagnostics` 现在都暴露 `projectionPlane`
  - `local-minimal-node` 的 runtime-dir 恢复流程会把 restore 后的 projection plane 状态写回 ops runtime
- 这让本文中的要求开始具备代码证据：
  - `5.5 Projection Plane` 不再只有概念指标，而开始有真实 snapshot/recovery counter
  - `8. Diagnostics` 中的 `ops/health` 与 `ops/diagnostics` 已拥有 projection plane 专属视图
  - `9. 观测治理要求` 中“新增 plane 级能力必须补 health 和 diagnostics”已在 snapshot/recovery 路径上得到第一次兑现
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` 的 observability 从“snapshot/recovery counter”推进到“replay/backlog/lag evidence”。
- 当前真实已落地的能力包括：
  - `projectionPlane.replay` 现在稳定暴露：
    - `backlogSize`
    - `replayedEventCount`
    - `durationMs`
  - `ops/lag` 现在开始暴露 plane-specific 的 `projection_replay` lag item
  - stale snapshot restart 场景已形成自动化验证，证明 replay evidence 与恢复结果一致
- 这让本文中的要求继续具备代码证据：
  - `5.5 Projection Plane` 中的：
    ...
    - `replay duration`
    已开始有真实实现，而不再只是文档指标
  - `8. Diagnostics` 中的 `ops/lag` 已开始拥有 projection-specific replay 证据
  - `9. 观测治理要求` 中“新增 plane 级能力必须补 health 和 lag 观测”已进一步兑现到 replay 主路径
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` 的 observability 推进到专门的 `ops/replay-status` 读面。
- 当前真实已落地的能力包括：
  - `ops/replay-status` 已真实落地，而不再停留在本文第 `8` 节的建议列表中
...
- 这让本文中的要求继续具备代码证据：
  ...
  - `9. 观测治理要求` 中“新增 plane 级能力必须补 health 和 lag 观测”已进一步推进到“补专门 replay 视图”
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` 的 observability 推进到 live `inbox / timeline update delay`。
- 当前真实已落地的能力包括：
  - `projectionPlane.updateDelay` 现在稳定暴露：
    ...
    - `scopeId`
    - `recordedAt`
  - `ops/health` 与 `ops/diagnostics` 现在都拥有这份 live apply 延迟证据
  - 这份指标不是外部脚本估算，而是直接来自 `projection-service.apply(...)` 主路径
- 这让本文中的要求继续具备代码证据：
  - `5.5 Projection Plane` 中的 `inbox/timeline update delay` 已开始真实落地
  - `9. 观测治理要求` 中“新增 plane 级能力必须补 health 和 diagnostics”已继续推进到 live projection 指标
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` 的 observability 推进到 live `projection lag`。
- 当前真实已落地的能力包括：
  - `ops/lag` 与 `diagnostics.lag` 现在稳定暴露：
  ...
  - 这份指标直接来自 `projection-service.apply(...)` 的真实主路径：
    ...
    - apply 成功后记录已追平的 committed offset
  - `ops/replay-status` 继续只保留 replay drill 语义，没有与 live lag 混用
- 这让本文中的要求继续具备代码证据：
  - `5.5 Projection Plane` 中的 live `projection lag` 已开始真实落地
  - `8. Diagnostics` 中的 `ops/lag` 已开始同时承载 replay 与 steady-state 两类 projection lag 证据
  - `9. 观测治理要求` 中“新增 plane 级能力必须补 health 和 lag 观测”已进一步推进到 steady-state projection apply 主路径
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` 的 observability 推进到 `rebuild duration`。
- 当前真实已落地的能力包括：
  - `projectionPlane.rebuildDurationMs` 现在稳定暴露在：
    - `ops/health`
    - `ops/diagnostics`
  - 这份指标直接来自 startup projection recovery 主路径，而不是 replay-only 指标推导或脚本侧估算
...
- 这意味着本文件中的 Step 11 观测要求已开始具备统一 vocabulary，但仍未形成：
  - 真实观测采样结果
  - alert threshold
  - SLO 守门判定
...
- 这意味着本文件中的 Step 11 观测要求现在已经拥有：
  - 性能量化输出
  - HA / DR drill 输出
  - 同一 `CI Smoke Tier / local-minimal` 基线下的两类证据
- 当前仍未兑现的部分：
  - alert threshold
  - SLO 守门判定
  - 更高层级环境的量化与 drill 输出
...
- 这意味着本文件中的 Step 11 观测要求现在已经拥有：
  ...
  - rollback vocabulary
  - 同一 `CI Smoke Tier / local-minimal` 基线下的统一输出口径
- 当前仍未兑现的部分：
  - alert threshold
  - SLO 守门判定
  - 更高层级环境的量化与 drill 输出
