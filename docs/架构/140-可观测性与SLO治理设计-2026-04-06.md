...
本文档用于把 `sdkwork-im` 的可观测性从“有一�?metrics �?diagnostics”提升为完整治理体系，明确：

- �?plane 需要暴露什么指标、日志、追踪和事件
- 哪些服务等级目标必须被持续观�?
- 如何把架构设计转化为运维可判断、可告警、可回溯的系�?
...
应围绕用户感知与系统不变量设�?SLO，而不是只�?CPU / 内存�?
...
- 新增主链功能必须同时�?metric / trace / error taxonomy�?
- 新增 plane 级能力必须补 health �?lag 观测�?
- 所有拒绝类错误必须�?reason code，而不是只有文字描述�?

## 10. 结论

`sdkwork-im` 的可观测性不能停留在“接�?Prometheus �?tracing”这一层，而必须形成和六个 plane 对齐�?SLO、告警和诊断体系。只有这样，架构才能真正进入可上线、可运维、可扩容状态�?

## 2026-04-09 增补：可观测性标准与当前实现边界

### A. 当前实现真相

- 当前仓库已形成部�?diagnostics、脚本化验证、链路指标与测试资产，但并不等于本文定义的全量四件套�?SLO 治理平台都已完整落地�?
- 文末 `As-Built` 才是当前真实证据�?

### B. 本文哪些是目标�?

- �?2-10 节中的观测四件套、各 plane 指标、SLO、告警与治理要求，属于标准目标面�?
- 这些章节定义的是长期观测治理标准，不等于当前所有服务都已接入完整指�?追踪/日志/事件体系�?

### C. 文档口径规则

- 写当前观测现状时，以本文 `As-Built`、当�?diagnostics/test/script 事实为准�?
- 写未来观测标准时，必须显式标�?`目标态`、`标准目标面` �?`治理目标`�?

## 2026-04-08 As-Built 1

- `Wave C / Step 09 / CP09-3` 已补�?`Projection Plane` 的第一条真�?diagnostics evidence�?
- 当前真实已落地的能力包括�?
  ...
  - snapshot persist / restore 现在会记录真实：
    ...
    - structured log
    - last failure
  - `ops-service` �?`/backend/v3/api/ops/health` �?`/backend/v3/api/ops/diagnostics` 现在都暴�?`projectionPlane`
  - `sdkwork-im-server` �?runtime-dir 恢复流程会把 restore 后的 projection plane 状态写�?ops runtime
- 这让本文中的要求开始具备代码证据：
  - `5.5 Projection Plane` 不再只有概念指标，而开始有真实 snapshot/recovery counter
  - `8. Diagnostics` 中的 `ops/health` �?`ops/diagnostics` 已拥�?projection plane 专属视图
  - `9. 观测治理要求` 中“新�?plane 级能力必须补 health �?diagnostics”已�?snapshot/recovery 路径上得到第一次兑�?
- 当前仍未兑现的部分：
...
- 当前真实已落地的能力包括�?
  - `projectionPlane.replay` 现在稳定暴露�?
    ...
    - `replayedEventCount`
    - `durationMs`
  - `ops/lag` 现在开始暴�?plane-specific �?`projection_replay` lag item
  - stale snapshot restart 场景已形成自动化验证，证�?replay evidence 与恢复结果一�?
- 这让本文中的要求继续具备代码证据�?
  - `5.5 Projection Plane` 中的�?
    ...
    - `replay duration`
    已开始有真实实现，而不再只是文档指�?
  - `8. Diagnostics` 中的 `ops/lag` 已开始拥�?projection-specific replay 证据
  - `9. 观测治理要求` 中“新�?plane 级能力必须补 health �?lag 观测”已进一步兑现到 replay 主路�?
- 当前仍未兑现的部分：
...
- 当前真实已落地的能力包括�?
  - `ops/replay-status` 已真实落地，而不再停留在本文�?`8` 节的建议列表�?
  - `Projection Plane` 现在可以通过该读面直接公开�?
    - `idle / replayed`
    - `backlogSize`
    - `replayedEventCount`
    - `durationMs`
  ...
  - 默认空闲态和真实 stale snapshot replay 态都已有自动化测试，证明该读面不是静态字段拼�?
- 这让本文中的要求继续具备代码证据�?
  - `5.5 Projection Plane` 中的 `replay throughput` 已开始落�?
  - `8. Diagnostics` 中建议的 `ops/replay-status` 已开始成为真实公开接口
  - `9. 观测治理要求` 中“新�?plane 级能力必须补 health �?lag 观测”已进一步推进到“补专门 replay 视图�?
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` �?observability 推进�?live `inbox / timeline update delay`�?
- 当前真实已落地的能力包括�?
  - `projectionPlane.updateDelay` 现在稳定暴露�?
    ...
    - `scopeId`
    - `recordedAt`
  - `ops/health` �?`ops/diagnostics` 现在都拥有这�?live apply 延迟证据
  - 这份指标不是外部脚本估算，而是直接来自 `projection-service.apply(...)` 主路�?
- 这让本文中的要求继续具备代码证据�?
  - `5.5 Projection Plane` 中的 `inbox/timeline update delay` 已开始真实落�?
  - `9. 观测治理要求` 中“新�?plane 级能力必须补 health �?diagnostics”已继续推进�?live projection 指标
  - 既有 Step 02 代码治理红线也在本轮被守住，没有为了补指标破坏模块边�?
- 当前仍未兑现的部分：
...
- `Wave C / Step 09 / CP09-3` 已继续把 `Projection Plane` �?observability 推进�?live `projection lag`�?
- 当前真实已落地的能力包括�?
  ...
  - 这份指标直接来自 `projection-service.apply(...)` 的真实主路径�?
    ...
    - apply 成功后记录已追平�?committed offset
  - `ops/replay-status` 继续只保�?replay drill 语义，没有与 live lag 混用
- 这让本文中的要求继续具备代码证据�?
  - `5.5 Projection Plane` 中的 live `projection lag` 已开始真实落�?
  - `8. Diagnostics` 中的 `ops/lag` 已开始同时承�?replay �?steady-state 两类 projection lag 证据
  - `9. 观测治理要求` 中“新�?plane 级能力必须补 health �?lag 观测”已进一步推进到 steady-state projection apply 主路�?
- 当前仍未兑现的部分：
...
- 当前真实已落地的能力包括�?
  - `projectionPlane.rebuildDurationMs` 现在稳定暴露在：
    - `ops/health`
    - `ops/diagnostics`
  - 这份指标直接来自 startup projection recovery 主路径，而不�?replay-only 指标推导或脚本侧估算
  - snapshot-only recovery 已有自动化测试证明：
    - `replay.durationMs` 仍可保持 `0`
    - `rebuildDurationMs` 仍可为正�?
- 这让本文中的要求继续具备代码证据�?
  - `5.5 Projection Plane` 中的�?
    - `projection lag`
    - `rebuild duration`
    ...
    - `replay throughput`
    现已全部具备真实实现
  - `8. Diagnostics` 中建议的 `ops/health` / `ops/lag` / `ops/diagnostics` / `ops/replay-status` 已在 `Projection Plane` 上形成最小闭�?
- 当前仍未兑现的部分：
  - 更完整的 SLO / alert threshold / error taxonomy 仍未落地
  - 其他 plane 的统一观测收口仍未落地
...
- 这意味着本文件中�?Step 11 观测要求已开始具备统一 vocabulary，但仍未形成�?
  - 真实观测采样结果
  - alert threshold
  - SLO 守门判定
...
- `Wave D / Step 11 / CP11-2` 已把 Step 11 的统一量化词汇推进到真实输出结果，而不再只停留�?catalog 命名�?
...
- 这意味着本文件中�?Step 11 观测要求现在已经拥有�?
  - 性能量化输出
  - HA / DR drill 输出
  - 同一 `CI Smoke Tier / self-hosted.split-services.development` 基线下的两类证据
- 当前仍未兑现的部分：
  - alert threshold
  - SLO 守门判定
  - 更高层级环境的量化与 drill 输出
...
- `Wave D / Step 11 / CP11-4` 已把 Step 11 的统一观测词汇扩展�?upgrade rollback�?
- 当前 `performance_ha_dr_drill_test` 已稳定输出：
...
- 这意味着本文件中�?Step 11 观测要求现在已经拥有�?
  ...
  - rollback vocabulary
  - 同一 `CI Smoke Tier / self-hosted.split-services.development` 基线下的统一输出口径
- 当前仍未兑现的部分：
  - alert threshold
  - SLO 守门判定
  - 更高层级环境的量化与 drill 输出
