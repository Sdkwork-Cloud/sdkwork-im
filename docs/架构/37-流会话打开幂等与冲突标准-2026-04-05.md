...
- `local-minimal-node` 对 `StreamingError` 做了统一降级映射，导致底层 `409 stream_conflict` 被错误包装成通用 `400`。
...
3. local-minimal-node：通过聚合节点重复打开流时保持幂等，且冲突错误码不被压平。
...
- `rtc-signaling-service` 是否存在 `rtc_session_id` 重复创建覆盖问题。
- `rtc-signaling-service` 的 invite / accept / reject / end 是否具备明确的状态机冲突语义。
