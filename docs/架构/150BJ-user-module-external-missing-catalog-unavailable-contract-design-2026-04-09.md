...
`user-module-external` already had unavailable semantics for unreadable or invalid catalogs, but the missing catalog-path branch still panicked during bootstrap.
...
- This design only covers:
  ...
  - missing or blank `CRAW_CHAT_USER_MODULE_EXTERNAL_CATALOG_PATH`
- It does not change:
  - invalid provider mode behavior
  - unreadable/invalid catalog file behavior
  - user-module provider health route exposure
