...
- `local-minimal-node` 聚合入口在每次请求后都会继续写 audit 和 notification side-effect，导致幂等重放也会重复落副作用。
...
`local-minimal-node` 复用 `AutomationRuntime` 时，必须显式区分：
...
3. 幂等重放不会重复追加自动化事件
4. `local-minimal-node` 幂等重放不会重复写 audit 记录
5. `local-minimal-node` 幂等重放不会产生第二条 `automation.result` 通知可见结果
...
- `media-service` 独立服务与文档对 `POST /api/v1/media/{id}/attach` 的能力描述是否一致，需要继续做文档与实现对账
