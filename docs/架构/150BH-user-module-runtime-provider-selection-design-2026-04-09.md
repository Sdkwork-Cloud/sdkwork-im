...
- `UserModuleProvider` 的主链路接入已存在。
- 但默认启动入口此前始终返回 `LocalUserModuleProvider`。
- 这导致 `user-module-external` 虽有契约、stub 与主链路测试，却没有真实默认装配路径。

## 2. 设计目标

- 默认仍走统一 `UserModuleProvider` port
- 默认值安全：未配置时仍为 `local`
...
- `sender / member / bootstrap member / mutation actor` 继续只消费 `UserModuleProvider`
