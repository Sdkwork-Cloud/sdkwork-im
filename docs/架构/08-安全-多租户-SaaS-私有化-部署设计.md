...
本文档用于冻�?`sdkwork-im` 在安全、多租户隔离、SaaS 形态和私有化部署上的统一设计，确保：
...
- 内部服务通信推荐 `mTLS`
- 默认最小权�?
- 默认 tenant-aware 鉴权与审�?
- 高风险写操作必须具备幂等和回放保�?
- `CCP` 权威字段边界必须前置到接入层
- 安全和多租户校验要尽量靠前进入接入层，而不是拖到业务末�?
...
- `tenantId` 必须来自已校验的 JWT、session �?trusted identity context
- `actorId / actorKind / sessionId / clientRouteId` 必须来自认证上下文或可信绑定关系
- `sender / routeEpoch / serverTs` 必须来自服务端推导与路由状�?
- 业务请求体不得覆盖这些权威字�?
- gateway 负责把权威上下文转换为内部命令上下文与审计字�?

### 3.4 协议安全边界

- 长连接握手必须遵�?`hello -> auth_bind -> session_resume` 的统一协议顺序
- 未完成认证绑定前，不允许进入普通业务帧阶段
- 客户端不得伪�?`tenantId`、`actorId`、`sender`、`routeEpoch`
- 恢复链路必须校验 `resume token`、`sessionId`、`clientRouteId` �?`routeEpoch`
- 设备侧在高信任场景可追加 `device signature`，但不替�?`TLS`
...
- 每个请求、连接、事件和审计记录都必须显式绑�?`tenant_id`
- 不允许从客户端业�?payload 直接信任租户字段
...
- 不做无限大的全局单集�?
- 一�?cell 是故障域、部署域、扩展域和运维域
- �?region 写入必须受控，不允许同一会话�?region 多主�?
...
- 支持滚动升级
- 支持安全下线、扩容和节点排空
- `bin/` 下脚本需支持安装、初始化配置、启动、停止、重启和状态检�?
...
`sdkwork-im` 的安全和部署设计必须以“认证上下文权威�?+ 双轨安全模型 + 多租户全链路隔离 + cell 化部署”为前提。只有这样，系统才能同时满足消费级消息体验、企业协作能力、AI 可治理和私有化落地的要求�?

## 2026-04-09 增补：当前安全落地与部署口径

### A. 当前安全真相

- 当前仓库真实已收口的安全核心是：认证上下文权威化、`tenant_id` 服务端推导、`sender/actor` 服务端生成、连接恢复链路校验、provider 治理面鉴权�?
...
- 文档必须区分“当前可运行 profile”与“目标交�?profile”�?

### C. 文档口径规则

- 写当前应用信息时，优先描述已存在服务、认证边界、ops/control-plane 已验证能力�?
- �?SaaS 或私有化章节时，必须显式标注为目标部署标准�?
- 不允许把未来合规、隔离、灾备能力写成当前已完成事实�?
