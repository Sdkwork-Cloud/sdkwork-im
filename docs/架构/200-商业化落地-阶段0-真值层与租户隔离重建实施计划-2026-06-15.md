...
| P0-4 | Outbox 为本地 JSONL/内存（`FileMessageSideEffectOutboxStore`），非分布式；`im_outbox_events` 表闲置 | `local-minimal-node/src/node/build.rs:1084`；`side_effect_outbox.rs` | ✅ |
...
新增 `services/local-minimal-node/src/node/outbox_relay.rs`（阶段 0 与主进程同生命周期，阶段 1 拆为独立 deployment）：
...
| 5 | `cargo test -p local-minimal-node --tests` | 双实例并发发消息，seq 单调且无空洞 |
| 6 | `cargo test -p local-minimal-node --tests` | outbox 投递幂等、重试、失败标记 |
