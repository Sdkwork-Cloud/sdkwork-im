...
新增 `services/sdkwork-im-gateway/src/node/outbox_relay.rs`（阶段 0 与主进程同生命周期，阶段 1 拆为独立 deployment）：
...
| 5 | `cargo test -p sdkwork-im-gateway --tests` | 双实例并发发消息，seq 单调且无空洞 |
| 6 | `cargo test -p sdkwork-im-gateway --tests` | outbox 投递幂等、重试、失败标记 |
