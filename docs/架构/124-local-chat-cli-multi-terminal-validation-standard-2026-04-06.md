...
�?`sdkwork-im` 提供一个可直接连接运行中服务的命令行验证工具，使开发者可以在多个终端窗口中模拟不同用户，验证标准 IM 对话与实时事件链路，而不需要手工拼�?HTTP 请求�?JWT�?
...
1. CLI 必须优先兼容公开服务使用的签�?Bearer Token�?
...
3. �?2xx HTTP 响应必须带出状态码和原始错误体�?

### 3.5 实时监听标准

1. `watch` 必须复用 `/im/v3/api/realtime/ws`�?
2. `watch` 必须自动发�?`subscriptions.sync`�?
...
powershell -ExecutionPolicy Bypass -File bin/chat-cli.ps1 -- `
  ...
  --device-id d_owner `
  --session-id s_owner `
  add-member --conversation-id c_demo --principal-id u_guest --principal-kind user --role member
```
...
2. CLI 不得要求开发者手工构�?JWT payload 和签名�?
