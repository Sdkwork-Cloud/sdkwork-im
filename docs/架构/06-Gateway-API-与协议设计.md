...
- `clientRouteId` 优先从 JWT claim 或可信头提取；仅在设备首次注册时允许显式输入，并在进入命令与投影层前绑定到认证上下文。
...
### 3.13 Client Route 与 Presence API 约定
...
IM 只保留客户端路由、presence 与 realtime 传输语义；设备目录、设备孪生、命令、遥测和 IoT 协议能力归属 sibling `sdkwork-aiot`，通过 `/app/v3/api/iot/*`、`/backend/v3/api/iot/*` 与 `sdkwork-aiot-*` SDK 消费，不在 Craw Chat IM API 中暴露旧 IM 设备目录、同步流或孪生路由。

`clientRouteId` 作为兼容的客户端路由身份字段仍可来自认证上下文或受信请求头；服务端内部应使用 client-route 语义处理路由、presence、realtime ack 和 WebSocket 绑定，不再把它建模为 Craw Chat 自有设备能力。
