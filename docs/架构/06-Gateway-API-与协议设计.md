...
- `143-统一协议总纲与分层设�?2026-04-06`
- `144-CCP传输绑定与握手协商设�?2026-04-06`
- `145-CCP数据协议与版本兼容安全设�?2026-04-06`

其中�?

- `HTTP / WebSocket / SSE / MQTT` 是标准传�?
...
- 内部 RPC 可使�?`gRPC` 或其他服务间协议，但不属于客户端协议族的一部分
- 所有外部请求的 `tenant / actor / sender` 都从认证上下文推�?
- 统一 `CCP envelope` 与错误模�?
- 协议版本�?schema 版本双轨治理
- 支持离散消息与流式消�?
...
- 当前代码已落地的最小闭环包括：会话创建与摘要查询、消息发送与时间线查询、媒体上传创�?完成/查询/绑定、流开�?检查点/完成、RTC 创建/邀�?接受/拒绝/结束�?
...
- `POST /im/v3/api/chat/conversations` 当前只接受以�?`conversationType`�?
  - `group`
  - `direct`
...
- `POST /im/v3/api/chat/conversations/{id}/members/transfer-owner` 用于 group owner 向另一�?active member 交接 owner 角色，请求体只接�?`memberId`�?
- 当前最小实现中�?
  - �?`group` 支持 transfer-owner
  - 仅当�?active `owner` 可以发起
  - 目标必须是另一�?active member
  - 交接后旧 owner 降为 `admin`，新 owner 成为唯一 `owner`
- owner transfer 统一写入 `conversation.owner_transferred`，并作为�?owner 后续 `leave` 的前置治理动作�?
- `POST /im/v3/api/chat/conversations/{id}/members/change-role` 用于 group owner 对另一�?active non-owner member 执行通用角色治理，请求体只接�?`memberId` 与目�?`role`�?
- 当前最小实现中�?
  - �?`group` 支持 `change-role`
  - 仅当�?active `owner` 可以发起
  - 目标必须是当�?active non-owner member
  - 目标 `role` 只允许变更为 `admin / member / guest`
  - 涉及 `owner` 的角色变化必须使�?`transfer-owner`
  - stale `memberId` 会被拒绝，不能命中历�?membership episode
- 通用角色治理统一写入 `conversation.member_role_changed`，返�?`previousMember / updatedMember / changedAt`，并同步更新读模型中的成员快照�?
- `POST /im/v3/api/chat/conversations/{id}/members/leave` 用于当前认证主体主动离开当前会话，请求体可为空，不接�?`tenantId`、`principalId`、`memberId`�?
- 当前最小实现中�?
  - `group` �?`admin / member / guest` active member 可以 leave
  - `group owner` 必须先完�?owner transfer，之后按�?owner 规则 leave
...
- `left` 后若�?owner/admin 再次通过 `members/add` 重新加入，服务端必须创建新的 membership episode�?
  - 历史 `left / removed` 成员记录保留，不覆盖
  - �?episode 生成新的 `memberId`
  - 当前 active member 关系总是指向最�?episode
...
- 当前最小实现中，`POST /im/v3/api/chat/conversations/{id}/messages` 支持 `text` �?`parts` 混合提交；`parts` 可包�?`text/data/media/signal/stream_ref`�?
- 已读推进统一�?`POST /im/v3/api/chat/conversations/{id}/read-cursor`，避免把 read/ack 状态混入消息发送命令�?
...
- 当前最小实现中，当 RTC 会话已绑�?`conversationId` 时，`invite/accept/reject/end` 会额外提交一�?`messageType=signal` 的消息，消息体包�?`SignalPart`，并进入时间线与摘要投影�?
...
- 当前最小实现中，自动化请求会同步走�?`automation.execution_requested -> automation.execution_completed` 最小链路，并为调用者生成一�?`automation.result` 站内通知�?
...
- 当前最小实现中，消息提交、通知请求、自动化执行都会留下审计锚点；运维面暴露单节�?`self-hosted.split-services.development` �?cluster / lag / diagnostic 视图�?

### 3.10 认证上下文约�?

- 外部 API 请求体不得显式提�?`tenantId`、`creatorId`、`senderId`、`initiatorId`�?
- `tenant`、`actor`、`session` 必须从已校验�?JWT / session / trusted identity context 中提取�?
- `clientRouteId` 优先�?JWT claim 或可信头提取；仅在设备首次注册时允许显式输入，并在进入命令与投影层前绑定到认证上下文�?
- gateway 负责把认证上下文转换成内部命令上下文�?envelope 字段�?
- 对外 app-facing 入口默认必须要求 `Authorization: Bearer ...`；`trusted identity headers` 仅允许用于内部可信链路、测试装配或显式声明�?internal profile�?
- 因此 `tenant_id` 仍然会出现在命令 envelope、事�?envelope、持久化对象与审计日志中，但这些字段属于服务端权威写入字段，不属于客户端可覆盖字段�?
- 消息载荷中的发送者统一使用 `sender` 对象，而不�?`senderId` + `senderKind` 平铺字段�?

### 3.11 已读游标 API 约定

- `GET /im/v3/api/chat/conversations/{id}/read-cursor`
- `POST /im/v3/api/chat/conversations/{id}/read-cursor`
- 租户、操作者、成员身份都必须由认证上下文和服务端成员关系推导，请求体不接�?`tenantId`、`principalId`、`memberId`�?
- `POST` 请求体建议：
...
- 当成员经�?`left -> rejoin` 形成新的 membership episode 时：
  - `GET /read-cursor` 只返回当�?active episode 对应�?`memberId`
  - �?episode 的初始游标必须从 `readSeq = 0` 开�?
  - 历史 episode 的游标仅作为历史数据保留，不能复用到�?episode
...
- 最小实现阶�?inbox 以查询视图方式构建，不承担精确分页、标签过滤与置顶规则；这些能力后续在不破坏基础结构的前提下追加�?
...
IM 只保留客户端路由、presence �?realtime 传输语义；设备目录、设备孪生、命令、遥测和 IoT 协议能力归属 sibling `sdkwork-aiot`，通过 `/app/v3/api/iot/*`、`/backend/v3/api/iot/*` �?`sdkwork-aiot-*` SDK 消费，不�?Sdkwork IM IM API 中暴露旧 IM 设备目录、同步流或孪生路由�?
