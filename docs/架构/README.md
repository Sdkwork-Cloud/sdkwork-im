...
14. [02-架构标准与总体设计](./02-架构标准与总体设计.md)
15. [03-模块规划与边界](./03-模块规划与边界.md)
16. [04-技术选型与可插拔策略](./04-技术选型与可插拔策略.md)
17. [05-数据模型与数据库设计](./05-数据模型与数据库设计.md)
18. [06-Gateway-API-与协议设计](./06-Gateway-API-与协议设计.md)
19. [07-缓存-流式通信-RTC-通知设计](./07-缓存-流式通信-RTC-通知设计.md)
20. [08-安全-多租户-SaaS-私有化-部署设计](./08-安全-多租户-SaaS-私有化-部署设计.md)
21. [09-实施计划](./09-实施计划.md)

## 二、优先级规则

为避免架构口径冲突，本文档集采用以下优先级：

- `130-142` 为当前阶段的主基线文档，优先级最高。
- `01-09` 为基础说明文档，必须与 `130-142` 保持一致。
- `11-24` 与 `26-129` 为专项标准文档，只在各自主题范围内细化规则，不能推翻总纲。
- 若专项标准与总纲出现冲突，以 `130-142` 为准，并回收修正文档。
