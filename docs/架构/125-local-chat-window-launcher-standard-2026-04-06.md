...
- 认证继续使用签名 Bearer Token 或显�?Bearer Token
...
本地脚本�?CLI 在未显式传入 `baseUrl` 时，必须优先读取�?

- `.runtime/self-hosted.split-services.development/config/self-hosted.split-services.development.env`
- 键：`sdkwork_im_BIND_ADDR`

并将其转换为本地访问地址�?

- `0.0.0.0`、`::`、`[::]` 统一映射�?`127.0.0.1`

避免脚本默认端口与当前本地配置不一致�?

### 4. 发送端标签标准

由于当前公开实时事件�?timeline 投影中不包含完整 sender 结构，聊天验证脚本默认通过 `messagePrefix` 将发送端标签拼接到消息摘要中，例如：
...
- 本地配置端口自动解析
- Windows 自动开窗验证闭�?
- Linux/macOS 终端探测型最佳努力脚�?

## 后续增强

- 在实时事件与 timeline 中补�?sender 结构
- �?Linux/macOS 补真实环境下的脚本回归验�?
