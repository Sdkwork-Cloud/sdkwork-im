...
在不引入额外后端测试接口的前提下，为 `craw-chat` 提供可直接落地的本地聊天验证闭环：
...
本地脚本和 CLI 在未显式传入 `baseUrl` 时，必须优先读取：

- `.runtime/local-minimal/config/local-minimal.env`
- 键：`CRAW_CHAT_BIND_ADDR`

并将其转换为本地访问地址：

- `0.0.0.0`、`::`、`[::]` 统一映射到 `127.0.0.1`
...
- 在实时事件与 timeline 中补齐 sender 结构
- 为 Linux/macOS 补真实环境下的脚本回归验证
