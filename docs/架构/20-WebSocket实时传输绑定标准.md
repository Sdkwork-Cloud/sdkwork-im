...
- `subscriptions.sync` 对应 `POST /api/v1/realtime/subscriptions/sync`
- `events.pull` 对应 `GET /api/v1/realtime/events`
- `events.ack` 对应 `POST /api/v1/realtime/events/ack`
...
- `session-gateway` 暴露 `/api/v1/realtime/ws`
- `local-minimal-node` 暴露同一路径，并复用同一 `RealtimeDeliveryRuntime`
- WebSocket `ack` 与 HTTP `GET /api/v1/realtime/events` 可交叉验证同一窗口状态
