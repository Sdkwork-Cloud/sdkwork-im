...
- 从 route migration / state handoff 恢复设备状态
...
`ensure_device_state(...)` 从 checkpoint store 读取到记录后，必须先做不变量收口：
...
`restore_device_state(snapshot)` 在写入 runtime 内存与 checkpoint store 前，必须先归一化：
