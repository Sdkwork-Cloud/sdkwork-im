...
1. `session.resume`
   - 判断当前设备是否需要补偿
   - 返回 `resumeRequired / resumeFromSyncSeq / latestSyncSeq`
2. `sync-feed`
   - 拉取 durable 补偿事件，保证不会漏消息

实时事件窗口只负责“在线期间的低延迟增量下行”，不替代 `sync-feed`。

### 2.3 先做设备级内核，再扩到群体 fanout

当前最小落地版本先聚焦“同一主体多设备”的实时同步，这和现有 `sync-feed`、`session.resume`、设备注册机制天然对齐。
...
- 设备级实时事件排队
- 有序事件窗口查询
- 与 `session.resume`、`sync-feed` 的边界协同
...
- 服务端以当前请求体替换该设备的订阅集
- `deviceId` 默认从鉴权上下文解析，只有未绑定设备时才允许请求体传入
...
## 6. 与 session.resume / sync-feed 的协同

标准恢复顺序：

1. 连接建立或应用恢复
2. 调用 `POST /api/v1/sessions/resume`
3. 若 `resumeRequired == true`，先拉 `GET /api/v1/devices/{id}/sync-feed`
4. 再开始消费实时事件窗口或实时推送通道

原因很明确：

- `sync-feed` 覆盖 durable 补偿
...
- 本地最小版本只保证“当前主体多设备”的实时一致性，不承担跨主体会话 ACL 扩散
