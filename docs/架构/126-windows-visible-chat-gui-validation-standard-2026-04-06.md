# 126 Windows 可见聊天验证窗体标准�?026-04-06�?

## 背景

在当�?Windows 自动化宿主环境中，直接启�?`cmd.exe` �?`powershell.exe` 形式的控制台聊天窗口，会生成无窗口句柄的 headless 进程�?
...
`bin/open-chat-test.ps1` �?Windows 上默认打开 GUI 聊天窗：

- 每个用户一个独�?WinForms 窗口
- 底层仍然复用 `chat-session`
- 通过重定�?stdin/stdout �?CLI 子进程通信
...
- 宿主进程隐藏
- `sdkwork-im-server.exe` 在宿主内以前台方式运�?
- 用健康检查轮询确认服务就�?
