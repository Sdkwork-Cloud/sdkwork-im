...
在当前实现中，公网入口已经统一收敛到 `build_public_app()` 或等价的 public builder。此前 public 路由允许“长得像 JWT 的 Bearer 字符串”直接进入后续授权逻辑，这会导致 `tenant_id`、`sub`、`permissions` 等声明可以被客户端伪造，不符合商用 IM 平台的最基本零信任边界。

本标准定义：所有公网上行 Bearer 必须先完成签名校验，再进入授权和业务处理。未签名、错误算法、错误签名、缺少公钥/密钥配置的请求，必须在认证层被拒绝。
...
- 当前标准环境变量：`CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`
...
只有在 Bearer 已完成签名校验并解析出合法身份后，才允许继续做权限判断，例如 `permission_denied`、跨主体可见性校验、资源所有权隔离等。

### 3.5 测试标准

- public auth 测试不得继续使用 `alg=none` 伪 JWT 作为“正常 Bearer”夹具。
- 测试必须显式注入 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`，并使用签名 helper 生成 Bearer。
...
- `bin/init-config-local.ps1`、`bin/init-config-local.sh` 负责生成并写入 `CRAW_CHAT_PUBLIC_BEARER_HS256_SECRET`。
- `bin/start-local.ps1`、`bin/start-local.sh` 负责读取并导出该密钥；缺失时启动失败。
- 各服务 `public_auth_test.rs` 与 `local-minimal-node` 的公网 e2e 测试已切换为签名 Bearer helper。

## 5. 禁止事项

- 禁止在 public builder 中继续使用“只解码不验签”的 Bearer 解析逻辑。
- 禁止把 trusted headers 作为公网入口兜底身份。
- 禁止在文档、脚本或测试中默认示范 `alg=none` Bearer 作为合法公网访问方式。
