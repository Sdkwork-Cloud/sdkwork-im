...
本轮 review 在 `rtc-signaling-service` 与 `local-minimal-node` 的 RTC 写路径中确认了一个状态机漏洞：

- RTC 会话进入 `accepted` 后，`invite` 仍然允许携带新的 `signalingStreamId` 改写现有会话。
- 聚合入口 `local-minimal-node` 会把这次改写视为一次新的有效变更，再次写入 IM `rtc.invite` 信令消息。
...
`POST /im/v3/api/rtc/sessions/{rtc_session_id}/invite` 必须遵循以下规则：
...
`local-minimal-node` 必须继续只在 `outcome.applied = true` 时发出 IM `rtc.invite`。
