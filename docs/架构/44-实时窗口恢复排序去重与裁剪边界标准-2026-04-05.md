...
`restore_device_state(snapshot)` 用于 route migration、runtime handoff、节点切换后的设备状态恢复。  
...
`restore_device_state(snapshot)` 在把 `snapshot.events` 写入 runtime 内存前，必须执行以下归一化：
...
- `session-gateway` 的 `RealtimeDeliveryRuntime::restore_device_state(...)` 必须执行窗口归一化
- 回归测试至少覆盖：
  - 乱序 handoff 后分页仍然单调
