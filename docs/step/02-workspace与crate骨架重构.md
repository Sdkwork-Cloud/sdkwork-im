...
本 step 用于把当前“服务偏重、crate 分层不足、超大文件承载过多职责”的工程形态，重构为符合目标架构的 workspace 骨架。

本 step 主要解决：

- 让 `contract / domain / app / interface / runtime / service / adapter` 分层具备真实目录承载
- 让 `services/*` 回归装配层，而不是继续承担业务大杂烩
- 让超大 `lib.rs` 被目录化拆分
- 为 `CCP`、Link Plane、Route Plane、AI / IoT 扩展等后续 step 预留明确落点
...
- 新增 `craw-chat-*` crate 家族承载目标分层
- 现有 `im-*` crate 在过渡期保留为兼容 facade 或迁移中间层

推荐骨架类别：

- 基础横切层：`craw-chat-kernel`、`craw-chat-config`、`craw-chat-observability`、`craw-chat-auth`、`craw-chat-policy`
- 契约与协议层：`craw-chat-contract-*`、`craw-chat-ccp-*`
- 领域层：`craw-chat-domain-*`
- 应用层：`craw-chat-app-*`
- 接口层：`craw-chat-interface-*`
- 运行时层：`craw-chat-runtime-*`
- 存储与适配层：`craw-chat-storage-*`、`craw-chat-adapter-*`
- 服务装配层：保留 `services/*`
...
- `domain-*` 不依赖 `axum`
- `domain-*` 不依赖具体数据库驱动
- `interface-*` 不直接依赖具体存储实现
- `runtime-*` 负责运行态，不承载稳定契约定义
...
1. 调整根级 `Cargo.toml`，引入目标 crate 目录规划
2. 新建空的 `craw-chat-*` crate 骨架和模块模板
3. 为高风险服务建立目录化模块结构
4. 先把旧 `lib.rs` 中最易拆的内容下沉到子模块
5. 建立文件长度检查和模块边界检查的脚本或约定
...
- `im-*` 旧 crate 提供过渡导出
- `services/*` 同时装配旧实现和新实现
- 新旧模块通过 feature、facade 或 adapter 渐进切换
...
- `02-A`：workspace、crate 拓扑与 Cargo 依赖面重构
- `02-B`：`services/*` 装配层收敛与目录边界整理
- `02-C`：超大 `lib.rs` 拆分、编译入口与测试入口保活
- 收口要求：共享目录树和 crate 命名规则由 `02-Owner` 统一拍板，任何车道不得绕过既定边界继续向旧 `lib.rs` 堆积逻辑。
- 车道编排参考：[`94-Step并行执行编排与车道拆分建议`](./94-Step并行执行编排与车道拆分建议.md)

### 8.3 架构能力闭环判定

- 新骨架必须能承接 Step 03 之后的协议、运行时、主链路改造，而不是只创建空目录。
- 如果高风险大文件仍继续增长，或新代码仍回流到旧 `lib.rs`，则本 step 未闭环。
- 闭环验收以 [`95-架构能力闭环验收标准`](./95-架构能力闭环验收标准.md) 中 Step 02 条目为准。

### 8.4 快速并行执行建议

- 先冻结目标 workspace 目录树、crate 命名和主写入边界，再放开拆分车道。
...
- 必备证据：`docs/review/step-02-架构兑现-YYYY-MM-DD.md` 与 `docs/review/step-02-架构回写决议-YYYY-MM-DD.md`。

## 9. 风险与回滚

### 9.1 风险

- 过早大规模迁移逻辑，可能导致编译面过大、问题定位困难
...
- 旧实现保留 facade，必要时可临时回切
- 每次拆分应按模块逐段提交，避免大爆炸式不可回滚变更

## 10. 完成定义

以下条件全部满足时，本 step 完成：

- 根级 workspace 已引入目标 crate 分层骨架
- 关键服务的大文件已开始收敛
- 新旧结构的迁移路径清晰
- 基本编译与 smoke 验证未被破坏
