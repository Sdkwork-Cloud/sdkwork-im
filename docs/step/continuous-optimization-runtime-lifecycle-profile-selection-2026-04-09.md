...
- 让 `init/install/start/stop/restart` 与现有 runtime ops 一样，稳定支持 `self-hosted.split-services.development / self-hosted.split-services.development` profile。
...
- 先写失败测试，冻结 `self-hosted.split-services.development` config 入口与 restart 参数透传合同。
- 复用 `_runtime-profile-common.*` 解析 profile config 链与 runtime-dir。
- 对 `self-hosted.split-services.development` 保持“独立 config 文件 + 共享 self-hosted.split-services.development runtime-dir”的当前兼容策略。
...
- 这轮不引入 `self-hosted.split-services.development` 独立拓扑。
