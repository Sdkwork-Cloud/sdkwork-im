...
- 当前服务包括 `conversation-runtime`、`session-gateway`、`streaming-service`、`rtc-signaling-service`、`projection-service`、`control-plane-api` 等
- 本地最小运行集成体为 `services/local-minimal-node`
- 已经存在跨平台 `bin/` 脚本以及 `tools/chat-cli`

当前最突出的结构性问题：

- `services/local-minimal-node/src/lib.rs` 约 `5609` 行
...
- 关键服务 smoke 测试清点
- 高风险文件行数统计脚本
- `bin/start-local.*`、`bin/status-local.*`、`tools/chat-cli` 存在性检查
