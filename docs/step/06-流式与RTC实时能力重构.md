# Step 06 - 流式�?RTC 实时能力重构

## 1. 目标与范�?

�?step 用于�?`stream` �?`rtc` 从“附属功能”升级为 `sdkwork-im` 的一等实时能力，并纳入统一协议、统一运行时和统一投影体系�?
...
- 不在�?step 内完�?AI / Agent 治理�?device 接入治理
- 不在�?step 内引入媒体面处理或复杂音视频基础设施
- 不在�?step 内完�?SDK �?CLI 的兼容收�?
...
- 与消息主链路的桥接规�?
- RTC provider plugin 契约、默�?provider 规则和录制产物落盘边�?
- 可重复执行的 stream / rtc 自动化验�?
...
- `docs/架构/143-统一协议总纲与分层设�?2026-04-06.md`
- `docs/架构/150-插件化提供商体系与设备接入设�?2026-04-08.md`
...
- RTC provider 还需要统一抽象�?plugin 体系，冻�?`火山引擎 / 阿里�?/ 腾讯云` 的选择边界
...
- 流不是消息字段，而是并列的一等模�?
- 流可以物化为消息、卡片、摘要或任务状�?
- RTC 信令�?conversation-bound 能力，受成员治理约束
- 流与 RTC 都要服从同一�?auth / sender / scope 规则
...
2. 补齐 `RtcProviderPort`、provider registry 和默�?provider 选择语义
3. 拆分 `domain-stream` �?`domain-rtc`
4. 拆分 `app-stream` �?`app-rtc`
5. �?`streaming-service`、`im-call-runtime` 接入新契约和�?runtime
...
- `services/sdkwork-im-gateway/tests/stream_runtime_persistence_test.rs`
- `services/sdkwork-im-gateway/tests/rtc_runtime_persistence_test.rs`
...
- RTC provider 已被限制在统一 plugin 边界内，默认 provider 与覆盖规则清�?
- 普通流、AI 流、设备流可以共用同一骨架
- 断线重连后，流的恢复语义可验�?

## 8. 检查点

- `CP06-1`：统一流模型、RTC 模型�?RTC provider 契约已经落实到契约与领域�?
- `CP06-2`：stream 生命周期�?materialization 规则已可自动化验�?
- `CP06-3`：RTC invite / accept / reject / end �?provider 选择测试跑�?
- `CP06-4`：流与消息、RTC 与消息的桥接规则已稳�?

### 8.1 推荐 review 产物

- `docs/review/step-06-执行�?YYYY-MM-DD.md`
...
- stream �?RTC 必须成为一等实时能力，生命周期、权限、桥接语义都可单独验证�?
- 如果 stream 仍只是消息上的一个字段，�?RTC 仍混入媒体侧内部逻辑而没有标�?signaling 面，�?RTC provider 仍散落在业务代码中，�?step 不算闭环�?
- 闭环验收�?[`95-架构能力闭环验收标准`](./95-架构能力闭环验收标准.md) �?Step 06 条目为准�?

### 8.4 快速并行执行建�?

- 先冻�?stream lifecycle �?RTC signaling 状态，再并行推进流式、RTC、消息桥接三车道�?
...
- 已落地到真实代码的部分：
  ...
  - `services/im-call-runtime/src/lib.rs`
    - `RtcRuntime` 已接�?`ProviderRegistry + RtcProviderPort`
    - 默认 `with_store(...)` 已装�?`rtc-volcengine`
    - `create_session(...)` 已真实调�?provider `create_session(...)`
    - `issue_participant_credential(...)` 已真实调�?provider `issue_participant_credential(...)`
    - `reject_session(...) / end_session(...)` 已真实调�?provider `close_session(...)`
  - `crates/im-domain-core/src/rtc.rs`
...
- 已继续把 Step 06 / Wave B 从“默�?provider runtime adapter 已接入”推进到“外�?RTC provider surface 已闭环”：
  - `services/im-call-runtime/src/lib.rs`
  ...
  - `services/sdkwork-im-gateway/src/node/rtc.rs`
...
- `services/sdkwork-im-gateway`
  ...
  - callback 保持 provider/integration 面，不引入厂�?DTO
  - artifact 继续�?session / conversation 边界约束
- 新增验证
  - `services/im-call-runtime/tests/http_smoke_test.rs`
...
- Step 06 / `06-B`
  - provider external surface 已从 `session + credential + health` 收口�?`callback + artifact`
...
- Step 06 / `06-B`
  - RTC provider external surface �?provider matrix 已形成最小闭�?
  - 下一轮优�?`ObjectStorageProvider` 统一录制产物归档，以�?local profile �?provider registry 注入�?
