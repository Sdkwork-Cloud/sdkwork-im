...
- 本地单机开发环境
- self-hosted.split-services.development / self-hosted.split-services.development profile
- Docker / Compose 启动
- 私有化与多环境配置模板
...
- 当前 self-hosted.split-services.development、self-hosted.split-services.development 的真实启动链路
...
- `self-hosted.split-services.development`：单机闭环验证
- `self-hosted.split-services.development`：更接近真实依赖的本地环境
...
- 新 profile 先以补充方式加入，避免覆盖已稳定的 self-hosted.split-services.development
