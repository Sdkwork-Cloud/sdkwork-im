...
- 本轮已兑现:
  ...
  - fresh regression：`cargo test -p sdkwork-im-gateway --offline --test deployment_profile_test -- --nocapture` = `64 passed`
  - full package regression：`cargo test -p sdkwork-im-gateway --offline --tests -- --nocapture` = `passed`
- 当前仍缺:
