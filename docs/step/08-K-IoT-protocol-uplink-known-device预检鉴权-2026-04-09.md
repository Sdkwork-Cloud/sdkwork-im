...
- `local-minimal-node`
- 当 `/api/v1/iot/protocol/uplink` 的目标 `deviceId` 已知时
- 先做 access preflight
...
- 对 `local-minimal-node` 的 `/api/v1/iot/protocol/uplink` 增加一个最小 preflight 规则：
...
- `/api/v1/iot/protocol/uplink` 在已知 `deviceId` 时已具备：
  - 先鉴权
  - 后 decode
- 注入式 adapter 测试已证明：
  - 未授权请求返回 `403`
