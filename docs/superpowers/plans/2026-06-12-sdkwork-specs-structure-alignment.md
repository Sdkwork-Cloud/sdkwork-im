...
`apps/sdkwork-chat-pc`, preserving pnpm workspace and lockfile authority while avoiding sandboxed
...
- `Cargo.toml` keeps `sdkwork_iam_context_service` as the single root workspace key for
  `../sdkwork-appbase/crates/sdkwork-iam-context-service` and removes the unused old
