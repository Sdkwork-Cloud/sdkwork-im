...
Make `sdkwork-api-gateway` the only unified API ingress for Sdkwork IM HTTP traffic, while preserving `sdkwork-im-server` as the owner of IM realtime transport, TCP/long-connection runtime behavior, and the product static shell/asset hosting boundary.
...
```text
renderer -> sdkwork-im-server:18079 -> internal proxy/router -> product services or sdkwork-api-gateway:3900
```
...
### Option B: Keep `sdkwork-im-server` as an outer proxy for API traffic, but forward to `sdkwork-api-gateway`
...
Remove the extra `sdkwork-im-server -> sdkwork-api-gateway` HTTP proxy hop for normal API traffic.
...
- `pnpm dev:desktop` does not spawn duplicate `sdkwork-api-gateway` instances
