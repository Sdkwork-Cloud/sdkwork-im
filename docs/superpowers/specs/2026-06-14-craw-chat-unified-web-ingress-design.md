# 2026-06-14 Craw Chat Unified API Ingress Design

## 1. Goal

Make `sdkwork-api-gateway` the only unified API ingress for Craw Chat HTTP traffic, while preserving `craw-chat-server` as the owner of IM realtime transport, TCP/long-connection runtime behavior, and the product static shell/asset hosting boundary.

The design fixes the current split where Craw Chat exposes:
...
- `sdkwork-api-gateway` owns API ingress, CORS, request identity, docs, OpenAPI aggregation, and HTTP routing
- `craw-chat-server` owns IM realtime transport and any direct transport protocols that cannot be modeled as ordinary HTTP ingress
...
```text
renderer -> craw-chat-server:18079 -> internal proxy/router -> product services or sdkwork-api-gateway:3900
```
...
`sdkwork-api-gateway` becomes the canonical external HTTP API entrypoint.
...
`craw-chat-server` remains externally reachable for realtime IM transport.
...
### Option A: Split ownership, no API proxy hop in `craw-chat-server` for normal HTTP
...
```text
renderer -> sdkwork-api-gateway -> product services / foundation services
renderer -> craw-chat-server -> realtime transport only
```
...
### Option B: Keep `craw-chat-server` as an outer proxy for API traffic, but forward to `sdkwork-api-gateway`
...
### Option C: Force all API traffic and realtime through `sdkwork-api-gateway`

Not recommended for Craw Chat.
...
browser / desktop renderer
  -> craw-chat-server static shell
  -> sdkwork-api-gateway for API traffic
...
- the summary must not imply that `sdkwork-appbase-app-api`, `sdkwork-drive-app-api`, and `sdkwork-notary-app-api` are separate public ingress endpoints when they all resolve to the same shared gateway base URL
...
Upstream Status
  ...
  Shared Foundation Gateway: http://127.0.0.1:3900 [sdkwork-appbase-app-api, sdkwork-drive-app-api, sdkwork-notary-app-api]
```
...
Move the product HTTP API ingress contract to `sdkwork-api-gateway`.

### Phase 2

Remove the extra `craw-chat-server -> sdkwork-api-gateway` HTTP proxy hop for normal API traffic.

### Phase 3

Keep `craw-chat-server` for static shell delivery, realtime transport, and any transport-specific APIs only.
...
- `pnpm tauri:dev` does not spawn duplicate `sdkwork-api-gateway` instances
...
- realtime transport remains accessible through `craw-chat-server`
- product HTTP API ingress does not depend on a second product-side proxy hop

## 10. Decision

Choose `sdkwork-api-gateway` as the single HTTP API ingress.

Keep `craw-chat-server` for static site delivery, realtime transport, and transport-specific responsibilities.

Do not preserve a second generic HTTP API proxy layer inside `craw-chat-server`.
