...
export function verifySdkSiteDocs(options = {}) {
  ...
  if (!/CrawChatClient/.test(appSource)) {
    failures.push('docs/sites/sdk/app-sdk.md must name the primary TypeScript client as CrawChatClient.');
  }
  if (!/createCrawChatClient/.test(appSource)) {
    failures.push('docs/sites/sdk/app-sdk.md must mention createCrawChatClient for TypeScript consumers.');
  }
  if (!/CrawChatClient\.create\(\{ backendConfig \}\)/.test(appSource)) {
    failures.push('docs/sites/sdk/app-sdk.md must document CrawChatClient.create({ backendConfig }) for TypeScript consumers.');
  }
  ...
  if (!/composed smoke test for `CrawChatClient`/.test(appSource)) {
    failures.push('docs/sites/sdk/app-sdk.md must document the CrawChatClient smoke test in the verification path.');
  }
  if (!/CrawChatSdkAdminClient\.create\(\{ backendConfig \}\)/.test(adminSource)) {
    failures.push('docs/sites/sdk/admin-sdk.md must document CrawChatSdkAdminClient.create({ backendConfig }) for TypeScript consumers.');
  }
  if (!/createCrawChatSdkAdminClient/.test(adminSource)) {
    failures.push('docs/sites/sdk/admin-sdk.md must mention createCrawChatSdkAdminClient for TypeScript consumers.');
  }
  ...
  if (!/CrawChatAdminClient/.test(adminSource)) {
    failures.push('docs/sites/sdk/admin-sdk.md must mention the primary Flutter admin client as CrawChatAdminClient.');
  }
  ...
  if (!/admin TypeScript uses generated `@sdkwork\/craw-chat-admin-backend-sdk` and composed `@sdkwork\/craw-chat-sdk-admin`/.test(languageSupportSource)) {
  ...
  if (!/admin Flutter uses generated `craw_chat_admin_backend_sdk` and composed `craw_chat_sdk_admin`/.test(languageSupportSource)) {
