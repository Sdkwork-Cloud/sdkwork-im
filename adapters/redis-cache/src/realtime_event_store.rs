...
impl sdkwork_im_contract_control::RealtimeEventWindowStore for RedisRealtimeEventWindowStore {
    fn load_window(
        ...
        let key = window_key(tenant_id, principal_kind, principal_id, device_id);
    ...
    fn trim_window(
        ...
        let _ = (tenant_id, principal_kind, principal_id, device_id);
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_window_key_is_segment_safe() {
        let k1 = window_key("tenant:a", "user", "b", "d1");
        let k2 = window_key("tenant", "user", "a:b", "d1");
        assert_ne!(k1, k2, "segment-safe keys must not collide");
    }

    #[test]
    fn test_window_key_contains_all_identity_segments() {
        let key = window_key("t1", "user", "u1", "d1");
