...
where presence_status = 'online'
  ...
  and last_seen_at <= $1
order by last_seen_at asc, tenant_id asc, principal_kind asc, principal_id asc, device_id asc
limit $2
"#;
...
impl PresenceStateStore for PostgresRealtimePresenceStateStore {
    ...
    fn expire_online_state_if_seen_at_or_before(
        ...
        let Some(current) = self.load_state(tenant_id, principal_kind, principal_id, device_id)?
        else {
            return Ok(None);
        };
