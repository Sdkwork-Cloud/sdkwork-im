...
use im_domain_core::rtc::{RtcSession, RtcSessionState, encode_im_call_key_segments};

#[derive(Default)]
pub(super) struct ImCallSessionRuntimeStore {
...
impl ImCallSessionRuntimeStore {
    pub(super) fn has_session(&self, tenant_id: &str, rtc_session_id: &str) -> bool {
        self.sessions_by_id
            .contains_key(rtc_scope_key(tenant_id, rtc_session_id).as_str())
    }

    pub(super) fn session(&self, tenant_id: &str, rtc_session_id: &str) -> Option<RtcSession> {
        self.sessions_by_id
            .get(rtc_scope_key(tenant_id, rtc_session_id).as_str())
            .cloned()
    }

    pub(super) fn insert_session(&mut self, session: RtcSession) -> Option<RtcSession> {
    ...
    #[cfg(test)]
    fn sessions_for_conversation(&self, tenant_id: &str, conversation_id: &str) -> Vec<RtcSession> {
    ...
    #[cfg(test)]
    fn sessions_for_state(&self, tenant_id: &str, state: &RtcSessionState) -> Vec<RtcSession> {
...
fn conversation_key(tenant_id: &str, conversation_id: &str) -> String {
    encode_im_call_key_segments([tenant_id, conversation_id])
}

fn state_key(tenant_id: &str, state: &RtcSessionState) -> String {
    encode_im_call_key_segments([tenant_id, state.as_wire_value()])
}
...
mod tests {
    ...
    fn test_session_scope_key_is_segment_safe() {
        let mut store = ImCallSessionRuntimeStore::default();
    ...
    #[test]
    fn test_state_index_updates_when_session_state_changes() {
        let mut store = ImCallSessionRuntimeStore::default();
        store.insert_session(session("tenant", "rtc-1", None));

        assert_eq!(
            store.sessions_for_state("tenant", &RtcSessionState::Started)[0].rtc_session_id,
            "rtc-1"
        );

        store.update_session("tenant", "rtc-1", |session| {
            session.state = RtcSessionState::Accepted;
        });

        assert!(
            store
                .sessions_for_state("tenant", &RtcSessionState::Started)
                .is_empty(),
            "old session state index entry should be removed after update"
        );
        assert_eq!(
            store.sessions_for_state("tenant", &RtcSessionState::Accepted)[0].rtc_session_id,
            "rtc-1"
        );
    }
}
