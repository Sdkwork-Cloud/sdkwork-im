...
fn unique_runtime_dir() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "craw_chat_user_module_runtime_selection_{unique}_{counter}"
    ))
}
...
#[tokio::test]
async fn test_default_app_uses_configured_external_user_module_provider() {
    let _guard = user_module_env_guard().await;
    let runtime_dir = unique_runtime_dir();
    fs::create_dir_all(&runtime_dir).expect("runtime dir should be created");

    let external_catalog_path = runtime_dir.join("external-user-module.json");
    fs::write(
        ...
        r#"{
            ...
            "users":[
                {
                    "tenantId":"t_demo",
                    "userId":"u_runtime_external_owner",
                ...
                {
                    "tenantId":"t_demo",
                    "userId":"u_runtime_external_member",
...
#[tokio::test]
async fn test_default_app_boots_with_external_user_module_provider_missing_catalog_path_and_returns_provider_unavailable()
 {
    let _guard = user_module_env_guard().await;
    let runtime_dir = unique_runtime_dir();
    fs::create_dir_all(&runtime_dir).expect("runtime dir should be created");
    ...
    let app = build_result.expect(
        "missing external user-module catalog path should boot with an unavailable provider instead of panicking",
    );
