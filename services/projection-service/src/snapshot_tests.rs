...
fn test_persist_conversation_snapshot_batches_projection_store_flushes() {
    ...
    assert!(
        projection
            .persist_conversation_snapshot("t_demo", "default", "c_batch", &metadata_store, &timeline_store,)
...
fn test_restore_conversation_snapshot_recovers_from_poisoned_summary_store_lock() {
    ...
    let result = panic::catch_unwind(AssertUnwindSafe(|| {
        projection.restore_conversation_snapshot("t_demo", "default", "c_demo", &metadata_store,
            &timeline_store,
        )
    }));
    assert!(
