...
fn test_ack_events_trims_window_and_tracks_checkpoint() {
    ...
    let ack =
        expect_ok(runtime.ack_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 1));
    ...
    let window = expect_ok(
        runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 10),
    );
...
fn test_ack_events_is_monotonic_and_clamped_to_latest_sequence() {
    ...
    let first_ack =
        expect_ok(runtime.ack_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 99));
    assert_eq!(first_ack.acked_through_seq, 1);
    assert_eq!(first_ack.trimmed_through_seq, 1);

    let second_ack =
        expect_ok(runtime.ack_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0));
...
fn test_failed_ack_checkpoint_persistence_does_not_commit_runtime_ack_state() {
    ...
    let error = runtime
        .ack_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 1)
...
fn test_runtime_persists_checkpoint_and_recovers_on_rebuild() {
    ...
    let persisted = checkpoint_store
        .checkpoint("t_demo", "default", "user", "u_demo", "d_pad")
    ...
    let restored = expect_ok(
        rebuilt_runtime.window_checkpoint_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );
...
fn test_runtime_restores_persisted_subscriptions_on_rebuild_without_resync() {
    ...
    expect_ok(
        rebuilt_runtime
            .ensure_client_route_state_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );
...
fn test_runtime_restores_unacked_events_from_durable_window_store_after_rebuild() {
    ...
    let restored = expect_ok(
        rebuilt_runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 10),
    );
    ...
    expect_ok(
        rebuilt_runtime.ack_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 1),
    );
    let persisted_after_ack = event_window_store
        .window("t_demo", "default", "user", "u_demo", "d_pad")
...
fn test_publish_does_not_restore_unmatched_registered_client_routes() {
    ...
    let window = expect_ok(
        rebuilt_runtime
            .list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_match", 0, 10),
    );
    assert_eq!(window.items.len(), 1);
    assert_eq!(
        window.items[0].payload,
...
fn test_failed_subscription_clear_preserves_runtime_subscription() {
    ...
    let error = runtime
        .clear_client_route_subscriptions_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad")
...
fn test_failed_checkpoint_persistence_rolls_back_published_runtime_event() {
    ...
    let checkpoint = expect_ok(
        runtime.window_checkpoint_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );
...
fn test_failed_multi_client_route_checkpoint_persistence_does_not_partially_commit_any_route() {
    ...
    for device_id in ["d_pad", "d_phone"] {
        let window = expect_ok(
            runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", device_id, 0, 10),
        );
        assert_eq!(window.items.len(), 0);
    }
}
...
fn test_sync_subscriptions_rejects_too_many_subscription_items() {
    ...
    let error = runtime
        .sync_subscriptions_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", oversized_items)
...
fn test_runtime_isolates_same_actor_id_across_principal_kinds() {
    ...
    let agent_window = expect_ok(
        runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "agent", "d_pad", 0, 10),
    );
...
fn test_failed_checkpoint_normalization_restore_retries_durable_repair() {
    ...
    let error = runtime
        .window_checkpoint_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad")
...
fn test_restore_client_route_state_checkpoint_failure_does_not_install_runtime_state() {
    ...
    assert!(
        subscription_store
            .subscriptions("t_demo", "default", "user", "u_demo", "d_pad")
            .is_none(),
...
fn test_restore_client_route_state_normalizes_event_order_for_monotonic_pagination() {
    ...
    let first_page = expect_ok(
        runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 2),
    );
    assert_eq!(
        first_page
            .items
    ...
    let second_page = expect_ok(
        runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 2, 2),
    );
    assert_eq!(
        second_page
            .items
...
fn test_list_events_rejects_zero_limit_at_runtime_boundary() {
    let runtime = RealtimeDeliveryRuntime::permissive_for_tests();
    expect_ok(
        runtime.ensure_client_route_state_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );

    let error = runtime
        .list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 0)
...
fn test_take_restore_client_route_state_transfers_disconnect_generation() {
    ...
    expect_ok(
        source_runtime
            .signal_device_disconnect_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );
    expect_ok(
        source_runtime
            .signal_device_disconnect_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    ...
    assert_eq!(
        expect_ok(
            source_runtime
                .disconnect_generation_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad")
        ),
        2
    );
    ...
    let snapshot = expect_ok(
        source_runtime
            .take_client_route_state_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad"),
    );
    assert_eq!(
        expect_ok(
            source_runtime
...
fn test_clearing_client_route_subscriptions_stops_future_realtime_delivery() {
    ...
    expect_ok(
        runtime.clear_client_route_subscriptions_for_principal_kind(
            "t_demo", "default", "u_demo", "user", "d_pad",
        ),
    );
...
fn test_restored_client_route_state_rebuilds_scope_fanout_index() {
    ...
    assert_eq!(delivered, 1);
    let window = expect_ok(
        target_runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 10),
    );
    assert_eq!(window.items.len(), 1);
...
fn test_take_client_route_state_clears_source_durable_subscriptions_before_lazy_restore() {
    ...
    assert!(
        subscription_store
            .load_subscriptions("t_demo", "default", "user", "u_demo", "d_pad")
...
fn test_publish_scope_event_enforces_bounded_client_route_window_and_persists_trim_checkpoint() {
    ...
    let window = expect_ok(
        runtime.list_events_for_principal_kind("t_demo", "default", "u_demo", "user", "d_pad", 0, 1000),
    );
