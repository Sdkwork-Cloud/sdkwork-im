...
pub(crate) struct DeviceSyncStateSnapshot {
...
impl DeviceSyncState {
    pub(crate) fn ensure_device_kind_available(
        ...
        if let Some(existing_owner_scope) =
            lock_device_sync_mutex(&self.device_owner_scopes, "registered device owner store")
    ...
    pub(crate) fn register_device(&self, auth: &AppContext, device_id: &str) {
        lock_device_sync_mutex(&self.registered_devices, "registered device store")
        ...
        lock_device_sync_mutex(&self.latest_sync_sequences, "latest sync sequence store")
    ...
    pub(crate) fn has_registered_device(&self, auth: &AppContext, device_id: &str) -> bool {
        lock_device_sync_mutex(&self.registered_devices, "registered device store")
            .get(
                typed_principal_scope_key(
                    auth.tenant_id.as_str(),
...
fn lock_device_sync_mutex<'a, T>(
    ...
    match mutex.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            tracing::warn!("recovered poisoned session-sync mutex lock={lock_name}");
            poisoned.into_inner()
        }
    }
}
...
mod tests {
    ...
    fn test_device_sync_state_isolated_by_actor_kind_for_same_actor_id() {
        ...
        let user_state = state
            .device_sync_state_snapshot(&user_auth, Some("d_user"))
            .expect("user sync state should resolve");
        let agent_state = state
            .device_sync_state_snapshot(&agent_auth, Some("d_agent"))
