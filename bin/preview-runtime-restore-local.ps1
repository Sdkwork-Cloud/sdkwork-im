...
if ($Help) {
    Write-Host "Usage: powershell -ExecutionPolicy Bypass -File bin/preview-runtime-restore-local.ps1 -BackupDir <path> [-ProfileName <local-minimal|local-default>] [-RuntimeDir <path>] [-Json] [-Release]"
    Write-Host "Preview managed local runtime-dir restore actions for the selected local-minimal/local-default profile from an explicit backup snapshot through the local-minimal-node preview entrypoint."
    exit 0
}
