...
show_help() {
  cat <<'EOF'
Usage: bash bin/prune-runtime-archives-local.sh [--profile <local-minimal|local-default>] [--runtime-dir <path>] [--json] [--release]

Prune archived local runtime-dir backup snapshots for the selected local-minimal/local-default profile whose retention window has elapsed and are not protected by legal hold.
EOF
}
...
prune_args=(prune-archived-runtime-backups --runtime-dir "$runtime_dir")
if [[ "$json_output" -eq 1 ]]; then
  prune_args+=(--json)
fi

if binary_path="$(resolve_binary_path "$ROOT_DIR" "$prefer_release")"; then
  exec "$binary_path" "${prune_args[@]}"
fi

if command -v cargo >/dev/null 2>&1; then
  exec cargo run -p local-minimal-node --offline -- "${prune_args[@]}"
fi
