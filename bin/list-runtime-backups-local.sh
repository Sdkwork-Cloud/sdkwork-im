...
show_help() {
  cat <<'EOF'
Usage: bash bin/list-runtime-backups-local.sh [--runtime-dir <path>] [--json] [--release]

List managed local-minimal runtime-dir backup snapshots with readiness preview through the local-minimal-node catalog entrypoint.
EOF
}
