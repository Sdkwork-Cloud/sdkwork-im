...
if ($Help) {
    Write-Host "Usage: powershell -ExecutionPolicy Bypass -File bin/prune-runtime-archives-local.ps1 [-ProfileName <local-minimal|local-default>] [-RuntimeDir <path>] [-Json] [-Release]"
    Write-Host "Prune archived local runtime-dir backup snapshots for the selected local-minimal/local-default profile whose retention window has elapsed and are not protected by legal hold."
    exit 0
}
...
$pruneArgs = @("prune-archived-runtime-backups", "--runtime-dir", $resolvedRuntimeDir)
if ($Json) {
    $pruneArgs += "--json"
}

if ($null -ne $binaryPath) {
    & $binaryPath @pruneArgs
    exit $LASTEXITCODE
}
