...
show_help() {
  cat <<'EOF'
Usage: bash bin/preview-runtime-restore-local.sh --backup-dir <path> [--profile <local-minimal|local-default>] [--runtime-dir <path>] [--json] [--release]

Preview managed local runtime-dir restore actions for the selected local-minimal/local-default profile from an explicit backup snapshot through the local-minimal-node preview entrypoint.
EOF
}
