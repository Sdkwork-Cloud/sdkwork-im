...
if ($Help) {
    Write-Host "Usage: powershell -ExecutionPolicy Bypass -File bin/repair-runtime-local.ps1 [-RuntimeDir <path>] [-Json] [-Release]"
    Write-Host "Repair missing managed local-minimal runtime-dir state files through the local-minimal-node repair entrypoint."
    exit 0
}
