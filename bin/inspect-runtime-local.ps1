...
if ($Help) {
    Write-Host "Usage: powershell -ExecutionPolicy Bypass -File bin/inspect-runtime-local.ps1 [-ProfileName <local-minimal|local-default>] [-RuntimeDir <path>] [-Json] [-Release]"
    Write-Host "Usage: cmd /c .\bin\inspect-runtime-local.cmd [--profile <local-minimal|local-default>] [--runtime-dir <path>] [--json] [--release]"
    Write-Host "Inspect managed local runtime-dir state files for the selected local-minimal/local-default profile through the local-minimal-node inspection entrypoint."
    exit 0
}
