...
///
/// 实现一个基本的音视频通话的流程如下：
/// 1.创建引擎 [RTCEngine.createRTCVideo]
/// 2.设置编码参数 [RTCEngine.setMaxVideoEncoderConfig]
/// 3.开启本地视频采集 [RTCEngine.startVideoCapture]
/// 4.设置本地视频渲染视图 [RTCViewContext.localContext]
/// 5.创建音视频通话房间 [RTCEngine.createRTCRoom]
...
/// 3.SDK 支持同时发布多个参数的视频流，接口是 [RTCEngine.enableSimulcastMode] 和 [RTCEngine.setVideoEncoderConfig]
...
class _RoomPageState extends State<RoomPage> {
  ...
  void _initVideoAndJoinRoom() async {
    /// 创建引擎
    _rtcVideo = await RTCEngine.createRTCEngine(
        RTCVideoContext(appId: appId, eventHandler: _videoHandler));
    ...
    _rtcVideo?.setVideoEncoderConfig(solution);

    /// 设置本地视频渲染视图
    setState(() {
      _localRenderContext = RTCViewContext.localContext(userId: widget.userId);
    });
    ...
    /// 加入房间
    UserInfo userInfo = UserInfo(userId: widget.userId, extraInfo: "");
  ...
  void _initRoomEventHandler() {
    ...
    _roomHandler.onUserLeave = (String uid, int reason) {
      ...
      if (_firstRemoteRenderContext?.userId == uid) {
        setState(() {
          _firstRemoteRenderContext = null;
        });
        _rtcVideo?.removeRemoteVideo(
            streamId: _firstRemoteRenderContext!.streamId!);
      }
      if (_secondRemoteRenderContext?.userId == uid) {
        setState(() {
          _secondRemoteRenderContext = null;
        });
        _rtcVideo?.removeRemoteVideo(
            streamId: _secondRemoteRenderContext!.streamId!);
      }
      if (_thirdRemoteRenderContext?.userId == uid) {
        setState(() {
          _thirdRemoteRenderContext = null;
        });
        _rtcVideo?.removeRemoteVideo(
            streamId: _thirdRemoteRenderContext!.streamId!);
      }
    };

    _roomHandler.onUserPublishStreamVideo =
        (String streamId, StreamInfo streamInfo, bool isPublish) {
      debugPrint('onUserPublishStreamVideo: $streamId ${streamInfo.userId}');
      _handleUserPublishStream(streamId, streamInfo);
    };

    _roomHandler.onUserPublishStreamAudio =
        (String streamId, StreamInfo streamInfo, bool isPublish) {
      debugPrint('onUserPublishStreamAudio: $streamId ${streamInfo.userId}');
      _handleUserPublishStream(streamId, streamInfo);
    };
  }

  void _handleUserPublishStream(String streamId, StreamInfo streamInfo) {
    String userId = streamInfo.userId;
    if (_firstRemoteRenderContext?.streamId == streamId ||
        _secondRemoteRenderContext?.streamId == streamId ||
        _thirdRemoteRenderContext?.streamId == streamId) {
      return;
    }
    ...
    if (_firstRemoteRenderContext == null) {
      setState(() {
        _firstRemoteRenderContext = RTCViewContext.remoteContext(
            roomId: widget.roomId, userId: userId, streamId: streamId);
      });
  ...
  void _enableLocalAudio() {
    ...
    /// 控制本地音频的发送状态：发送/不发送
    if (_enableAudio) {
      _rtcRoom?.publishStreamAudio(true);
    } else {
      _rtcRoom?.publishStreamAudio(false);
    }
  }
