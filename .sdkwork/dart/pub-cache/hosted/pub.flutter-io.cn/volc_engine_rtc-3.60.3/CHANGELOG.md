# 3.60.3
- 修复 Android 开启混淆后, SDK 无法正常使用的 Bug。

# 3.60.2
- 获取视频特效管理器接口 `getVideoEffectPlayer` 更名为 `getVideoEffectInterface`。
- 创建引擎时配置项 `RTCVideoContext` 的 `autoCreateVideoEffectPlayer` 参数更名为 `autoCreateVideoEffectInterface`。

# 3.60.1

> 请参阅[升级指南](https://bytedance.larkoffice.com/docx/YKovdgQ5QoG1wbx1BzUc6S53nrb)了解3.58.x到3.60.1的详细变更。

此版本为单流版本，这意味着：

- 一次只能向房间内外的其他端点发送一个流。例如，您不能同时向房间发送屏幕录制内容和摄像头录制内容（音频不受此限制）。

> 如果您需要同时发送屏幕录制内容和摄像头录制内容，请联系我们，我们将为您提供可用的离线集成版本。

# 3.58.4

- 修复 Android 下 `startScreenCapture` 接口返回值类型错误导致 Flutter 抛错的问题 (不影响功能使用)。
- 修复 iOS 下离房时有概率触发 `No implementation found for method cancel` 的错误。
- 修改 3.58.3 版本 ChangeLog 中的描述错误。

# 3.58.3

变化详情可参考: [变更内容](https://bytedance.larkoffice.com/docx/PVwudkMeZocfJox9x5AcvKEHn6c?from=from_copylink)。

## 新增特性

支持 Android 16KB Page size。

支持 Android/iOS Platform 的音频数据回调:

- `registerAudioFrameObserver`: 注册/注销 音频数据回调观察者
- `enableAudioFrameCallback`: 开启音频回调
- `disableAudioFrameCallback`: 关闭音频回调
- `RTCAudioFrameObserverEventHandler.onRecordAudioFrame`: 返回麦克风录制的音频数据
- `RTCAudioFrameObserverEventHandler.onPlaybackAudioFrame`: 返回订阅的所有远端用户混音后的音频数据
- `RTCAudioFrameObserverEventHandler.onRemoteUserAudioFrame`: 返回远端单个用户的音频数据
- `RTCAudioFrameObserverEventHandler.onMixedAudioFrame`: 返回本地麦克风录制和订阅的所有远端用户混音后的音频数据

## API 变更

SDK 对流媒体的发布和订阅上切分了视频、音频维度。

- `RoomConfig.isAutoPublish` ---> `RoomConfig.isPublishVideo`/`RoomConfig.isPublishAudio`
- `unpublishStream`/`publishStream` ---> `publishStreamVideo`/`publishStreamAudio`
- `unpublishScreen`/`publishScreen` ---> `publishScreenVideo`/`publishScreenAudio`
- `subscribeStream`/`unsubscribeStream` ---> `subscribeStreamVideo`/`subscribeStreamAudio`
- `subscribeScreen`/`unsubscribeScreen` ---> `subscribeScreenVideo`/`subscribeScreenAudio`

以及上述涉及的 API 所相关的回调也有相应的变化。

# 3.58.2

## 修复问题

1. 设置远端视频渲染画面时，支持设置渲染角度
