...
const authorityPath = path.join(workspaceRoot, 'openapi', 'admin-control-plane.openapi.yaml');
const derivedPath = path.join(workspaceRoot, 'openapi', 'admin-control-plane.sdkgen.yaml');
const assemblyPath = path.join(workspaceRoot, '.sdkwork-assembly.json');
const authority = readAuthorityMeta(authorityPath, yaml);

const languageConfigs = {
  typescript: {
    workspace: 'sdkwork-craw-chat-sdk-admin-typescript',
    packages: [
      {
        layer: 'generated',
  ...
  flutter: {
    workspace: 'sdkwork-craw-chat-sdk-admin-flutter',
    packages: [
      {
        layer: 'generated',
        path: ['generated', 'server-openapi'],
