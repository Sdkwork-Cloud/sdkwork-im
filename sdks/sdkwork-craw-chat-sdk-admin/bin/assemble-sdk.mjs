...
function buildTypeScriptConsumerSurface(discoverySurface, packages) {
  ...
  const domains = [
    { name: 'protocol', operationGroups: ['protocol'] },
    { name: 'providers', operationGroups: ['providers'] },
    { name: 'cluster', operationGroups: ['cluster'] },
    { name: 'social', operationGroups: ['social'] },
    { name: 'system', operationGroups: ['system'] },
  ].filter((entry) =>
    entry.operationGroups.every((operationGroup) => availableOperationGroups.has(operationGroup)),
  );

  return {
    primaryClient: 'CrawChatSdkAdminClient',
...
function buildFlutterConsumerSurface(discoverySurface, packages) {
  ...
  return {
    primaryClient: 'CrawChatAdminClient',
    operationGroups: [...availableOperationGroups].sort(),
    domains,
    manualTransports: [],
  };
...
function renderLanguageAssembly(workspaceRoot, discoverySurface, language) {
  if (language === 'flutter') {
    const workspaceName = 'sdkwork-craw-chat-sdk-admin-flutter';
    const packages = [
      {
        layer: 'generated',
        ...
        readManifest(manifestPath) {
          return {
            ...
            entrypoints: {
              library: 'lib/craw_chat_admin_backend_sdk.dart',
            },
          };
        },
      },
      {
        ...
        readManifest(manifestPath) {
          return {
            ...
            description: readYamlScalar(manifestPath, 'description'),
            entrypoints: {
              library: 'lib/craw_chat_sdk_admin.dart',
            },
          };
        },
      },
  ...
  const workspaceName = 'sdkwork-craw-chat-sdk-admin-typescript';
  const packages = [
    {
      layer: 'generated',
      path: ['generated', 'server-openapi'],
...
export function assembleSdk(options = {}) {
  ...
  const authorityPath = path.join(workspaceRoot, 'openapi', 'craw-chat-control-plane.openapi.json');
  const derivedPath = path.join(workspaceRoot, 'openapi', 'craw-chat-control-plane.sdkgen.json');
...
if (isCli) {
  process.stdout.write(assembleSdk());
}
