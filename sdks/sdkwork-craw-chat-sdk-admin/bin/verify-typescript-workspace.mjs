...
export function verifyTypeScriptWorkspace(options = {}) {
  ...
  const composedSmokeTestPath = path.join(
    ...
    'composed',
    'test',
    'craw-chat-sdk-admin-client.test.mjs',
  );
  ...
  if (!existsSync(generatedPackagePath)) {
    ...
    if (generatedPackage.name !== '@sdkwork/craw-chat-admin-backend-sdk') {
      failures.push(
        `Generated TypeScript package name must be "@sdkwork/craw-chat-admin-backend-sdk", received "${generatedPackage.name}".`,
      );
    }
  ...
  if (!existsSync(composedPackagePath)) {
    ...
    if (composedPackage.name !== '@sdkwork/craw-chat-sdk-admin') {
      failures.push(
        `Composed TypeScript package name must be "@sdkwork/craw-chat-sdk-admin", received "${composedPackage.name}".`,
      );
    }
    if (!String(composedPackage.dependencies?.['@sdkwork/craw-chat-admin-backend-sdk'] || '').includes('../generated/server-openapi')) {
      failures.push('Composed TypeScript package must depend on the local generated admin backend package.');
    }
    ...
    if (!String(composedPackage.scripts?.test || '').includes('./test/craw-chat-sdk-admin-client.test.mjs')) {
      failures.push('Composed TypeScript package must expose a smoke-test script.');
    }
  }

  if (!existsSync(generatorRuntimePath)) {
    failures.push('Missing admin TypeScript generator runtime helper bin/generator-runtime.mjs.');
  }
  if (!existsSync(generatedBuildScriptPath)) {
    failures.push('Missing admin TypeScript generated-package build script.');
  }
  if (!existsSync(generatedVerifyScriptPath)) {
    failures.push('Missing admin TypeScript generated-package verification script.');
  }

  if (!existsSync(composedSdkPath)) {
    failures.push('Missing composed src/sdk.ts.');
  } else {
    const composedSdkSource = read(workspaceRoot, 'sdkwork-craw-chat-sdk-admin-typescript/composed/src/sdk.ts');
    if (!/export class CrawChatSdkAdminClient/.test(composedSdkSource)) {
      failures.push('Composed src/sdk.ts must export CrawChatSdkAdminClient.');
    }
    for (const domain of ['protocol', 'providers', 'cluster', 'social', 'system']) {
      if (!new RegExp(`readonly ${domain}:`, 'm').test(composedSdkSource)) {
        failures.push(`CrawChatSdkAdminClient must expose a "${domain}" domain.`);
      }
    }
    if (!/static async create/.test(composedSdkSource)) {
      failures.push('CrawChatSdkAdminClient must expose a static async create factory.');
    }
  }
  ...
  if (!existsSync(composedIndexPath)) {
    ...
    const composedIndexSource = read(workspaceRoot, 'sdkwork-craw-chat-sdk-admin-typescript/composed/src/index.ts');
    if (!/CrawChatSdkAdminClient/.test(composedIndexSource)) {
      failures.push('Composed src/index.ts must re-export CrawChatSdkAdminClient.');
    }
  }
  ...
  if (!existsSync(composedTypesPath)) {
    ...
    const composedTypesSource = read(workspaceRoot, 'sdkwork-craw-chat-sdk-admin-typescript/composed/src/types.ts');
    ...
    if (!composedTypesSource.includes("@sdkwork/craw-chat-admin-backend-sdk")) {
  ...
  if (!existsSync(composedGeneratedBackendTypesPath)) {
    ...
    const composedGeneratedBackendTypesSource = read(
      workspaceRoot,
      'sdkwork-craw-chat-sdk-admin-typescript/composed/src/generated-backend-types.ts',
    );
    ...
    if (!composedGeneratedBackendTypesSource.includes("@sdkwork/craw-chat-admin-backend-sdk")) {
  ...
  if (!existsSync(composedSmokeTestPath)) {
    failures.push('Missing composed smoke test test/craw-chat-sdk-admin-client.test.mjs.');
  }
...
if (isCli) {
  ...
  if (failures.length > 0) {
    console.error('[sdkwork-craw-chat-sdk-admin] TypeScript workspace verification failed:');
  ...
  try {
    ...
    console.error(`[sdkwork-craw-chat-sdk-admin] ${error.message}`);
    process.exit(1);
  }

  console.log('[sdkwork-craw-chat-sdk-admin] TypeScript workspace verification passed.');
}
