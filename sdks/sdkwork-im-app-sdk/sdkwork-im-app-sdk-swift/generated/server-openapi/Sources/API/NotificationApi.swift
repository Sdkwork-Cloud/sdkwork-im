...
public class NotificationApi {
    ...
    public func notificationsList() async throws -> NotificationListResponse? {
        return try await client.get(ApiPaths.appPath("/notifications"), responseType: NotificationListResponse.self)
    }

    /// Request a notification task
    public func notificationsRequestsCreate(body: RequestNotification) async throws -> NotificationRequestResponse? {
        return try await client.post(ApiPaths.appPath("/notifications/requests"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: NotificationRequestResponse.self)
    }

    /// Get a notification task
    public func notificationsRetrieve(notificationId: String) async throws -> NotificationTask? {
        return try await client.get(ApiPaths.appPath("/notifications/\(serializePathParameter(notificationId, PathParameterSpec(name: "notificationId", style: "simple", explode: false)))"), responseType: NotificationTask.self)
    }
