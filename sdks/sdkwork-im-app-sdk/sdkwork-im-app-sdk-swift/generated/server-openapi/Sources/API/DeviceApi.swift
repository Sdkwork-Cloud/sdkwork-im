...
public class DeviceApi {
    ...
    /// Get the device twin
    public func devicesTwinRetrieve(deviceId: String) async throws -> DeviceTwinView? {
        return try await client.get(ApiPaths.appPath("/devices/\(serializePathParameter(deviceId, PathParameterSpec(name: "deviceId", style: "simple", explode: false)))/twin"), responseType: DeviceTwinView.self)
    }

    /// Update the desired state for a device twin
    public func devicesTwinDesiredUpdate(deviceId: String, body: UpdateDeviceTwinDesiredRequest) async throws -> DeviceTwinView? {
        return try await client.post(ApiPaths.appPath("/devices/\(serializePathParameter(deviceId, PathParameterSpec(name: "deviceId", style: "simple", explode: false)))/twin/desired"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: DeviceTwinView.self)
    }

    /// Update the reported state for a device twin
    public func devicesTwinReportedUpdate(deviceId: String, body: UpdateDeviceTwinReportedRequest) async throws -> DeviceTwinView? {
        return try await client.post(ApiPaths.appPath("/devices/\(serializePathParameter(deviceId, PathParameterSpec(name: "deviceId", style: "simple", explode: false)))/twin/reported"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: DeviceTwinView.self)
    }
