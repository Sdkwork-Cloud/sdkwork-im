...
func (a *DeviceApi) SessionsResume(body sdktypes.ResumeDeviceSessionRequest) (sdktypes.DeviceSessionView, error) {
    raw, err := a.client.Post(ImApiPath("/device/sessions/resume"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.DeviceSessionView
        return zero, err
    }
    return decodeResult[sdktypes.DeviceSessionView](raw)
}

// Disconnect a device runtime session
func (a *DeviceApi) SessionsDisconnect(body sdktypes.RegisterDeviceRequest) (sdktypes.DeviceSessionDisconnectResponse, error) {
    raw, err := a.client.Post(ImApiPath("/device/sessions/disconnect"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.DeviceSessionDisconnectResponse
        return zero, err
    }
    return decodeResult[sdktypes.DeviceSessionDisconnectResponse](raw)
}

// Register a device for sync feed delivery
func (a *DeviceApi) RegistrationsCreate(body sdktypes.RegisterDeviceRequest) (sdktypes.RegisteredDeviceView, error) {
    raw, err := a.client.Post(ImApiPath("/devices/register"), body, nil, nil, "application/json")
    if err != nil {
...
func (a *DeviceApi) SyncFeedRetrieve(deviceId string, afterSeq *int, limit *int) (sdktypes.DeviceSyncFeedResponse, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(ImApiPath(fmt.Sprintf("/devices/%s/sync_feed", SerializePathParameter(deviceId, PathParameterSpec{Name: "deviceId", Style: "simple", Explode: false}))), query), nil, nil)
    if err != nil {
