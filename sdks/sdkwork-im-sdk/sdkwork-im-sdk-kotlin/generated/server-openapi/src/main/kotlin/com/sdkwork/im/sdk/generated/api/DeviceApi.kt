...
class DeviceApi(private val client: HttpClient) {
    ...
    suspend fun sessionsResume(body: ResumeDeviceSessionRequest): DeviceSessionView? {
        val raw = client.post(ApiPaths.imPath("/device/sessions/resume"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<DeviceSessionView>() {})
    }

    /** Disconnect a device runtime session */
    suspend fun sessionsDisconnect(body: RegisterDeviceRequest): DeviceSessionDisconnectResponse? {
        val raw = client.post(ApiPaths.imPath("/device/sessions/disconnect"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<DeviceSessionDisconnectResponse>() {})
    }

    /** Register a device for sync feed delivery */
    suspend fun registrationsCreate(body: RegisterDeviceRequest): RegisteredDeviceView? {
        val raw = client.post(ApiPaths.imPath("/devices/register"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<RegisteredDeviceView>() {})
    }

    /** Retrieve bounded device sync feed entries */
    suspend fun syncFeedRetrieve(deviceId: String, afterSeq: Int? = null, limit: Int? = null): DeviceSyncFeedResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("afterSeq", afterSeq, "form", true, false, null),
            QueryParameterSpec("limit", limit, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.imPath("/devices/${serializePathParameter(deviceId, PathParameterSpec("deviceId", "simple", false))}/sync_feed"), query))
