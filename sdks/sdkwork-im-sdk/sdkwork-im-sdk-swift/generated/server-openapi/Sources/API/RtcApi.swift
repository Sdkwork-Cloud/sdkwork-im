...
public class RtcApi {
    ...
    public func sessionsCreate(body: CreateRtcSessionRequest) async throws -> RtcSession? {
        return try await client.post(ApiPaths.imPath("/rtc/sessions"), body: body, params: nil, headers: nil, contentType: "application/json", responseType: RtcSession.self)
    }

    /// Retrieve IM-backed RTC session state
    public func sessionsRetrieve(rtcSessionId: String) async throws -> RtcSession? {
        return try await client.get(ApiPaths.imPath("/rtc/sessions/\(serializePathParameter(rtcSessionId, PathParameterSpec(name: "rtcSessionId", style: "simple", explode: false)))"), responseType: RtcSession.self)
    }
