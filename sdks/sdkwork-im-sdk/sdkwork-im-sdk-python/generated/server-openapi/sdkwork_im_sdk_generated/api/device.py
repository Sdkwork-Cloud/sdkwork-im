...
from ..models import DeviceSessionDisconnectResponse, DeviceSessionView, DeviceSyncFeedResponse, RegisterDeviceRequest, RegisteredDeviceView, ResumeDeviceSessionRequest
...
class DeviceApi:
    ...
    def __init__(self, client: HttpClient):
        self._client = client
        self.sessions = DeviceSessionsApi(client)
        self.registrations = DeviceRegistrationsApi(client)
        self.sync_feed = DeviceSyncFeedApi(client)


class DeviceSessionsApi:
    """device device.sessions API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def resume(self, body: ResumeDeviceSessionRequest) -> DeviceSessionView:
        """Resume a device runtime session"""
        return self._client.post(f"/im/v3/api/device/sessions/resume", json=body)

    def create_disconnect(self, body: RegisterDeviceRequest) -> DeviceSessionDisconnectResponse:
        """Disconnect a device runtime session"""
        return self._client.post(f"/im/v3/api/device/sessions/disconnect", json=body)

class DeviceRegistrationsApi:
    """device device.registrations API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def registrations_create(self, body: RegisterDeviceRequest) -> RegisteredDeviceView:
        """Register a device for sync feed delivery"""
        return self._client.post(f"/im/v3/api/devices/register", json=body)

class DeviceSyncFeedApi:
    """device device.sync_feed API client."""

    def __init__(self, client: HttpClient):
        self._client = client


    def list(self, device_id: str, after_seq: Optional[int] = None, limit: Optional[int] = None) -> DeviceSyncFeedResponse:
        ...
        return self._client.get(_append_query_string(f"/im/v3/api/devices/{serialize_path_parameter(device_id, {'name': 'deviceId', 'style': 'simple', 'explode': False})}/sync_feed", query))
