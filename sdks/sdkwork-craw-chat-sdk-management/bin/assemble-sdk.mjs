...
function fail(message) {
  console.error(`[sdkwork-craw-chat-sdk-management] ${message}`);
  process.exit(1);
}
...
function buildTypeScriptConsumerSurface(discoverySurface, packages) {
  ...
  return {
    primaryClient: 'CrawChatSdkManagementClient',
...
function buildFlutterConsumerSurface(discoverySurface, packages) {
  ...
  return {
    primaryClient: 'CrawChatManagementClient',
    operationGroups: [...availableOperationGroups].sort(),
    domains,
    manualTransports: [],
  };
...
function renderLanguageAssembly(workspaceRoot, discoverySurface, language) {
  if (language === 'flutter') {
    const workspaceName = 'sdkwork-craw-chat-sdk-management-flutter';
    const packages = [
      {
        layer: 'generated',
        ...
        readManifest(manifestPath) {
          return {
            ...
            entrypoints: {
              library: 'lib/craw_chat_management_backend_sdk.dart',
            },
          };
        },
      },
      {
        ...
        readManifest(manifestPath) {
          return {
            ...
            description: readYamlScalar(manifestPath, 'description'),
            entrypoints: {
              library: 'lib/craw_chat_sdk_management.dart',
            },
          };
        },
      },
  ...
  const workspaceName = 'sdkwork-craw-chat-sdk-management-typescript';
  const packages = [
    {
      layer: 'generated',
      path: ['generated', 'server-openapi'],
