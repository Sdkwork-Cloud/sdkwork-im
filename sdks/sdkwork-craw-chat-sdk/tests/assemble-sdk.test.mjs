...
test('assemble-sdk emits manifest-backed language package details', async () => {
  ...
  assert.equal(assembly.workspace, 'sdkwork-craw-chat-sdk');
  ...
  assert.equal(
    ...
    'sdkwork-craw-chat-sdk-typescript/generated/server-openapi/package.json',
  );
  assert.equal(typescript.name, '@sdkwork/craw-chat-backend-sdk');
  ...
  assert.equal(flutter.name, 'backend_sdk');
  assert.ok(Array.isArray(flutter.packages));
  assert.deepEqual(
    flutter.packages.map((entry) => entry.layer),
    ['generated', 'composed'],
  );
  assert.equal(flutter.packages[1].name, 'craw_chat_sdk');
