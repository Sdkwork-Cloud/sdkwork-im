...
export class CrawChatWebSocketReceiver {
    ...
    close(code, reason) {
        ...
        this.cleanupPendingRequests(new CrawChatSdkError('websocket_closed', 'Realtime connection closed before the pending operation completed.'));
    ...
    async resolveSocket() {
        ...
        if (!url) {
            throw new CrawChatSdkError('websocket_url_required', 'websocketBaseUrl or connect({ url }) is required to establish realtime connectivity.');
        }
    ...
    handleSocketClose(event) {
        ...
        this.cleanupPendingRequests(new CrawChatSdkError('websocket_closed', 'Realtime websocket closed before the pending operation completed.'));
        if (!this.isConnected) {
            this.rejectConnect(new CrawChatSdkError('websocket_closed', 'Realtime websocket closed before the connected frame arrived.'));
        }
        this.resetSocketState();
    }
    handleSocketError(event) {
        ...
        if (!this.isConnected) {
            this.rejectConnect(event instanceof Error
                ? event
                : new CrawChatSdkError('websocket_transport_error', 'Realtime websocket failed before the connection became ready.'));
        }
    }
    ...
    async handleCcpControlFrame(frame) {
        if (frame.type === 'hello_ack') {
            if (!frame.data.accepted) {
                this.rejectConnect(new CrawChatSdkError('websocket_protocol_error', 'Realtime CCP hello negotiation was rejected by the server.'));
                return;
            }
        ...
        if (frame.type === 'goaway' || frame.type === 'error') {
            const error = new CrawChatSdkError('websocket_protocol_error', frame.data.message, {
    ...
    requireSocket() {
        if (!this.socket) {
            throw new CrawChatSdkError('websocket_not_connected', 'Realtime connection is not connected. Call connect() first.');
        }
        return this.socket;
    }
    createPendingRequest(requestId, kind) {
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(new CrawChatSdkError('websocket_request_timeout', `Realtime websocket request ${requestId} timed out.`, {
                    requestId,
                    kind,
                }));
...
function createDefaultSocket(request) {
    ...
    if (typeof WebSocketConstructor !== 'function') {
        throw new CrawChatSdkError('websocket_factory_required', 'No global WebSocket implementation is available. Provide webSocketFactory to establish realtime connectivity in this runtime.');
    }
    if (Object.keys(request.headers).length > 0) {
        throw new CrawChatSdkError('websocket_factory_required', 'The default WebSocket implementation cannot attach Authorization headers. Provide webSocketFactory for authenticated realtime connections.');
    }
...
function requireCcpOptions(options) {
    ...
    throw new CrawChatSdkError('ccp_auth_required', 'CCP websocket mode requires ccp.principalId and ccp.actorKind so auth_bind can be negotiated.');
}
