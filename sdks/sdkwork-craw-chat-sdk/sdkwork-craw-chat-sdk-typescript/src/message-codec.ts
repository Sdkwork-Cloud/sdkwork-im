...
import type {
  ...
  CrawChatInternalReceiverUnknownEvent,
} from './receiver-internal-types.js';
import type {
  ContentPart,
  CrawChatCardAction,
  CrawChatDecodableMessageBody,
  CrawChatDecodedDataPayload,
  CrawChatDecodedMessage,
  CrawChatDecodedMessageAttachment,
  CrawChatDecodedRtcSignal,
  CrawChatDecodedStreamFrame,
  MessageBody,
  PostMessageRequest,
  RealtimeEvent,
  RtcSignalEvent,
...
export function decodeMessageBody(
  body: CrawChatDecodableMessageBody,
): CrawChatDecodedMessage {
...
export function decodeStreamFrame(frame: StreamFrame): CrawChatDecodedStreamFrame {
  return {
...
export function decodeRealtimeEvent(
  ...
  if (isDecodableMessagePayload(payloadRecord.body)) {
    ...
    } satisfies CrawChatInternalReceiverMessageEvent;
  }
  ...
  if (
    ...
    } satisfies CrawChatInternalReceiverRtcSignalEvent;
  }
  ...
  if (
    ...
    } satisfies CrawChatInternalReceiverDataEvent;
  }
  ...
  return {
    ...
    scopeId: event.scopeId,
    payload,
  } satisfies CrawChatInternalReceiverUnknownEvent;
}

function buildDecodedMessage<TType extends CrawChatDecodedMessage['type']>(
  type: TType,
  body: CrawChatDecodableMessageBody,
  attachments: CrawChatDecodedMessageAttachment[],
  content: Extract<CrawChatDecodedMessage, { type: TType }>['content'],
): Extract<CrawChatDecodedMessage, { type: TType }> {
  return {
    type,
    ...
    rawBody: body,
  } as Extract<CrawChatDecodedMessage, { type: TType }>;
}

function decodeMediaAttachment(part: ContentPart): CrawChatDecodedMessageAttachment {
  return {
...
function getBodyParts(body: CrawChatDecodableMessageBody): ContentPart[] {
  return body.parts ?? [];
}

function getBodyText(body: CrawChatDecodableMessageBody): string | undefined {
  return isPostMessageRequest(body) ? body.text : undefined;
}

function getBodyRenderHints(body: CrawChatDecodableMessageBody): StringMap | undefined {
...
function toCardActions(value: unknown): CrawChatCardAction[] {
  if (!Array.isArray(value)) {
    return [];
  }
