...
import { resolveSdkworkChatPcClientId } from '../../apps/sdkwork-chat-pc/packages/sdkwork-clawchat-pc-chat/src/services/ClientIdentityService';

const CLIENT_ID_STORAGE_KEY = 'sdkwork-chat-pc:client-id:v1';
const LEGACY_IM_DEVICE_ID_STORAGE_KEY = 'sdkwork-chat-pc:im-device-id:v1';
...
async function main(): Promise<void> {
  ...
  localStorage.clear();
  localStorage.setItem(LEGACY_IM_DEVICE_ID_STORAGE_KEY, 'sdkwork-chat-pc-1780523380759-zhyejitm');
  const regeneratedClientId = resolveSdkworkChatPcClientId();
  assertClientIdIsLocal(regeneratedClientId);
  assert.notEqual(regeneratedClientId, 'sdkwork-chat-pc-1780523380759-zhyejitm');
  assert.equal(localStorage.getItem(CLIENT_ID_STORAGE_KEY), regeneratedClientId);

  console.log('sdkwork-chat-pc client identity contract passed');
}
