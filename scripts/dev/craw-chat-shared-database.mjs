...
function assertNoCanonicalLegacyAliases(env) {
  if (normalizeDatabaseField(env.SDKWORK_CHAT_DATABASE_PROVIDER)) {
    throw new Error(
      'SDKWORK_CHAT_DATABASE_PROVIDER is not standard; use SDKWORK_CHAT_DATABASE_ENGINE',
    );
  }
  if (normalizeDatabaseField(env.SDKWORK_CHAT_DATABASE_SSLMODE)) {
    throw new Error(
      'SDKWORK_CHAT_DATABASE_SSLMODE is not standard; use SDKWORK_CHAT_DATABASE_SSL_MODE',
    );
  }
}
...
function resolvePostgresDatabaseUrlFromFields(env) {
  ...
  const host = envValue(env, 'SDKWORK_CHAT_DATABASE_HOST', 'SDKWORK_CLAW_DATABASE_HOST');
  const database = envValue(env, 'SDKWORK_CHAT_DATABASE_NAME', 'SDKWORK_CLAW_DATABASE_NAME');
  ...
  const password = envValue(
    env,
    'SDKWORK_CHAT_DATABASE_PASSWORD',
    'SDKWORK_CLAW_DATABASE_PASSWORD',
  );
  const missing = [];
  if (!host) {
    missing.push('SDKWORK_CHAT_DATABASE_HOST');
  }
  if (!database) {
    missing.push('SDKWORK_CHAT_DATABASE_NAME');
  }
  if (!username) {
    missing.push('SDKWORK_CHAT_DATABASE_USERNAME');
  }
  if (!password) {
    missing.push('SDKWORK_CHAT_DATABASE_PASSWORD');
  }
  if (missing.length > 0) {
    throw new Error(
      `SDKWORK_CHAT_DATABASE_ENGINE=postgresql requires ${missing.join(', ')}`,
    );
  }

  const port = envValue(env, 'SDKWORK_CHAT_DATABASE_PORT', 'SDKWORK_CLAW_DATABASE_PORT');
  ...
  appendPostgresQueryParam(
    params,
    'sslmode',
    envValue(env, 'SDKWORK_CHAT_DATABASE_SSL_MODE', 'SDKWORK_CLAW_DATABASE_SSLMODE'),
  );
...
function resolveConfiguredSqliteUrl(env) {
  ...
  const deploymentMode = normalizeDatabaseField(env.SDKWORK_CHAT_DEPLOYMENT_MODE)
...
function databaseBridgeEnv({
  ...
  const resolvedMaxConnections = maxConnections
    ?? envValue(env, 'SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS', 'SDKWORK_CLAW_DATABASE_MAX_CONNECTIONS');
  return {
    SDKWORK_CHAT_DATABASE_ENGINE: engine,
    SDKWORK_CHAT_DATABASE_URL: databaseUrl,
    SDKWORK_CLAW_DATABASE_URL: databaseUrl,
    ...(resolvedMaxConnections
      ? {
        SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS: resolvedMaxConnections,
...
export function resolveCrawChatSharedDatabaseConfig({
  env = process.env,
  repoRoot: _root = repoRoot,
} = {}) {
  assertNoCanonicalLegacyAliases(env);
  const databaseUrl = normalizeDatabaseUrl(env.SDKWORK_CHAT_DATABASE_URL)
