...
const dbScriptPath = path.join(repoRoot, 'scripts/dev/craw-chat-postgres-db.mjs');
...
const parsedSplitConfig = parsePostgresConfig({
  configText: [
    ...
    'SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD=admin pass',
...
const parsedWslPsqlConfig = parsePostgresConfig({
  configText: [
    ...
    'SDKWORK_CHAT_DATABASE_ADMIN_USERNAME=postgres',
    'SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD=admin pass',
    'SDKWORK_CHAT_DATABASE_PSQL_COMMAND=wsl.exe',
    'SDKWORK_CHAT_DATABASE_PSQL_ARGS=-d Ubuntu-22.04 -- psql',
    'SDKWORK_CHAT_DATABASE_PSQL_PATH_STYLE=wsl',
    '',
  ].join('\n'),
  configPath: '.env.postgres',
});
...
assert.throws(
  ...
  /SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD|SDKWORK_CHAT_DATABASE_ADMIN_URL/u,
...
for (const doc of [configIndex, ubuntuWslGuide, devGuide]) {
  for (const required of [
    ...
    'SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD',
...
console.log('craw-chat PostgreSQL pnpm database command contract passed');
