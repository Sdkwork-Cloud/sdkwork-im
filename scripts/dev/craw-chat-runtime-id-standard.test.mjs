...
assert.match(
  ...
  'craw-chat-runtime-id must be a workspace crate so database insert code can use the standard runtime ID generator.',
);

const runtimeIdSource = readText('crates', 'craw-chat-runtime-id', 'src', 'lib.rs');
for (const expectedText of [
  'use sdkwork_id::{SnowflakeIdError, SnowflakeIdGenerator};',
  'pub const SDKWORK_CHAT_ID_NODE_ID_ENV: &str = "SDKWORK_CHAT_ID_NODE_ID";',
...
for (const relativePath of [
  ...
  assert.match(
    template,
    /^SDKWORK_CHAT_ID_NODE_ID=\d+$/mu,
    `${relativePath} must declare an explicit Snowflake node id for distributed deployments.`,
  );
}

const releasePlanner = readText('scripts', 'release', 'plan-craw-chat-install-packages.mjs');
assert.match(
  releasePlanner,
  /'SDKWORK_CHAT_ID_NODE_ID'/u,
  'release package database/runtime env override policy must preserve SDKWORK_CHAT_ID_NODE_ID.',
);
