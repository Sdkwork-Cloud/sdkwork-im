...
async function main(): Promise<void> {
  ...
  assert.deepEqual(
    agentSdkCalls[3],
    {
      body: {
        ...
        manifest: standardManifest('agent.my.docs', 'Updated Agent', 'updated agent'),
        requestedAt: agentSdkCalls[3]?.body?.requestedAt,
      },
      id: 'agent.my.docs',
  ...
  assert.deepEqual(
    agentSdkCalls[5],
    {
      body: {
        bindingId: 'binding.manifest.default',
        deploymentId: agentSdkCalls[5]?.body?.deploymentId,
  ...
  assert.deepEqual(
    agentSdkCalls.slice(7).map((call) => ({
      agentId: call.id,
