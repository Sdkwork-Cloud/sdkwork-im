...
assert.equal(manifest.app.displayName, 'SDKWork Chat', 'display name may carry the product label');
assert.equal(manifest.publish.config.workspaceRoot, 'apps/craw-chat');
assert.equal(manifest.devApp.sourceRoot, 'apps/craw-chat');
...
for (const required of [
  ...
  'SDKWORK_CHAT_REDIS_HOST=127.0.0.1',
...
const sharedDbModule = await import(
  pathToFileURL(path.join(repoRoot, 'scripts/dev/craw-chat-shared-database.mjs')).href
);
const canonicalPostgres = sharedDbModule.resolveCrawChatSharedDatabaseConfig({
  env: {
    SDKWORK_CHAT_DATABASE_ENGINE: 'postgresql',
    SDKWORK_CHAT_DATABASE_HOST: '127.0.0.1',
    SDKWORK_CHAT_DATABASE_PORT: '15432',
    SDKWORK_CHAT_DATABASE_NAME: 'sdkwork_ai_dev',
    SDKWORK_CHAT_DATABASE_USERNAME: 'sdkwork_ai_dev',
    SDKWORK_CHAT_DATABASE_PASSWORD: 'chat pass',
    SDKWORK_CHAT_DATABASE_SSL_MODE: 'disable',
    SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS: '11',
  },
  repoRoot,
});
assert.equal(canonicalPostgres.kind, 'postgresql');
assert.equal(
  canonicalPostgres.env.SDKWORK_CHAT_DATABASE_URL,
...
assert.equal(canonicalPostgres.env.SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS, '11');
assert.equal(canonicalPostgres.env.SDKWORK_CLAW_DATABASE_MAX_CONNECTIONS, '11');

assert.throws(
  () => sharedDbModule.resolveCrawChatSharedDatabaseConfig({
    env: {
      SDKWORK_CHAT_DATABASE_PROVIDER: 'postgresql',
      SDKWORK_CHAT_DATABASE_HOST: '127.0.0.1',
      SDKWORK_CHAT_DATABASE_NAME: 'sdkwork_ai_dev',
      SDKWORK_CHAT_DATABASE_USERNAME: 'sdkwork_ai_dev',
      SDKWORK_CHAT_DATABASE_PASSWORD: 'sdkworkdev123',
  ...
  /SDKWORK_CHAT_DATABASE_PROVIDER.*not standard/u,
...
assert.throws(
  () => sharedDbModule.resolveCrawChatSharedDatabaseConfig({
    env: {
      ...
      SDKWORK_CHAT_DATABASE_USERNAME: 'sdkwork_ai_dev',
      SDKWORK_CHAT_DATABASE_PASSWORD: 'sdkworkdev123',
      SDKWORK_CHAT_DATABASE_SSLMODE: 'disable',
    },
    repoRoot,
  }),
  /SDKWORK_CHAT_DATABASE_SSLMODE.*not standard/u,
  'new app config must reject legacy DATABASE_SSLMODE spelling',
);

const canonicalSqlite = sharedDbModule.resolveCrawChatSharedDatabaseConfig({
  env: {
    SDKWORK_CHAT_DEPLOYMENT_MODE: 'desktop',
  },
  repoRoot,
});
...
assert.equal(canonicalSqlite.env.SDKWORK_CHAT_DATABASE_ENGINE, 'sqlite');
assert.equal(canonicalSqlite.env.SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS, '1');
assert.equal(
  canonicalSqlite.env.SDKWORK_CLAW_DATABASE_URL,
  canonicalSqlite.env.SDKWORK_CHAT_DATABASE_URL,
  'desktop SQLite config must be bridged to the current Rust-compatible env name',
);

const planModule = await import(
  pathToFileURL(path.join(repoRoot, 'scripts/release/plan-craw-chat-install-packages.mjs')).href
);
const plan = planModule.createCrawChatInstallPackagePlan({ version: '1.2.3' });
...
assert.ok(linuxServer.databasePolicy.envOverrides.includes('SDKWORK_CHAT_DATABASE_ENGINE'));
assert.ok(linuxServer.databasePolicy.envOverrides.includes('SDKWORK_CHAT_LOG_DIR'));
...
assert.ok(linuxDesktop.databasePolicy.envOverrides.includes('SDKWORK_CHAT_DATABASE_URL'));
assert.ok(linuxDesktop.databasePolicy.envOverrides.includes('SDKWORK_CHAT_DATABASE_FILE'));
