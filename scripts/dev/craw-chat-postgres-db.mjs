...
function configFromEnvObject(env, configPath) {
  if (normalizeField(env.SDKWORK_CHAT_DATABASE_PROVIDER)) {
    throw new Error('SDKWORK_CHAT_DATABASE_PROVIDER is not standard; use SDKWORK_CHAT_DATABASE_ENGINE');
  }
  if (normalizeField(env.SDKWORK_CHAT_DATABASE_SSLMODE)) {
    throw new Error('SDKWORK_CHAT_DATABASE_SSLMODE is not standard; use SDKWORK_CHAT_DATABASE_SSL_MODE');
  }
  const databaseFromUrl = parsePostgresDatabaseUrl(
    env.SDKWORK_CHAT_DATABASE_URL ?? env.SDKWORK_CLAW_DATABASE_URL,
    env.SDKWORK_CHAT_DATABASE_SCHEMA ?? env.SDKWORK_CLAW_DATABASE_SCHEMA,
  );
  const engine = normalizeField(env.SDKWORK_CHAT_DATABASE_ENGINE ?? env.SDKWORK_CLAW_DATABASE_PROVIDER);
  if (!databaseFromUrl && engine && !/^postgres(?:ql)?$/iu.test(engine)) {
    throw new Error(`unsupported SDKWork Chat database engine: ${engine}`);
  }
  const database = databaseFromUrl ?? {
    database: normalizeField(env.SDKWORK_CHAT_DATABASE_NAME ?? env.SDKWORK_CLAW_DATABASE_NAME),
    host: normalizeField(env.SDKWORK_CHAT_DATABASE_HOST ?? env.SDKWORK_CLAW_DATABASE_HOST),
    password: normalizeField(env.SDKWORK_CHAT_DATABASE_PASSWORD ?? env.SDKWORK_CLAW_DATABASE_PASSWORD),
    port: normalizeField(env.SDKWORK_CHAT_DATABASE_PORT ?? env.SDKWORK_CLAW_DATABASE_PORT) ?? '5432',
    schema: normalizeField(env.SDKWORK_CHAT_DATABASE_SCHEMA ?? env.SDKWORK_CLAW_DATABASE_SCHEMA)
      ?? normalizeField(env.SDKWORK_CHAT_DATABASE_NAME ?? env.SDKWORK_CLAW_DATABASE_NAME),
    sslmode: normalizeField(env.SDKWORK_CHAT_DATABASE_SSL_MODE ?? env.SDKWORK_CLAW_DATABASE_SSLMODE),
    username: normalizeField(env.SDKWORK_CHAT_DATABASE_USERNAME ?? env.SDKWORK_CLAW_DATABASE_USERNAME),
  };
  const adminFromUrl = parsePostgresDatabaseUrl(
    env.SDKWORK_CHAT_DATABASE_ADMIN_URL ?? env.SDKWORK_CLAW_DATABASE_ADMIN_URL,
    undefined,
  );
  const admin = adminFromUrl ?? {
    database: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_DATABASE ?? env.SDKWORK_CLAW_DATABASE_ADMIN_DATABASE) ?? 'postgres',
    host: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_HOST ?? env.SDKWORK_CLAW_DATABASE_ADMIN_HOST) ?? database.host,
    password: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD ?? env.SDKWORK_CLAW_DATABASE_ADMIN_PASSWORD),
    port: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_PORT ?? env.SDKWORK_CLAW_DATABASE_ADMIN_PORT) ?? database.port,
    sslmode: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_SSL_MODE ?? env.SDKWORK_CLAW_DATABASE_ADMIN_SSLMODE) ?? database.sslmode,
    username: normalizeField(env.SDKWORK_CHAT_DATABASE_ADMIN_USERNAME ?? env.SDKWORK_CLAW_DATABASE_ADMIN_USERNAME) ?? 'postgres',
  };
  return {
    admin,
    database,
    psql: {
      args: parseCommandArgs(
        env.SDKWORK_CHAT_DATABASE_PSQL_ARGS ?? env.SDKWORK_CLAW_DATABASE_PSQL_ARGS,
      ),
      command: normalizeField(
        env.SDKWORK_CHAT_DATABASE_PSQL_COMMAND ?? env.SDKWORK_CLAW_DATABASE_PSQL_COMMAND,
      ),
      pathStyle: normalizeField(
        env.SDKWORK_CHAT_DATABASE_PSQL_PATH_STYLE ?? env.SDKWORK_CLAW_DATABASE_PSQL_PATH_STYLE,
      ),
    },
    source: {
      format: 'env',
...
function validateAdminConfig(admin) {
  ...
  for (const field of ['host', 'database', 'username']) {
    if (!normalizeField(admin[field])) {
      missing.push(`SDKWORK_CHAT_DATABASE_ADMIN_${field.toUpperCase()}`);
    }
  }
  if (missing.length > 0) {
...
function validateAdminExecutionConfig(admin) {
  ...
  if (!normalizeField(admin.password)) {
    throw new Error(
      'PostgreSQL initialization requires SDKWORK_CHAT_DATABASE_ADMIN_PASSWORD or SDKWORK_CHAT_DATABASE_ADMIN_URL',
    );
  }
}
...
export function parsePostgresConfig({
  ...
  validateDatabaseConfig(config.database, 'SDKWORK_CHAT_DATABASE_');
...
function createRoleAndDatabaseSql(config, redactSecrets) {
  ...
  return [
    '-- Craw Chat PostgreSQL role and database initialization.',
...
function createSchemaAndGrantSql(config) {
  ...
  const username = sqlLiteral(config.database.username);
  return [
    '-- Craw Chat PostgreSQL schema and grants initialization.',
    '\\set ON_ERROR_STOP on',
...
export function formatPostgresDbPlan(plan) {
  return [
    `Craw Chat PostgreSQL database plan (${plan.mode})`,
...
function helpText() {
  return [
    'Usage:',
    '  node scripts/dev/craw-chat-postgres-db.mjs --mode plan --config .env.postgres --dry-run',
    '  node scripts/dev/craw-chat-postgres-db.mjs --mode init --config .env.postgres',
    '  node scripts/dev/craw-chat-postgres-db.mjs --mode migrate --config .env.postgres',
    '',
    'Modes:',
...
export function runPostgresDbCli({
  ...
  if (env.SDKWORK_CHAT_DATABASE_SCHEMA || env.SDKWORK_CLAW_DATABASE_SCHEMA) {
    config.database.schema = env.SDKWORK_CHAT_DATABASE_SCHEMA ?? env.SDKWORK_CLAW_DATABASE_SCHEMA;
  }
  ...
  for (const step of plan.steps) {
    stdout.write(`[craw-chat-db] ${step.label}\n`);
  ...
  stdout.write('[craw-chat-db] PostgreSQL database task completed\n');
  return { plan: redactedPlan, status: 0 };
}
...
if (path.resolve(process.argv[1] ?? '') === __filename) {
  try {
    ...
    process.stderr.write(`[craw-chat-db] ${error instanceof Error ? error.message : String(error)}\n`);
    process.exitCode = 1;
  }
}
