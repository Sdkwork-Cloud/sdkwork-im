...
import { DEFAULT_RELEASE_VERSION, normalizeCrawChatReleaseVersion } from './craw-chat-release-version.mjs';

const INSTALL_PACKAGE_SCHEMA_VERSION = '2026-06-04.craw-chat.install-packages.v1';
...
const RUNTIME_DISPLAY_NAME = 'SDKWork Chat';
const SERVER_BINARY_BASENAME = 'craw-chat-server';
...
function printHelp() {
  console.log(`Usage: node scripts/release/plan-craw-chat-install-packages.mjs [options]

Create and validate the Craw Chat cross-platform release package plan.
...
function createCrawChatInstallPackagePlan({
  version = DEFAULT_RELEASE_VERSION,
  platforms = SUPPORTED_PLATFORMS,
  architectures = SUPPORTED_ARCHITECTURES,
  deploymentModes = SUPPORTED_DEPLOYMENT_MODES,
} = {}) {
  const normalizedVersion = normalizeCrawChatReleaseVersion(version);
  ...
  for (const deploymentMode of selectedDeploymentModes) {
    for (const platform of selectedPlatforms) {
      for (const architecture of selectedArchitectures) {
        packages.push(createCrawChatInstallPackageItem({
          architecture,
          deploymentMode,
          platform,
...
function createCrawChatInstallPackageItem({
  architecture,
  deploymentMode,
  platform,
  version,
  ...
  const packagePrefix = deploymentMode === 'desktop' ? 'craw-chat-desktop' : 'craw-chat-server';
...
function buildServerArchiveArtifacts(binaryName) {
  return [
    ...
    {
      kind: 'pc-web-dist',
      path: 'web/sdkwork-chat-pc/dist',
      source: 'apps/sdkwork-chat-pc/dist',
      required: true,
    },
    {
  ...
  return [
    {
      ...
      path: 'desktop',
      source: 'apps/sdkwork-chat-pc/packages/sdkwork-clawchat-pc-desktop/src-tauri/target/release/bundle',
      required: true,
    },
    {
...
function databasePolicyFor({ platform, runtimeProfile }) {
  ...
  if (runtimeProfile === 'desktop') {
    return {
      ...
      envOverrides: [
        ...
        'SDKWORK_CHAT_DATA_DIR',
        'SDKWORK_CHAT_LOG_DIR',
        'SDKWORK_CHAT_CACHE_DIR',
        'SDKWORK_CHAT_DATABASE_ENGINE',
        'SDKWORK_CHAT_DATABASE_FILE',
        'SDKWORK_CHAT_DATABASE_URL',
      ],
    };
  }
  ...
  return {
    ...
    envOverrides: [
      ...
      'SDKWORK_CHAT_DATA_DIR',
      'SDKWORK_CHAT_LOG_DIR',
      'SDKWORK_CHAT_RUN_DIR',
      'SDKWORK_CHAT_ID_NODE_ID',
      ...
      'SDKWORK_CHAT_DATABASE_ENGINE',
      'SDKWORK_CHAT_DATABASE_HOST',
      'SDKWORK_CHAT_DATABASE_PORT',
      'SDKWORK_CHAT_DATABASE_NAME',
      'SDKWORK_CHAT_DATABASE_SCHEMA',
      'SDKWORK_CHAT_DATABASE_USERNAME',
      'SDKWORK_CHAT_DATABASE_PASSWORD_FILE',
      'SDKWORK_CHAT_DATABASE_SSL_MODE',
      'SDKWORK_CHAT_DATABASE_MAX_CONNECTIONS',
      'SDKWORK_CHAT_REDIS_ENABLED',
      'SDKWORK_CHAT_REDIS_HOST',
      'SDKWORK_CHAT_REDIS_PORT',
      'SDKWORK_CHAT_REDIS_DATABASE',
      'SDKWORK_CHAT_REDIS_PASSWORD_FILE',
      'SDKWORK_CHAT_REDIS_KEY_PREFIX',
      'SDKWORK_CHAT_REDIS_TLS',
      'SDKWORK_CHAT_REDIS_MAX_CONNECTIONS',
    ],
  };
}
...
function serviceIntegrationFor(platform) {
  if (platform === 'windows') {
    return {
      ...
      manifest: 'service/windows/CrawChatServer.xml',
    };
  }
  if (platform === 'macos') {
    return {
      kind: 'launchd',
      manifest: 'service/macos/com.sdkwork.crawchat.server.plist',
    };
  }
  return {
    kind: 'systemd',
    manifest: 'service/linux/craw-chat-server.service',
  };
}

function validateCrawChatInstallPackagePlan(plan) {
  const issues = [];
...
function renderCrawChatInstallPackagePlan(plan) {
  return [
    `[craw-chat-install-packages] product: ${plan.product}`,
    `[craw-chat-install-packages] schema: ${plan.schemaVersion}`,
    `[craw-chat-install-packages] version: ${plan.version}`,
    `[craw-chat-install-packages] supported platforms: ${plan.platforms.join(', ')}`,
    `[craw-chat-install-packages] supported architectures: ${plan.architectures.join(', ')}`,
    `[craw-chat-install-packages] deployment modes: ${plan.deploymentModes.join(', ')}`,
    `[craw-chat-install-packages] packages: ${plan.packages.length}`,
    ...plan.packages.map((packageItem) => [
      `[craw-chat-install-packages]   ${packageItem.id}`,
...
async function main(argv = process.argv.slice(2)) {
  ...
  const plan = createCrawChatInstallPackagePlan({
    architectures: settings.architectures,
    deploymentModes: settings.deploymentModes,
    platforms: settings.platforms,
    version: settings.version,
  });
  const issues = validateCrawChatInstallPackagePlan(plan);
  if (settings.json) {
    ...
    for (const line of renderCrawChatInstallPackagePlan(plan)) {
      console.log(line);
    }
    if (issues.length > 0) {
      console.error('[craw-chat-install-packages] validation issues:');
      for (const issue of issues) {
        console.error(`[craw-chat-install-packages]   ${issue}`);
      }
    } else if (settings.check) {
      console.log('[craw-chat-install-packages] validation passed');
    }
  }
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().then((code) => {
    ...
    console.error(`[craw-chat-install-packages] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
