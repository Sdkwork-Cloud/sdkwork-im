...
import { collectCrawChatDesktopBundles } from './collect-craw-chat-desktop-bundles.mjs';
...
} from './plan-craw-chat-install-packages.mjs';

const __filename = fileURLToPath(import.meta.url);
const repoRoot = path.resolve(path.dirname(__filename), '..', '..');
const STAGING_SCHEMA_VERSION = '2026-06-04.craw-chat.release-staging.v1';
...
function printHelp() {
  console.log(`Usage: node scripts/release/stage-craw-chat-release-package.mjs [options]

Stage one Craw Chat server archive or desktop bundle package from production build outputs.
...
function createCrawChatReleaseStagingPlan({
  packageId = currentHostServerPackageId(),
  root = repoRoot,
  stagingRoot = null,
  version = DEFAULT_RELEASE_VERSION,
} = {}) {
  const installPlan = createCrawChatInstallPackagePlan({ version });
  const planIssues = validateCrawChatInstallPackagePlan(installPlan);
...
function createServerStagingActions({ packageItem, root, stagingRoot }) {
  ...
  actions.push(
    ...
    copyAction(
      path.join(root, 'deployments', 'systemd', 'craw-chat-server.service'),
      path.join(stagingRoot, 'service', 'linux', 'craw-chat-server.service'),
      'linux service template',
      true,
      stagingRoot,
    ),
    copyAction(
      path.join(root, 'deployments', 'launchd', 'com.sdkwork.crawchat.server.plist'),
      path.join(stagingRoot, 'service', 'macos', 'com.sdkwork.crawchat.server.plist'),
      'macos service template',
      true,
      stagingRoot,
    ),
    copyAction(
      path.join(root, 'deployments', 'windows-service', 'CrawChatServer.xml'),
      path.join(stagingRoot, 'service', 'windows', 'CrawChatServer.xml'),
      'windows service template',
      true,
      stagingRoot,
    ),
    copyAction(
      path.join(root, 'apps', 'sdkwork-chat-pc', 'dist'),
      path.join(stagingRoot, 'web', 'sdkwork-chat-pc', 'dist'),
      'sdkwork-chat-pc web dist',
      true,
      stagingRoot,
    ),
    generatedAction(
...
function createDesktopStagingActions({ packageItem, root, stagingRoot }) {
  const desktopManifest = collectCrawChatDesktopBundles({
...
function validateCrawChatReleaseStagingPlan(plan, { requireSources = true } = {}) {
  const issues = [];
...
async function stageCrawChatReleasePackage(plan) {
  const issues = validateCrawChatReleaseStagingPlan(plan, { requireSources: true });
  if (issues.length > 0) {
...
function createServerEnvExample(packageItem) {
  ...
  return [
    ...
    `SDKWORK_CHAT_CONFIG_FILE=${paths.configDir}/chat.toml`,
    `SDKWORK_CHAT_DATA_DIR=${paths.dataDir}`,
    `SDKWORK_CHAT_LOG_DIR=${paths.logDir}`,
    `SDKWORK_CHAT_RUN_DIR=${paths.runDir}`,
    ...
    `SDKWORK_CHAT_DATABASE_PASSWORD_FILE=${paths.configDir}/database.secret`,
    ...
    `SDKWORK_CHAT_REDIS_PASSWORD_FILE=${paths.configDir}/redis.secret`,
    'SDKWORK_CHAT_REDIS_KEY_PREFIX=chat',
    'SDKWORK_CHAT_REDIS_TLS=false',
    'SDKWORK_CHAT_REDIS_MAX_CONNECTIONS=16',
    'CRAW_CHAT_BROWSER_ORIGINS=https://chat.example.com',
    `CRAW_CHAT_ADMIN_SITE_DIR=${paths.installRoot}/web/sdkwork-chat-pc/dist`,
    `CRAW_CHAT_PORTAL_SITE_DIR=${paths.installRoot}/web/sdkwork-chat-pc/dist`,
    '',
  ].join('\n');
}
...
function createInstallGuide(packageItem) {
  ...
  return [
    ...
    '## Included Files',
    '',
    '- `bin/craw-chat-server` or `bin/craw-chat-server.exe`: release server binary.',
    ...
    '- `web/sdkwork-chat-pc/dist`: production PC web assets.',
    '',
    '## Quick Start',
    '',
...
function createInstallManifest(packageItem) {
  return {
    schemaVersion: '2026-06-04.craw-chat.install-manifest.v1',
...
async function main(argv = process.argv.slice(2)) {
  ...
  const plan = createCrawChatReleaseStagingPlan(settings);
  const issues = validateCrawChatReleaseStagingPlan(plan, { requireSources: !settings.dryRun });
  const payload = {
    ok: issues.length === 0,
    issues,
  ...
  const result = await stageCrawChatReleasePackage(plan);
  if (settings.json) {
    ...
    console.log(`[craw-chat-release-stage] staged: ${result.stagingRoot}`);
    console.log(`[craw-chat-release-stage] files: ${result.files.length}`);
  }
  return 0;
}

async function runAllStaging(settings) {
  const plans = createCrawChatInstallPackagePlan({ version: settings.version }).packages.map((packageItem) =>
    createCrawChatReleaseStagingPlan({
  ...
  const issues = plans.flatMap((plan) =>
    validateCrawChatReleaseStagingPlan(plan, { requireSources: !settings.dryRun })
  ...
  for (const plan of plans) {
    await stageCrawChatReleasePackage(plan);
  }
  return 0;
}
...
function renderStagingPlan(plan) {
  return [
    `[craw-chat-release-stage] package: ${plan.package.id}`,
    `[craw-chat-release-stage] staging root: ${plan.stagingRoot}`,
    ...plan.actions.map((action) => `[craw-chat-release-stage]   ${action.kind}: ${action.label} -> ${path.relative(plan.stagingRoot, action.targetPath)}`),
  ];
}
...
function printIssues(issues) {
  ...
  console.error('[craw-chat-release-stage] validation issues:');
  for (const issue of issues) {
    console.error(`[craw-chat-release-stage]   ${issue}`);
  }
}
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().then((code) => {
    ...
    console.error(`[craw-chat-release-stage] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
...
export {
  ...
  main,
  parseStagingArgs,
  stageCrawChatReleasePackage,
  validateCrawChatReleaseStagingPlan,
};
