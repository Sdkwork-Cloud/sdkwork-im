...
import { createCrawChatInstallPackagePlan } from './plan-craw-chat-install-packages.mjs';

function printHelp() {
  console.log(`Usage: node scripts/release/validate-craw-chat-install-artifacts.mjs --package-id <id> --artifact-path <path> [options]

Validate Craw Chat release package payload layout before GitHub Release upload.
...
function validateCrawChatInstallArtifact({
  artifactPath,
  packageId,
  root = process.cwd(),
  version = DEFAULT_RELEASE_VERSION,
  ...
  const packageItem = createCrawChatInstallPackagePlan({ version }).packages.find((item) => item.id === packageId);
  if (!packageItem) {
...
function validateArchiveEntries(packageItem, entries) {
  ...
  if (packageItem.deploymentMode === 'server-archive') {
    ...
    requireEntryPrefix(entries, 'web/sdkwork-chat-pc/dist/', packageItem.id, issues);
...
async function main(argv = process.argv.slice(2)) {
  ...
  const result = validateCrawChatInstallArtifact(settings);
  if (settings.json) {
    console.log(JSON.stringify(result, null, 2));
  } else if (result.ok) {
    console.log(`[craw-chat-install-artifact-validate] ok: ${result.packageId} ${result.artifactName}`);
  } else {
    console.error(`[craw-chat-install-artifact-validate] failed: ${result.packageId ?? '(missing package id)'}`);
    for (const issue of result.issues) {
      console.error(`[craw-chat-install-artifact-validate]   ${issue}`);
    }
  }
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().then((code) => {
    ...
    console.error(`[craw-chat-install-artifact-validate] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
