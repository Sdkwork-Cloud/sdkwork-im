...
import { createCrawChatInstallPackagePlan, validateCrawChatInstallPackagePlan } from './plan-craw-chat-install-packages.mjs';
import { currentHostServerPackageId } from './stage-craw-chat-release-package.mjs';

const __filename = fileURLToPath(import.meta.url);
const repoRoot = path.resolve(path.dirname(__filename), '..', '..');
const BUILD_SCHEMA_VERSION = '2026-06-04.craw-chat.install-package-build.v1';
const PACKAGE_MANIFEST_SCHEMA_VERSION = '2026-06-04.craw-chat.release-package-manifest.v1';
const AGGREGATE_MANIFEST_SCHEMA_VERSION = '2026-06-04.craw-chat.release-packages-manifest.v1';
...
function printHelp() {
  console.log(`Usage: node scripts/release/build-craw-chat-install-package.mjs [options]

Build Craw Chat release archives from staged server or desktop package files.
...
function createCrawChatInstallPackageBuildPlan({
  packageId = currentHostServerPackageId(),
  outputDir = null,
  ...
  if (packageItem.deploymentMode === 'desktop') {
    ...
    for (const artifact of packageItem.artifacts) {
      if (artifact.path === 'bin') {
        ...
        plannedPaths.add('service/linux/craw-chat-server.service');
        plannedPaths.add('service/macos/com.sdkwork.crawchat.server.plist');
        plannedPaths.add('service/windows/CrawChatServer.xml');
      } else {
...
function validateCrawChatInstallPackageBuildPlan(buildPlan) {
  const issues = [];
...
async function buildCrawChatInstallPackageArchive(buildPlan) {
  const issues = validateCrawChatInstallPackageBuildPlan(buildPlan);
  if (issues.length > 0) {
...
async function main(argv = process.argv.slice(2)) {
  ...
  const buildPlan = createCrawChatInstallPackageBuildPlan({
  ...
  const result = await buildCrawChatInstallPackageArchive(buildPlan);
  if (settings.json) {
    console.log(JSON.stringify({
      ok: true,
    ...
    console.log(`[craw-chat-install-package] written: ${result.archivePath}`);
    console.log(`[craw-chat-install-package] sha256: ${result.archive.sha256}`);
  }
  return 0;
}

async function runAllInstallPackageBuilds(settings) {
  const packageIds = createCrawChatInstallPackagePlan({ version: settings.version }).packages.map((item) => item.id);
  const plans = packageIds.map((packageId) => createCrawChatInstallPackageBuildPlan({
    packageId,
    outputDir: settings.outputDir,
    requireStagedFiles: !settings.dryRun,
    root: repoRoot,
  ...
  const issues = plans.flatMap((plan) =>
    validateCrawChatInstallPackageBuildPlan(plan).map((issue) => `${plan.package.id}: ${issue}`)
  );
  if (settings.json && (settings.check || settings.dryRun)) {
    console.log(JSON.stringify({
      ok: issues.length === 0,
  ...
  const results = [];
  for (const plan of plans) {
    results.push(await buildCrawChatInstallPackageArchive(plan));
  }
  if (settings.json) {
    console.log(JSON.stringify({
      ok: true,
...
function renderInstallPackageBuildPlan(buildPlan) {
  return [
    `[craw-chat-install-package] package: ${buildPlan.package.id}`,
    `[craw-chat-install-package] archive: ${buildPlan.archivePath}`,
    `[craw-chat-install-package] manifest: ${buildPlan.manifestPath}`,
    `[craw-chat-install-package] entries: ${buildPlan.entries.length}`,
    ...buildPlan.entries.map((entry) => `[craw-chat-install-package]   ${entry.archivePath}${entry.planned ? ' (planned)' : ''}`),
  ];
}
...
function printIssues(issues) {
  ...
  console.error('[craw-chat-install-package] validation issues:');
  for (const issue of issues) {
    console.error(`[craw-chat-install-package]   ${issue}`);
  }
}
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().then((code) => {
    ...
    console.error(`[craw-chat-install-package] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
...
export {
  ...
  SHA256SUMS_FILE,
  buildCrawChatInstallPackageArchive,
  createAggregateManifest,
  createCrawChatInstallPackageBuildPlan,
  ...
  sha256,
  validateCrawChatInstallPackageBuildPlan,
};
