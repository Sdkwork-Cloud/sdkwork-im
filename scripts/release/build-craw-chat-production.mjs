...
function printHelp() {
  console.log(`Usage: node scripts/release/build-craw-chat-production.mjs [options]

Build Craw Chat production artifacts for server archives and desktop installers.
...
function createCrawChatProductionBuildPlan({
  arch = null,
  env = process.env,
  platform = null,
  root = repoRoot,
  ...
  if (includeWebAssets) {
    steps.push({
      label: 'build sdkwork-chat-pc web assets',
      command: pnpmCommand(),
      args: ['release:build'],
      cwd: root,
      env,
  ...
  if (includeServer) {
    steps.push({
      label: 'build craw-chat-server release binary',
      command: cargoCommand(),
      args: ['build', '-p', 'web-gateway', '--bin', 'craw-chat-server', '--release'],
      cwd: root,
      env,
    });
  }
  ...
  if (includeDesktop) {
    ...
    steps.push({
      ...
      args: [
        '--dir',
        'apps/sdkwork-chat-pc/packages/sdkwork-clawchat-pc-desktop',
        'desktop:build:local',
        '--',
        '--target',
  ...
  return {
    schemaVersion: '2026-06-04.craw-chat.production-build.v1',
    target,
    root,
    steps,
    expectedArtifacts: {
      serverBinary: path.join(root, 'target', 'release', process.platform === 'win32' ? 'craw-chat-server.exe' : 'craw-chat-server'),
      pcWebDist: path.join(root, 'apps', 'sdkwork-chat-pc', 'dist'),
...
function renderCrawChatProductionBuildPlan(plan) {
  return [
    `[craw-chat-production-build] target: ${plan.target}`,
    `[craw-chat-production-build] root: ${plan.root}`,
    ...plan.steps.map((step) => `[craw-chat-production-build]   ${step.label}: ${step.command} ${step.args.join(' ')}`),
    `[craw-chat-production-build] expected server binary: ${plan.expectedArtifacts.serverBinary}`,
    `[craw-chat-production-build] expected web dist: ${plan.expectedArtifacts.pcWebDist}`,
    `[craw-chat-production-build] expected desktop bundle root: ${plan.expectedArtifacts.desktopBundleRoot}`,
  ];
}

function runStep(step) {
  return new Promise((resolve, reject) => {
    console.error(`[craw-chat-production-build] ${step.label}: ${step.command} ${step.args.join(' ')}`);
...
async function main(argv = process.argv.slice(2)) {
  ...
  const plan = createCrawChatProductionBuildPlan(settings);
  if (settings.json) {
    ...
    for (const line of renderCrawChatProductionBuildPlan(plan)) {
      console.log(line);
    }
  }
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().then((code) => {
    ...
    console.error(`[craw-chat-production-build] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  });
}
...
export {
  cargoCommand,
  createCrawChatProductionBuildPlan,
  main,
  parseProductionBuildArgs,
  pnpmCommand,
  renderCrawChatProductionBuildPlan,
