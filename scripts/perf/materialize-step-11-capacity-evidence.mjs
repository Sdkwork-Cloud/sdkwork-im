...
const artifacts = [
  {
    ...
    value: {
      ...
      collectedAt,
      sourceProfile: 'self-hosted.split-services.development',
      ...
      connectP99Ms: 16.892,
      boundary:
        'This artifact backfills published local CP11-2 connection evidence into the Capacity Tier artifact root using capacity_json field names. It is doc-captured from CI Smoke Tier / self-hosted.split-services.development output rather than a dedicated capacity-dedicated topology run.',
    },
  },
  {
...
for (const artifact of artifacts) {
  ...
  const content = artifact.markdownSections
    .map((section) => `## ${section}\n\nDoc-captured Capacity Tier ${section} for self-hosted.split-services.development evidence backfill on ${collectedAt}.`)
    .join('\n\n');
...
- boundary: this root now carries all seven truthful local Capacity Tier artifacts. \`Capacity Tier\` is \`${indexJson.state}\` after doc-captured backfill from published CI Smoke Tier / self-hosted.split-services.development evidence.
