...
- boundary: this root now carries all seven truthful local artifacts. `Pre-Release Tier` is `evidence_collected_gate_blocked`, not full gate sign-off, because the artifacts are backfilled from published CI Smoke Tier / local-minimal evidence while `Capacity Tier` is now `evidence_collected_gate_passed` from the same local doc-capture backfill.
