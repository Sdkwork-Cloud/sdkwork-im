...
fn test_commit_envelope_normalizes_organization_id() {
    let envelope = CommitEnvelope::minimal("evt_demo", "t_demo", "message.posted", "conversation", "c_demo", 1)
        .with_organization_id("org_a");
    assert_eq!(envelope.normalized_organization_id(), "org_a");
    assert_eq!(
        CommitEnvelope::minimal("evt_demo", "t_demo", "message.posted", "conversation", "c_demo", 1)
