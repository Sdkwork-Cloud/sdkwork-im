...
mod tests {
    ...
    fn test_disconnect_fence_record_merge_keeps_latest_disconnect() {
        let latest = RealtimeDisconnectFenceRecord {
            ...
            fence_token: "t_demo:user:u_demo:d_pad:2026-05-06T00:00:02.000Z:s_new".into(),
        };
        let stale = RealtimeDisconnectFenceRecord {
            ...
            fence_token: "t_demo:user:u_demo:d_pad:2026-05-06T00:00:01.000Z:s_old".into(),
        };
        ...
        assert_eq!(
            merged.fence_token,
            "t_demo:user:u_demo:d_pad:2026-05-06T00:00:02.000Z:s_new"
        );
    }
    ...
    fn test_realtime_event_window_diagnostics_lists_top_high_risk_windows_without_payloads() {
        let snapshot = RealtimeEventWindowDiagnosticsSnapshot::from_records(vec![
            realtime_window_record("t_demo", "user", "u_demo", "d_one", 3, 0),
            realtime_window_record("t_demo", "user", "u_demo", "d_two", 7, 7),
            realtime_window_record("t_demo", "agent", "a_demo", "d_agent", 5, 0),
            realtime_window_record("t_demo", "user", "u_demo", "d_empty", 0, 9),
            realtime_window_record("t_demo", "user", "u_demo", "d_three", 6, 0),
            realtime_window_record("t_demo", "user", "u_demo", "d_four", 4, 0),
    ...
    fn test_realtime_principal_scope_key_extends_client_route_scope_prefix() {
        let principal = realtime_principal_scope_key("t_demo", "org_a", "user", "u_demo");
        let client_route =
            realtime_client_route_scope_key("t_demo", "org_a", "user", "u_demo", "d_pad");
        assert!(
            client_route.starts_with(principal.as_str()),
            "client route scope key must extend principal scope key prefix"
        );
        assert_ne!(
            realtime_principal_scope_key("t_demo", "org_a", "user", "u_demo"),
            realtime_principal_scope_key("t_demo", "org_b", "user", "u_demo"),
        );
    }
