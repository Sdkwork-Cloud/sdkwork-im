...
use im_storage_runtime::{
    storage_config_upsert_from_input, StoreBackedStorageRuntime, StorageConfigUpsert,
    StorageRuntimeState,
};
...
impl StorageDomainSnapshotStore for MemoryStorageSnapshotStore {
    fn load_snapshot(&self, _domain: &str) -> Result<Option<StorageDomainSnapshot>, ContractError> {
        Ok(self.snapshot.lock().expect("snapshot mutex should lock").clone())
    }
...
fn runtime_state_supports_global_and_tenant_storage_resolution() {
    ...
    assert_eq!(effective_tenant.binding.provider_plugin_id, "object-storage-google");
    ...
    assert_eq!(effective_global.binding.provider_plugin_id, "object-storage-aws");
}
...
fn runtime_state_validates_mode_specific_secret_fields_from_stored_records() {
    ...
    assert_eq!(validation.message, "Interoperability Secret Key is required.");
}
...
fn runtime_state_restores_validation_and_fallback_from_domain_snapshot() {
    let snapshot = StorageDomainSnapshot::new(StorageCatalog::object_storage())
        ...
        .with_config(
            im_storage_contracts::StorageConfigRecord::new_global("object-storage-aws"),
        )
    ...
    assert_eq!(invalid_validation.stage, StorageValidationStage::Credentials);
    assert_eq!(
...
fn runtime_state_loads_default_catalog_when_store_is_empty() {
    ...
    let state =
        StorageRuntimeState::load_from_store(&store, StorageCatalog::object_storage())
...
fn store_backed_runtime_persists_global_and_tenant_mutations_through_shared_store() {
    ...
    let restored = StoreBackedStorageRuntime::load(
        runtime.store().clone(),
