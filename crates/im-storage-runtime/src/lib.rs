...
use im_storage_contracts::{
    ...
    StorageConfigSnapshot, StorageConfigUpsertInput, StorageCredentialMode,
    StorageDomainSnapshot, StorageDomainSnapshotStore, StorageEffectiveConfig,
    StorageProviderSchema, StorageSchemaField, StorageScopeKind, StorageScopeRef,
    StorageSecretRecord, StorageValidationResult, StorageValidationStage,
...
impl StorageRuntimeState {
    ...
    pub fn delete_tenant(&mut self, tenant_id: &str) -> bool {
        ...
        self.record_audit("delete", &scope, removed.binding.provider_plugin_id.as_str());
        true
    }
    ...
    pub fn effective_for_tenant(&self, tenant_id: &str) -> Option<StorageEffectiveConfig> {
        ...
        self.tenant_configs
            ...
            .map(|record| StorageEffectiveConfig {
                ...
                secret: record.secret.as_ref().map(StorageSecretRecord::redacted_summary),
            })
    }
    ...
    fn normalize_record(
        ...
        let binding = match scope.kind {
            StorageScopeKind::Global => StorageBindingRecord::new_global(
        ...
        let secret = match input.secret {
            ...
            None => existing_secret.filter(|secret| {
                secret.provider_plugin_id == input.provider_plugin_id
            }),
        };
...
pub fn storage_config_upsert_from_input(
    ...
    if let Some(bucket_or_container) = trimmed_optional_string(input.config.bucket_or_container.as_deref()) {
...
fn validate_storage_scope_record(
    ...
    let provider_schema = catalog
        ...
        .ok_or_else(|| StorageInputValidationError::schema(format!(
...
{
    for field in provider_schema.common_fields.iter().filter(|field| field.required) {
...
fn validate_secret_payload(
    ...
    for field in provider_schema.credential_fields.iter().filter(|field| {
        field.required && field_applies_to_credential_mode(field, credential_mode)
    }) {
