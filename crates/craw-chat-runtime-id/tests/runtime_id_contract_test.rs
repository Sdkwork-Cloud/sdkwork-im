...
fn runtime_id_config_requires_explicit_distributed_node_id() {
    ...
    let invalid = RuntimeIdConfig::from_env_pairs([(SDKWORK_CHAT_ID_NODE_ID_ENV, "not-a-number")]);
    assert!(matches!(
        invalid,
        Err(RuntimeIdError::InvalidNodeIdConfig { .. })
    ));

    let out_of_range = RuntimeIdConfig::from_env_pairs([(SDKWORK_CHAT_ID_NODE_ID_ENV, "1024")]);
    assert!(matches!(
        out_of_range,
    ...
    let config = RuntimeIdConfig::from_env_pairs([(SDKWORK_CHAT_ID_NODE_ID_ENV, "42")])
