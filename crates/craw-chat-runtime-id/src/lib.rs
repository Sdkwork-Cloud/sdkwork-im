...
impl RuntimeIdConfig {
        ...
        V: AsRef<str>,
    {
        let Some(raw_node_id) = pairs.into_iter().find_map(|(name, value)| {
            (name.as_ref() == SDKWORK_CHAT_ID_NODE_ID_ENV).then(|| value.as_ref().trim().to_owned())
        }) else {
        ...
        let node_id =
            raw_node_id
                ...
                .map_err(|error| RuntimeIdError::InvalidNodeIdConfig {
                    env_name: SDKWORK_CHAT_ID_NODE_ID_ENV,
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingNodeId => write!(
                formatter,
                "{SDKWORK_CHAT_ID_NODE_ID_ENV} is required for runtime Snowflake ID generation"
            ),
