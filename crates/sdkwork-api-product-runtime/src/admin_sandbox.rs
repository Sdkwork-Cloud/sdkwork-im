...
use im_storage_runtime::{
    storage_config_upsert_from_input, StoreBackedStorageRuntime,
};
...
impl SharedAdminSandboxState {
    pub fn seeded() -> Self {
        Self {
            inner: Arc::new(Mutex::new(AdminSandboxState::seeded(None))),
        }
    }

    pub fn seeded_with_storage_file(file_path: impl Into<PathBuf>) -> Self {
        Self {
            inner: Arc::new(Mutex::new(AdminSandboxState::seeded(Some(
                file_path.into(),
            )))),
        }
    }
...
pub async fn handle_admin_sandbox_request(
    ...
    if method == Method::POST {
        match segment_refs.as_slice() {
            ...
            ["storage", "config"] => match serde_json::from_value::<StorageConfigUpsertInput>(
                input.clone(),
            ) {
                Ok(storage_input) => match storage_config_upsert_from_input(
                    ...
                    Ok(storage_upsert) => match guard.storage_runtime.save_global(storage_upsert) {
                        Ok(saved_snapshot) => {
...
mod tests {
    ...
    async fn sandbox_manages_storage_providers_configs_validation_and_fallback() {
        ...
        let aws_provider = providers
            .iter()
            .find(|provider| string_field(provider, "providerPluginId") == Some("object-storage-aws"))
        ...
        assert!(aws_common_fields.iter().any(|field| {
            string_field(field, "name") == Some("bucketOrContainer")
        }));
        ...
        assert_eq!(
            string_field(&global_validation, "status"),
            Some("healthy")
        );
        assert_eq!(
            string_field(&global_validation, "stage"),
            Some("presign")
        );
        ...
        assert_eq!(
            string_field(&tenant_validation, "status"),
            Some("healthy")
        );
        assert_eq!(
    ...
    async fn sandbox_rejects_storage_credentials_missing_required_mode_fields() {
        ...
        let payload = payload.expect("invalid storage credential save should return an error payload");
        assert_eq!(
            payload
                .get("error")
