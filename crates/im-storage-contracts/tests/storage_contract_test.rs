...
fn tenant_override_resolves_before_global_fallback() {
    ...
    let snapshot = StorageDomainSnapshot::new(catalog)
        ...
        .with_config(StorageConfigRecord::new_tenant("tenant-a", "object-storage-google"))
    ...
    assert_eq!(effective.resolved_scope, StorageScopeRef::tenant("tenant-a"));
    assert_eq!(effective.binding.provider_plugin_id, "object-storage-google");
    assert_eq!(
        effective
            .secret
            .as_ref()
