...
use sdkwork_rpc_client::{
    connect_grpc_channel, pick_endpoint, CompositeNameResolver, DiscoveryNameResolver,
    DiscoveryNameResolverConfig, LoadBalanceAlgorithm, NameResolver, RoundRobinCursor,
    StaticNameResolver, WatchingDiscoveryNameResolver,
};
...
pub fn build_im_rpc_name_resolver_from_env(
...
mod tests {
    ...
    fn discovery_profile_requires_discovery_endpoint_env() {
        ...
        let error =
            build_im_rpc_name_resolver_from_env().expect_err("missing discovery endpoint");
        assert!(matches!(error, RpcFrameworkError::Configuration(_)));
