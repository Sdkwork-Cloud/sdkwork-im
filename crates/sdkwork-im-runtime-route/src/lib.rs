...
fn route_key(tenant_id: &str, principal_id: &str, principal_kind: &str, device_id: &str) -> String {
    encode_route_key_segments([tenant_id, principal_kind, principal_id, device_id])
}

fn encode_route_key_segments<'a>(segments: impl IntoIterator<Item = &'a str>) -> String {
...
mod tests {
    ...
    fn test_bind_recovers_from_poisoned_routes_lock() {
        ...
        let result = panic::catch_unwind(AssertUnwindSafe(|| {
            directory.bind(
                RouteBindingRequest::new("t_demo", "u_demo", "user", "d_pad", "node_a")
    ...
    fn test_restore_if_current_restores_route_only_when_expected_binding_matches() {
        ...
        let restored = directory
            .lookup("t_demo", "u_demo", "user", "d_pad")
    ...
    fn test_restore_if_current_reconciles_drained_restore_owner_back_to_draining() {
        ...
        let temporary = directory
            .bind(
                RouteBindingRequest::new("t_demo", "u_demo", "user", "d_pad", "node_b")
                    .with_session_id(Some("s_demo"))
                    .with_connection_kind("http"),
            )
    ...
    fn test_route_directory_principal_identity_is_strictly_typed() {
        ...
        assert!(
            ...
            "RouteDirectory route key must use segment-safe length-prefixed encoding"
        );
        assert!(
            production_source.contains(
                "pub fn lookup(\n        &self,\n        tenant_id: &str,\n        principal_id: &str,\n        principal_kind: &str,"
    ...
    fn test_route_directory_key_encoding_is_segment_safe() {
        ...
        assert_eq!(
            directory
                .lookup("tenant", "principal:segment", "user", "device")
        ...
        assert_eq!(
            directory
                .lookup("tenant", "principal", "user", "segment:device")
